...
function fallbackMarketplaceName(options2) {
  ...
  return `cursor-marketplace-${(_a20 = options2.fallbackId) !== null && _a20 !== void 0 ? _a20 : (0, import_node_crypto4.randomUUID)()}`;
}
function readMarketplaceName(json2, options2) {
  if (isRecord(json2) && typeof json2.name === "string") {
    const name17 = normalizeMarketplaceName(json2.name);
    if (name17.length > 0) {
      return name17;
    }
  }
...
function readOwner(json2) {
  if (isRecord(json2) && isRecord(json2.owner)) {
    const name17 = typeof json2.owner.name === "string" && json2.owner.name.trim().length > 0 ? json2.owner.name : "Unknown";
    const email2 = typeof json2.owner.email === "string" && json2.owner.email.trim().length > 0 ? json2.owner.email : void 0;
    return Object.assign({ name: name17 }, email2 !== void 0 ? { email: email2 } : {});
  }
  return { name: "Unknown" };
}
...
function readMetadata(json2) {
  if (!isRecord(json2) || !isRecord(json2.metadata)) {
    return void 0;
  }
  const metadata = {
    description: readOptionalString(json2.metadata.description),
    version: readOptionalString(json2.metadata.version),
    pluginRoot: readOptionalString(json2.metadata.pluginRoot)
  };
...
function readAuthor(value) {
  ...
  const email2 = readOptionalString(value.email);
  return Object.assign({ name: name17 }, email2 !== void 0 ? { email: email2 } : {});
}
function readMcpServers(value) {
  ...
  if (Array.isArray(value)) {
    const servers = value.filter((item) => typeof item === "string" || isRecord(item));
...
function derivePluginNameFromSource(source) {
  ...
  if (typeof source === "string") {
    const name18 = normalizeMarketplaceName((0, import_node_path10.basename)(source));
    return name18.length > 0 ? name18 : void 0;
  }
  if (source.source === "github") {
    const name18 = normalizeMarketplaceName((_a20 = source.repo.split("/").at(-1)) !== null && _a20 !== void 0 ? _a20 : "");
    return name18.length > 0 ? name18 : void 0;
  }
  if (source.source === "git-subdir") {
    const name18 = normalizeMarketplaceName((0, import_node_path10.basename)(source.path));
    return name18.length > 0 ? name18 : void 0;
  }
  ...
  const name17 = normalizeMarketplaceName((0, import_node_path10.basename)(urlPath));
...
function parseMarketplaceManifest(content, options2 = {}) {
  ...
  const rawPlugins = Array.isArray(json2.plugins) ? json2.plugins : [];
  ...
  return {
    success: true,
    data: {
      name: readMarketplaceName(json2, options2),
      owner: readOwner(json2),
      description: readOptionalString(json2.description),
      plugins: validPlugins,
      metadata: readMetadata(json2),
    ...
    switch (source.source) {
      case "github":
        results.push({
          ...
          effectiveRef: (_a20 = source.sha) !== null && _a20 !== void 0 ? _a20 : source.ref
        });
        break;
      case "url":
        results.push({
...
function isPathSafe(path30) {
  if (path30.includes("..")) {
    return false;
  }
  if ((0, import_node_path10.isAbsolute)(path30)) {
    return false;
  }
  if (path30.includes("://")) {
    return false;
