...
function classifyCloneError(error3) {
  const msg = String(error3).toLowerCase().replaceAll("\\", "/");
...
function isKilledSubprocessError(error3) {
  if (typeof error3 !== "object" || error3 === null) {
    return false;
  }
  if (error3.killed === true) {
    return true;
  }
  return isKilledSubprocessError(error3.cause);
}
function cloneErrorTag(error3) {
  return isKilledSubprocessError(error3) ? "killed_by_budget" : classifyCloneError(error3);
}
function getCanonicalMarketplacePathSegments(gitUrl) {
  ...
  return {
    host: canonicalRepoHost(parsed.host.replace(/^www\./i, "")).toLowerCase(),
    pathSegments: [parsed.owner.toLowerCase(), parsed.repo.toLowerCase()]
  };
}
function manifestCacheKey(clonePath, options2) {
  ...
  return JSON.stringify({
    clonePath,
    repoName: (_a20 = options2.repoName) !== null && _a20 !== void 0 ? _a20 : null,
...
var MarketplaceCacheManager = class _MarketplaceCacheManager {
  ...
  get sparsePluginClones() {
    var _a20;
    var _b2;
    return (_b2 = (_a20 = this.options) === null || _a20 === void 0 ? void 0 : _a20.sparsePluginClones) !== null && _b2 !== void 0 ? _b2 : false;
  }
  ...
  cloneResolvedRef(cloneDir, gitUrl, resolvedRef, options2) {
    return __awaiter23(this, void 0, void 0, function* () {
      ...
      const execOpts = {
        ...
        extraGitConfig: Object.assign(Object.assign({}, resolveExtraGitConfig((_a20 = this.options) === null || _a20 === void 0 ? void 0 : _a20.extraGitConfig)), { "safe.directory": cloneDir })
      };
  ...
  createStagingDir() {
    return __awaiter23(this, void 0, void 0, function* () {
      const stagingDir = (0, import_node_path22.join)(this.cacheRoot, "_staging", (0, import_node_crypto6.randomUUID)());
      yield (0, import_promises12.mkdir)(stagingDir, { recursive: true });
      return stagingDir;
    });
  }
  /**
  ...
  moveToCanonicalDir(stagingDir, cloneDir) {
    return __awaiter23(this, void 0, void 0, function* () {
      yield (0, import_promises12.mkdir)((0, import_node_path22.dirname)(cloneDir), { recursive: true });
      yield (0, import_promises12.rm)(cloneDir, RECURSIVE_RM_OPTIONS);
      yield (0, import_promises12.rename)(stagingDir, cloneDir);
      return cloneDir;
    });
  }
  cleanStaleStagingDirs() {
    return __awaiter23(this, void 0, void 0, function* () {
      const stagingRoot = (0, import_node_path22.join)(this.cacheRoot, "_staging");
      try {
        const entries = yield (0, import_promises12.readdir)(stagingRoot, { withFileTypes: true });
        const now = Date.now();
        yield Promise.all(entries.filter((entry) => entry.isDirectory()).map((entry) => __awaiter23(this, void 0, void 0, function* () {
          try {
            const entryPath = (0, import_node_path22.join)(stagingRoot, entry.name);
            const stats = yield (0, import_promises12.stat)(entryPath);
            if (now - stats.mtimeMs > _MarketplaceCacheManager.STALE_STAGING_THRESHOLD_MS) {
              yield (0, import_promises12.rm)(entryPath, RECURSIVE_RM_OPTIONS);
            }
          } catch (_a20) {
          }
        })));
  ...
  getLegacyCloneDir(marketplaceId, ref) {
    return (0, import_node_path22.join)(this.cacheRoot, sanitize(marketplaceId), sanitize(ref));
  }
  getCanonicalCloneDir(gitUrl, ref) {
    const { host, pathSegments } = getCanonicalMarketplacePathSegments(gitUrl);
    return (0, import_node_path22.join)(this.cacheRoot, sanitize(host), ...pathSegments.map(sanitize), sanitize(ref));
  }
  getCanonicalRepoRootDir(gitUrl) {
    const { host, pathSegments } = getCanonicalMarketplacePathSegments(gitUrl);
    return (0, import_node_path22.join)(this.cacheRoot, sanitize(host), ...pathSegments.map(sanitize));
  }
  pruneSiblingCloneDirs(gitUrl, keepRef, legacyCloneDir) {
    return __awaiter23(this, void 0, void 0, function* () {
      ...
      try {
        const entries = yield (0, import_promises12.readdir)(repoRootDir, { withFileTypes: true });
        yield Promise.all(entries.filter((entry) => entry.isDirectory() && entry.name !== keepDirName).map((entry) => (0, import_promises12.rm)((0, import_node_path22.join)(repoRootDir, entry.name), RECURSIVE_RM_OPTIONS)));
      } catch (_a20) {
      }
      if (legacyCloneDir !== void 0) {
        try {
          yield (0, import_promises12.rm)(legacyCloneDir, RECURSIVE_RM_OPTIONS);
        } catch (_b2) {
        }
      }
    });
  ...
  ensureCloned(marketplaceId_1, gitUrl_1, ref_1) {
    return __awaiter23(this, arguments, void 0, function* (marketplaceId, gitUrl, ref, pluginLogger = noopPluginMetricsLogger, options2) {
      const inner = this.serializedOnRepo(this.getCanonicalRepoRootDir(gitUrl), () => {
        var _a20;
        return this.ensureClonedImpl(marketplaceId, gitUrl, ref, pluginLogger, (_a20 = options2 === null || options2 === void 0 ? void 0 : options2.materialize) !== null && _a20 !== void 0 ? _a20 : "all");
      });
  ...
  ensureMaterialized(clonePath_1, spec_1) {
    return __awaiter23(this, arguments, void 0, function* (clonePath, spec, pluginLogger = noopPluginMetricsLogger) {
      const inner = this.serializedOnRepo((0, import_node_path22.dirname)(clonePath), () => __awaiter23(this, void 0, void 0, function* () {
        var _a20;
        const start = performance.now();
        try {
          yield materializeSparseDirs(clonePath, spec, {
            extraGitConfig: resolveExtraGitConfig((_a20 = this.options) === null || _a20 === void 0 ? void 0 : _a20.extraGitConfig),
  ...
  ensureClonedImpl(marketplaceId, gitUrl, ref, pluginLogger, materialize2) {
    return __awaiter23(this, void 0, void 0, function* () {
      var _a20, _b2, _c2, _d;
      ...
      if (sshCloneUrl !== null) {
        try {
          ...
          if (this.sparsePluginClones && isKilledSubprocessError(error3)) {
            throw error3;
          }
          pluginLogger.log("error", "Failed to resolve remote ref using SSH URL, falling back to HTTPS", {
            ...
            ref,
            error: String(error3),
            errorCategory: classifyCloneError(error3)
          });
          sshCloneUrl = null;
      ...
      if (yield this.isCloneComplete(cloneDir)) {
        ...
        if (cachedCloneIsSparse) {
          yield materializeSparseDirs(cloneDir, materialize2, {
      ...
      return this.cloneViaStaging(cloneDir, gitUrl, sshCloneUrl, resolvedRef, legacyCloneDir, ref, pluginLogger, emitMetric, materialize2);
    });
  }
  /**
   ...
   * Cross-process safe.
   */
  cloneViaStaging(cloneDir, gitUrl, sshCloneUrl, resolvedRef, legacyCloneDir, ref, pluginLogger, emitMetric, materialize2) {
    return __awaiter23(this, void 0, void 0, function* () {
      var _a20;
      ...
      const heartbeat = this.sparsePluginClones ? setInterval(() => {
        ...
        void (0, import_promises12.utimes)(stagingDir, now, now).catch(() => {
        });
      }, _MarketplaceCacheManager.STAGING_HEARTBEAT_INTERVAL_MS) : void 0;
      (_a20 = heartbeat === null || heartbeat === void 0 ? void 0 : heartbeat.unref) === null || _a20 === void 0 ? void 0 : _a20.call(heartbeat);
      try {
        let outcome;
        if (sshCloneUrl !== null) {
          try {
            ...
            pluginLogger.log("error", "Falling back to HTTPS clone due to SSH clone failure", {
              ...
              ref,
              error: String(error3),
              errorCategory: cloneErrorTag(error3)
            });
            yield (0, import_promises12.rm)(stagingDir, RECURSIVE_RM_OPTIONS);
            yield (0, import_promises12.mkdir)(stagingDir, { recursive: true });
  ...
  isCloneComplete(cloneDir) {
    return __awaiter23(this, void 0, void 0, function* () {
      try {
        const stats = yield (0, import_promises12.stat)(cloneDir);
        if (!stats.isDirectory())
          return false;
        const entries = yield (0, import_promises12.readdir)(cloneDir);
        if (!entries.includes(".git"))
          return false;
        if (entries.length > 1)
          return true;
        try {
          const sparseMarker = yield (0, import_promises12.stat)((0, import_node_path22.join)(cloneDir, ".git", "info", "sparse-checkout"));
  ...
  copyPluginToDir(sourcePath, targetDir) {
    return __awaiter23(this, void 0, void 0, function* () {
      yield (0, import_promises12.mkdir)(targetDir, { recursive: true });
      yield (0, import_promises12.cp)(sourcePath, targetDir, {
        recursive: true,
  ...
  readManifest(clonePath_1) {
    return __awaiter23(this, arguments, void 0, function* (clonePath, options2 = {}) {
      var _a20;
      const cacheKey3 = manifestCacheKey(clonePath, options2);
      if (this.manifestCache.has(cacheKey3)) {
        return (_a20 = this.manifestCache.get(cacheKey3)) !== null && _a20 !== void 0 ? _a20 : null;
      }
  ...
  resolvePluginPath(clonePath, pluginName) {
    return __awaiter23(this, void 0, void 0, function* () {
      ...
      if (!entry)
        return null;
      const resolved = resolvePluginSourcePath(entry.source, (_a20 = manifest.metadata) === null || _a20 === void 0 ? void 0 : _a20.pluginRoot);
      if (!resolved)
        return null;
...
function readMarketplaceManifestFromDir(dir_1) {
  return __awaiter23(this, arguments, void 0, function* (dir, options2 = {}) {
    for (const manifestRelPath of MARKETPLACE_MANIFEST_PATHS) {
      const manifestPath = (0, import_node_path22.join)(dir, manifestRelPath);
      let content;
      try {
        content = yield (0, import_promises12.readFile)(manifestPath, "utf-8");
      } catch (_a20) {
        continue;
      }
...
function discoverPluginsFromManifest(manifest) {
  var _a20;
  const plugins = [];
  const classified = parseAndClassifyManifestEntries(manifest.plugins, (_a20 = manifest.metadata) === null || _a20 === void 0 ? void 0 : _a20.pluginRoot);
