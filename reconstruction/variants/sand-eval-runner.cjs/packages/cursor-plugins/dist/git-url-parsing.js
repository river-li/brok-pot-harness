 ...
 * Source: ../packages/cursor-plugins/dist/git-url-parsing.js
...
function parseGitUrl(gitUrl) {
  ...
  if (isOriginGitHost(host) && ((_a20 = pathSegments[0]) === null || _a20 === void 0 ? void 0 : _a20.toLowerCase()) === "git") {
...
function parseAndValidateScmUrl(input) {
  ...
  if (isGitHubCompatibleHost(parsed)) {
    ...
    return {
      url: `https://${host}/${pathPrefix}${parsed.owner}/${parsed.repo}`,
      owner: parsed.owner,
      repo: parsed.repo,
      host,
      provider: "github"
...
function isGitHubCompatibleHost(parsed) {
  switch (parsed.provider) {
    case "github":
      return true;
    case "generic":
      if (isOriginGitHost(parsed.host)) {
        return false;
      }
      return detectNonGitHubProvider(parsed.host) === null;
    case "gitlab":
    case "bitbucket":
    case "azure_devops":
      return false;
    default: {
      const _exhaustive = parsed.provider;
      return false;
    }
  }
}
