 ...
 * Source: ../packages/shell-exec/dist/zsh-light.js
 * Bundle: sand-host/sand-eval-runner.cjs
 * See reconstruction-manifest.json for exact byte ranges. */
// @recovered-fragment 1/1
function getZshPath2() {
...
async function initZshLightState(options2) {
  const env = {
    ...process.env,
    ...SHELL_ENV_OVERRIDES,
    ...options2?.env
  ...
  const args = [
    ...
    "extendedglob",
    "-ilc",
    `${dump_zsh_state_light_default} builtin printf '${STATE_MARKER}\\n'; dump_zsh_state_light`
  ];
  const child = spawnWithSignal(getZshPath2(), args, {
    env,
...
var __addDisposableResource7, __disposeResources7, ZshLightState;
var init_zsh_light = __esm({
  "../packages/shell-exec/dist/zsh-light.js"() {
    "use strict";
    init_dist();
    init_dist3();
    init_core2();
    init_dump_zsh_state_light();
    init_output_limiter();
    init_policy_merge();
    init_sandbox();
    init_sudo();
    init_types4();
    __addDisposableResource7 = function(env, value, async) {
    ...
    ZshLightState = class _ZshLightState {
      cwd;
      state;
      ...
      clone(workingDirectory) {
        return new _ZshLightState(workingDirectory ?? this.cwd, this.state);
      }
      async *execute(ctx, command, options2) {
        const env_1 = { stack: [], error: void 0, hasError: false };
        try {
          const span = __addDisposableResource7(env_1, createSpan(ctx.withName("ZshLightState.execute")), false);
          ...
          const c = (
            ...
            `builtin export PATH="/usr/bin:/bin:/usr/sbin:/sbin\${PATH:+:$PATH}"; snap=$(command cat <&3); builtin unsetopt aliases 2>/dev/null; builtin unalias -m '*' 2>/dev/null || true; ${dump_zsh_state_light_default} builtin eval "$snap" && { builtin unsetopt nounset 2>/dev/null || true; builtin eval "\${__CURSOR_SANDBOX_ENV_RESTORE:-}" 2>/dev/null; builtin export PWD="$(builtin pwd)"; builtin setopt aliases 2>/dev/null; ${sudoAliasInjection}${core2}; }; COMMAND_EXIT_CODE=$?; dump_zsh_state_light >&4; builtin exit $COMMAND_EXIT_CODE`
          );
          const args = [
            "-c",
            c,
...
`;
              if (newState.startsWith(markerLine)) {
                ...
                const { cwd: nextCwd, rest } = splitPwdAndState(stateWithoutMarker);
