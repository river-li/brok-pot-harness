...
function cacheKey(ctx) {
  var _a20;
  return `${ctx.privacyMode}:${(_a20 = ctx.enforceRedaction) !== null && _a20 !== void 0 ? _a20 : "gate"}`;
}
...
function toRedactedCoreMessages(messages, modeOrContext) {
  return messages.map((msg) => toRedactedCoreMessage(msg, modeOrContext));
}
function fromRedactedCoreMessages(messages, purpose, opts) {
  return messages.map((msg) => fromRedactedCoreMessage(msg, purpose, opts));
}
...
function fromRedactedToolMessage(message, purpose, opts) {
  const content = message.content.map((part) => {
    ...
    const unwrappedExperimentalContent = (_a20 = part.experimental_content) === null || _a20 === void 0 ? void 0 : _a20.map((item) => {
