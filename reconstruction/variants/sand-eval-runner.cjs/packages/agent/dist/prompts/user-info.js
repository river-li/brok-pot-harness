...
function isSkillPath(path30) {
  const normalizedPath = path30.replace(/\\/g, "/");
...
function getRuleDir2(mdcPath) {
  ...
  const literalRuleDir = normalizeToUnixPath((0, import_node_path48.normalize)((0, import_node_path48.dirname)(normalizedPath)));
...
function buildFriendlyDate(date5, timeZone) {
  try {
...
function resolveMetaAgentNotesDirectory(props) {
  ...
  if (notesSessionId && notesSessionId.length > 0 && projectFolder && projectFolder.length > 0) {
    return (0, import_node_path48.join)(projectFolder, "agent-notes", notesSessionId);
  }
  return void 0;
}
