...
var __addDisposableResource52 = function(env, value, async) {
...
var __disposeResources52 = /* @__PURE__ */ (function(SuppressedError2) {
  return function(env) {
    function fail(e) {
...
var AnysphereAgent = class {
  ...
  async runStream(parentCtx, state, action, mcpTools, onStateUpdate, options2) {
    ...
    try {
      const spanCtxt = __addDisposableResource52(env_1, createSpan(parentCtx.withName("runStream")), false);
      ...
      const onStateUpdateWithFlush = async (innerCtx, state2) => {
        ...
        try {
          const span = __addDisposableResource52(env_2, createSpan(innerCtx.withName("onStateUpdateWithFlush")), false);
  ...
  async setupSingleStep(parentCtx, state, action, onStateUpdate) {
    ...
    try {
      const spanCtxt = __addDisposableResource52(env_3, createSpan(parentCtx.withName("setupSingleStep")), false);
      let ctx = withLogAttributes(spanCtxt.ctx, {
        action: action.action.case,
        modelName: this.config.modelId,
        ...this.config.canonicalModelName !== void 0 && {
      ...
      const onStateUpdateWithFlush = async (innerCtx, innerState) => {
        ...
        try {
          const span = __addDisposableResource52(env_4, createSpan(innerCtx.withName("onStateUpdateWithFlush")), false);
          await this.blobStore.flush(span.ctx);
  ...
  async executeToolCall(parentCtx, state, descriptor2, mcpTools, splitStepData, fileOperationLockManager, onStateUpdate) {
    ...
    try {
      const spanCtxt = __addDisposableResource52(env_5, createSpan(parentCtx.withName("executeToolCall")), false);
      ...
      __disposeResources52(env_5);
    }
  }
  async prepareSubagent(parentCtx, state, descriptor2, mcpTools, splitStepData, fileOperationLockManager) {
    const env_6 = { stack: [], error: void 0, hasError: false };
    try {
      const spanCtxt = __addDisposableResource52(env_6, createSpan(parentCtx.withName("prepareSubagent")), false);
      ...
      __disposeResources52(env_6);
    }
  }
  async finalizeStep(parentCtx, state, splitStepData, toolCallResults, _mcpTools, onStateUpdate) {
    const env_7 = { stack: [], error: void 0, hasError: false };
    try {
      const spanCtxt = __addDisposableResource52(env_7, createSpan(parentCtx.withName("finalizeStep")), false);
      const ctx = spanCtxt.ctx;
      const baseExecutor = new RedactedPromptToolExecutor(this.promptSession.getExecutor(), PrivacyMode.UNSPECIFIED);
      ...
      const onStateUpdateWithFlush = async (innerCtx, innerState) => {
        ...
        try {
          const span = __addDisposableResource52(env_8, createSpan(innerCtx.withName("onStateUpdateWithFlush")), false);
          await onStateUpdate?.(span.ctx, fromRedactedConversationStateStructure(innerState, PrivacyCapability.UNSAFE_ALWAYS_ALLOWED));
          await this.blobStore.flush(span.ctx);
        } catch (e_8) {
          env_8.error = e_8;
  ...
  async computeSummarizedConversationState(parentCtx, state, requestContext) {
    ...
    try {
      const spanCtxt = __addDisposableResource52(env_9, createSpan(parentCtx.withName("computeSummarizedConversationState")), false);
      const ctx = spanCtxt.ctx;
