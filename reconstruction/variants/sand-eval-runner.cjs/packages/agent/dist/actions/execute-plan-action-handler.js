...
var __disposeResources40 = /* @__PURE__ */ (function(SuppressedError2) {
  return function(env) {
    function fail(e) {
...
var ExecutePlanActionHandler = class extends AbstractUserMessageActionHandler {
  ...
  async resolvePlanFilePath(ctx, requestContext, action, planFileContent) {
    ...
    try {
      ...
      const writeResult2 = await this.resourceAccessor.get(writeExecutorResource).execute(ctx, new WriteArgs({
      ...
      if (writeResult2.result.case !== "success") {
        logger68.warn(ctx, "agent.execute_plan.plan_file_recreate_failed", {
          ...
          readResultCase,
          writeResultCase: writeResult2.result.case
        });
        return {
          planFilePath: providedPlanFilePath,
          recreatedPlanFilePath: void 0,
  ...
  async initializeConversation(ctx, action, rootPromptExecutor, stateHandler, mcpTools) {
    ...
    if (stateHandler.todos.length === 0) {
      try {
        const parsed = grayMatter(unredResolvedPlanContent);
...
To-do's from the plan have already been created. Do not create them again. Mark them as in_progress as you work, starting with the first one. Don't stop until you have completed all the to-dos.`);
  ...
  async handle(parentCtx, action, rootPromptExecutor, stateHandler, mcpTools, onStateUpdate) {
    ...
    try {
      const span = __addDisposableResource40(env_1, createSpan(parentCtx.withName("ExecutePlanActionHandler.handle")), false);
      const ctx = span.ctx;
      ...
      __disposeResources40(env_1);
    }
  }
  async handleSingleStep(parentCtx, action, rootPromptExecutor, stateHandler, mcpTools, onStateUpdate) {
    const env_2 = { stack: [], error: void 0, hasError: false };
    try {
      const span = __addDisposableResource40(env_2, createSpan(parentCtx.withName("ExecutePlanActionHandler.handleSingleStep")), false);
      const ctx = span.ctx;
      const { turn, mergedMcpTools, requestContext } = await this.initializeConversation(ctx, action, rootPromptExecutor, stateHandler, mcpTools);
      ...
      __disposeResources40(env_2);
    }
  }
  async handleModelStep(parentCtx, action, rootPromptExecutor, stateHandler, mcpTools, onStateUpdate) {
    const env_3 = { stack: [], error: void 0, hasError: false };
    try {
      const span = __addDisposableResource40(env_3, createSpan(parentCtx.withName("ExecutePlanActionHandler.handleModelStep")), false);
      const ctx = span.ctx;
