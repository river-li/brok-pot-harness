...
var McpExecToolNotFoundError = class extends CustomToolCallError {
  constructor(errorMessage4) {
    super(ToolErrorClassification.UNEXPECTED_ENVIRONMENT, {
      error: errorMessage4,
      clientVisibleErrorMessage: errorMessage4,
      modelVisibleErrorMessage: errorMessage4
    });
  }
};
...
var McpWithoutReadToolDefinitionError = class extends CustomToolCallError {
  constructor(error3, readToolDefReminder, classification = ToolErrorClassification.OTHER_ERROR) {
    const errorMessage4 = error3 instanceof Error ? error3.message : String(error3);
    super(classification, {
      error: `Error in call_mcp_tool: ${errorMessage4} without reading the tool definition`,
      clientVisibleErrorMessage: "Tool execution error",
      modelVisibleErrorMessage: `${errorMessage4}. ${readToolDefReminder}`
    });
    this.error = error3;
    this.readToolDefReminder = readToolDefReminder;
  }
...
var McpInvalidArgsToolDefinitionReminderError = class extends CustomToolCallError {
  constructor(error3, readToolDefReminder) {
    super(ToolErrorClassification.INVALID_ARGS, {
      error: error3.message,
      clientVisibleErrorMessage: "Tool execution error",
      modelVisibleErrorMessage: `${error3.message}. ${readToolDefReminder}`
    });
  }
};
function getWrappedMcpErrorClassification(error3) {
  if (error3 instanceof CustomToolCallError) {
    return error3.classification;
  }
  return ToolErrorClassification.OTHER_ERROR;
}
function isInvalidMcpArgumentsError(error3) {
  return getWrappedMcpErrorClassification(error3) === ToolErrorClassification.INVALID_ARGS;
}
function createMcpTransportError(errorMessage4, cause) {
  const classification = classifyMcpErrorMessage(errorMessage4);
  ...
  return new Error(errorMessage4, { cause });
}
...
function truncateMcpToolName(name17) {
  ...
  const hash = (0, import_node_crypto17.createHash)("sha256").update(name17).digest("hex").substring(0, HASH_SUFFIX_LENGTH);
...
async function raceMcpExecAgainstSteerRelease(args) {
  ...
  try {
    ...
    const execPromise = run(execCtx).then((execResult) => ({ kind: "settled", execResult }), (error3) => ({ kind: "errored", error: error3 }));
...
async function rejectSmartModeMcpPreflightBlock(ctx, args) {
  if (args.enabled && args.requestSmartModeApproval === true && args.approvalProvider === void 0) {
    ...
    if (args.agentType === AgentType.BACKGROUND) {
      const error4 = new ToolCallRejectedError(parentBlockReason);
      await args.onRejectWithoutNativeApproval?.(error4);
      throw error4;
    }
  ...
  if (decision.kind === "reject") {
    const error4 = new ToolCallRejectedError(decision.reason);
    await args.onRejectWithoutNativeApproval?.(error4);
    throw error4;
  }
  if (args.approvalProvider !== void 0 && args.requestSmartModeApproval === true) {
    ...
    const error4 = new ToolCallRejectedError(approvalDecision.reason ?? decision.reason);
    await args.onRejectWithoutNativeApproval?.(error4);
    throw error4;
  }
  const error3 = new ToolCallRejectedError(createSmartModeMcpBlockedAutonomousReason(decision.reason, args.useNestedMcpDetails === true));
  await args.onRejectWithoutNativeApproval?.(error3);
  throw error3;
}
var createMcpTool = (resourceAccessor, mcpToolDefinition, options2 = {}) => {
  ...
  const execute = async (parentCtx, interactionHandler, rawArgs, meta) => {
    ...
    try {
      const spanCtxt = __addDisposableResource31(env_1, createSpan(parentCtx.withName("mcpExecute")), false);
      const args = rawArgs;
...
The MCP server rejected these arguments as invalid. Before retrying, inspect this MCP tool's input schema/tool definition and rebuild the arguments from that schema.
  ...
  return {
    ...
    serializeError: (error3) => {
      if (error3 instanceof McpPermissionDeniedError) {
      ...
      if (error3 instanceof ToolCallRejectedError) {
        return createMcpToolCall(new McpToolCall({
          result: new McpToolResult({
            result: {
              case: "rejected",
              value: new McpRejected({ reason: error3.message })
            }
          })
        }));
      }
          ...
          result: new McpToolResult({
            result: {
              ...
              value: new McpToolError({
                error: error3.clientVisibleErrorMessage,
                readToolDefReminder: error3.modelVisibleErrorMessage
              })
            }
          })
        }));
        ...
        result: new McpToolResult({
          result: {
            ...
            value: new McpToolError({
              ...
              readToolDefReminder: errorMessage4
            })
          }
        })
      }));
...
var createCallMcpTool = (options2) => {
  ...
  const execute = async (parentCtx, interactionHandler, args, meta) => {
    ...
    try {
      const spanCtxt = __addDisposableResource31(env_2, createSpan(parentCtx.withName("callMcpToolExecute")), false);
      ...
      const emitErrorMetrics = (ctx, input) => {
        const { failureReason, retryable, calledWithoutReadDef: calledWithoutReadDef2, error: error3 } = input;
        const durationMs = Date.now() - startTime;
        emitCallMcpToolMetrics(ctx, {
          mcpMode,
          durationMs,
        ...
        if (error3 !== void 0) {
          reportMcpMetaToolFailure(ctx, error3, {
            tool: name17,
            failureReason,
            retryable,
            server: args.server,
...
The MCP server rejected these arguments as invalid. Before retrying, read the MCP server descriptor files for "${args.server}" and the tool definition for "${args.toolName}" so the arguments match the schema.
      ...
      if (allowInteractiveMcpAuth && args.toolName === MCP_AUTH_VIRTUAL_TOOL_NAME) {
        ...
        try {
          ...
          tracker.trackMcpToolCallResult(spanCtxt.ctx, {
            ...
            errorClassification: error3 instanceof Error ? getWrappedMcpErrorClassification(error3) : ToolErrorClassification.OTHER_ERROR
          });
          emitErrorMetrics(spanCtxt.ctx, {
            failureReason: error3 instanceof ToolCallRejectedError ? CALL_MCP_TOOL_FAILURE_REASONS.REJECTED : CALL_MCP_TOOL_FAILURE_REASONS.OTHER,
            retryable: false,
            calledWithoutReadDef,
            error: error3
          });
      ...
      return await interactionHandler.executeToolCall(spanCtxt.ctx, createMcpToolCall(baseToolCall), meta.toolCallId, async (ctx) => {
        ...
        const toolDefinitionBinding = smartModeApprovalProvider === void 0 ? void 0 : {
          ...
          hash: toolDescriptor === void 0 ? void 0 : (0, import_node_crypto17.createHash)("sha256").update(toolDefinitionText).digest("hex")
        };
        ...
        try {
          ...
          if (readToolDefReminder !== void 0) {
            logger53.error(spanCtxt.ctx, "Error in call_mcp_tool", { error: error3 });
            throw new McpWithoutReadToolDefinitionError(classifiedError, readToolDefReminder, getWrappedMcpErrorClassification(classifiedError));
          }
          if (isInvalidMcpArgumentsError(classifiedError)) {
            throw new McpInvalidArgsToolDefinitionReminderError(classifiedError, invalidArgsToolDefReminder);
...
Example:
{
  ...
  ${exampleDescriptionLine}"arguments": { "query": "example", "limit": 10 }
    ...
    serializeError: (error3) => {
      if (error3 instanceof McpWithoutReadToolDefinitionError) {
        return createMcpToolCall(new McpToolCall({
          result: new McpToolResult({
            result: {
              case: "error",
      ...
      if (error3 instanceof McpServerDoesNotExistError) {
        return createMcpToolCall(new McpToolCall({
          result: new McpToolResult({
            result: {
              case: "error",
              value: new McpToolError({
                error: error3.clientVisibleErrorMessage,
                readToolDefReminder: error3.readServerDefReminder
              })
            }
          })
        }));
      }
      if (error3 instanceof McpToolDoesNotExistError) {
        return createMcpToolCall(new McpToolCall({
          result: new McpToolResult({
            result: {
              case: "error",
              value: new McpToolError({
                error: error3.clientVisibleErrorMessage,
                readToolDefReminder: error3.readToolDefReminder
              })
            }
          })
        }));
    ...
    case "success":
      return {
        content: result.value.content.map((item) => {
          if (item.content.case === "text") {
            const textContent2 = item.content.value;
            if (textContent2.outputLocation) {
              const totalSize = Number(textContent2.outputLocation.sizeBytes);
              const lineCount = Number(textContent2.outputLocation.lineCount);
              const sizeStr = totalSize >= 1024 ? `${(totalSize / 1024).toFixed(1)} KB` : `${totalSize} bytes`;
              return {
                type: "text",
                text: `Large output has been written to: ${textContent2.outputLocation.filePath} (${sizeStr}, ${lineCount} lines)`
              };
            }
            return {
              type: "text",
