...
var __addDisposableResource49 = function(env, value, async) {
...
var __disposeResources49 = /* @__PURE__ */ (function(SuppressedError2) {
  return function(env) {
    function fail(e) {
...
function shouldSkipApproval(parsed) {
  return parsed.hostname === "cursor.com" && (parsed.pathname === "/docs" || parsed.pathname === "/docs.md" || parsed.pathname === "/llms.txt" || parsed.pathname.startsWith("/docs/"));
}
function getLocalNetworkRejectionMessage(parsed) {
  const hostname2 = parsed.hostname.toLowerCase();
  const hostDisplay = parsed.port ? `${hostname2}:${parsed.port}` : hostname2;
  ...
  if (isLoopbackIpHost(hostname2)) {
    return `Cannot fetch from localhost (${hostDisplay}) because this tool runs from an isolated server.`;
  }
  if (isPrivateIpHost(hostname2)) {
...
var createWebFetchTool = (webFetchService, promptVersion, options2) => {
  ...
  const execute = async (parentCtx, interactionHandler, rawArgs, meta) => {
    ...
    try {
      const spanCtxt = __addDisposableResource49(env_1, createSpan(parentCtx.withName("webFetchExecute")), false);
      const executeCore = async (ctx, args) => {
        ...
        if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
          throw new ToolCallArgParseError(`Invalid URL protocol: ${parsed.protocol} (must be http or https)`);
        }
        if (options2?.stripCredentialedUrls === true && (parsed.username !== "" || parsed.password !== "")) {
          parsed.username = "";
          parsed.password = "";
          args.url = parsed.toString();
        }
        const localRejection = getLocalNetworkRejectionMessage(parsed);
        if (localRejection) {
        ...
        const enforceSmartModePreflight = async (preflightCtx) => {
          ...
          if (args.requestSmartModeApproval === true && smartModeClassifierEnabled) {
            ...
            return new SmartModeApproval({
              requestId: (0, import_node_crypto26.randomUUID)(),
        ...
        const runPreflightAndQuery = async (preflightCtx) => {
          ...
          const skipApproval = smartModeApproval === void 0 && (smartModeClassifierAllowed || shouldSkipApproval(parsed));
      ...
      if (error3 instanceof ToolCallRejectedError) {
        return createWebFetchToolCall(new WebFetchToolCall({
          result: new WebFetchResult({
            result: {
              case: "rejected",
              value: new WebFetchRejected({ reason: error3.message })
            }
          })
        }));
      }
      const errorMessage4 = (() => {
        if (error3 instanceof ToolCallError) {
          return error3.clientVisibleErrorMessage;
        }
