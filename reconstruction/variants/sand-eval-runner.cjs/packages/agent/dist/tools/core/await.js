...
var __addDisposableResource44 = function(env, value, async) {
...
var __disposeResources44 = /* @__PURE__ */ (function(SuppressedError2) {
  return function(env) {
    function fail(e) {
  ...
  while (stack.length > 0) {
    ...
    for (const file of node.childrenFiles) {
      if (file.name === targetFileName) {
        return normalizePathJoin(node.absPath, file.name);
      }
    }
...
function isTimeoutLikeError(errorMessage4) {
...
function isNoRunningBackgroundSubagentError(errorMessage4) {
  const message = errorMessage4.toLowerCase();
...
async function readSnapshot(ctx, resourceAccessor, path30, toolCallId, machineId) {
  ...
  switch (readResult.result.case) {
    case "success": {
      const success = readResult.result.value;
      const content = success.output.case === "content" ? success.output.value : success.output.case === "data" ? import_node_buffer3.Buffer.from(success.output.value).toString("utf8") : "";
      const outputLength = Math.max(0, toNumberLike(success.fileSize));
      return {
        exists: true,
        content,
    ...
    case "permissionDenied":
      throw new Error(`Permission denied reading ${path30}`);
    case "rejected":
      throw new Error(readResult.result.value.reason || `Read rejected for ${path30}`);
    case "invalidFile":
      throw new Error(readResult.result.value.reason || `Invalid file: ${path30}`);
    case "error":
      throw new Error(readResult.result.value.error || `Read failed for ${path30}`);
    case void 0:
      throw new Error(`Unknown read result for ${path30}`);
    default: {
...
var createAwaitTool = (resourceAccessor, options2, promptVersion) => {
  ...
  const execute = async (parentCtx, interactionHandler, rawArgs, meta) => {
    ...
    try {
      const spanCtxt = __addDisposableResource44(env_1, createSpan(parentCtx.withName("awaitExecute")), false);
      ...
      return interactionHandler.executeToolCall(ctx, createAwaitToolCall(baseToolCall), meta.toolCallId, async (toolCtx) => {
        ...
        if (!isShellTaskId(taskId)) {
          ...
          const readSubagentTranscriptSnapshot = async (path30) => path30 ? readSnapshot(toolCtx, resourceAccessor, path30, meta.toolCallId).catch(() => ({
            exists: false,
            content: "",
            outputLength: 0
          ...
          const rawSubagentAwaitPromise = (async () => {
            ...
            switch (legacyResult.result.case) {
              case "success": {
                const success = legacyResult.result.value;
                return new SubagentAwaitResult({
                  result: {
                    case: "complete",
                    value: new SubagentAwaitComplete({
                      agentId: success.agentId || taskId,
              ...
              case "error": {
                ...
                const errorMessage4 = subagentError.error || "";
                if (isTimeoutLikeError(errorMessage4)) {
                  return new SubagentAwaitResult({
                    result: {
                      case: "stillRunning",
                ...
                if (isNoRunningBackgroundSubagentError(errorMessage4)) {
                  return new SubagentAwaitResult({
                    result: {
                      case: "notFound",
          ...
          let subagentAwaitResult;
          {
            ...
            try {
              const raced = await Promise.race([
                rawSubagentAwaitPromise.then((result) => ({ kind: "result", result }), (error3) => ({ kind: "error", error: error3 })),
          ...
          switch (subagentAwaitResult.result.case) {
            ...
            case "error": {
              const errorMessage4 = subagentAwaitResult.result.value.error || "";
              if (isTimeoutLikeError(errorMessage4) || isNoRunningBackgroundSubagentError(errorMessage4)) {
                throwIfAborted(toolCtx);
              }
              throw new Error(errorMessage4 || "Failed to await subagent task");
            }
            case void 0:
        ...
        while (true) {
          throwIfAborted(toolCtx);
          const snapshot = await readSnapshot(toolCtx, resourceAccessor, outputPath, meta.toolCallId, machineId).catch((error3) => {
            throw new Error(error3 instanceof Error ? error3.message : "Failed to read shell output");
          });
          if (!snapshot.exists) {
