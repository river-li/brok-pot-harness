...
init_dist();
init_zod();
var __addDisposableResource48 = function(env, value, async) {
...
var __disposeResources48 = /* @__PURE__ */ (function(SuppressedError2) {
  return function(env) {
    function fail(e) {
...
function preprocessTodoMerge(value) {
  ...
  const merge4 = preprocessLenientBoolean(value.merge);
  return merge4 === value.merge ? value : { ...value, merge: merge4 };
}
var todoToolSchemaForParsing = external_exports.preprocess(preprocessTodoMerge, strictTodoToolSchemaForParsing);
function renderUpdateTodosSuccessMessage(success, promptVersion) {
  ...
  if (success.todos.some((t) => t.status === TodoStatus.PENDING) && success.todos.every((t) => t.status !== TodoStatus.IN_PROGRESS)) {
...
<system_reminder>${FINISHED_TODO_CLEANUP_REMINDER}</system_reminder>`;
  ...
  if (promptVersion === "dsv3-1018") {
    ...
    if (wasMerge) {
      ...
      const todosJson = success.todos.map((t) => ({
        id: t.id,
        content: t.content,
...
var createUpdateTodosTool = (resourceAccessor, stateHandler, promptVersion = "latest") => {
  ...
  const execute = async (parentCtx, interactionHandler, argsStream, meta) => {
    ...
    try {
      const spanCtxt = __addDisposableResource48(env_1, createSpan(parentCtx.withName("updateTodosExecute")), false);
