...
function getPdfWorkerPool() {
  ...
  if (_pdfWorkerPool === void 0) {
    _pdfWorkerPool = new import_piscina2.Piscina({
      minThreads: 1,
      maxThreads: Math.max(1, Math.floor(import_node_os15.default.availableParallelism() / 4)),
...
async function extractPdfText(bytes) {
  ...
  const result = await getPdfWorkerPool().run(params, {
    filename: import_node_path54.default.join(workerDir2, `./pdf-worker.${extension2}`)
  });
  return result.text;
}
