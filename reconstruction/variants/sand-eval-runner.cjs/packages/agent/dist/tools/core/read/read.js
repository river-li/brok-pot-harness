 ...
 * Source: ../packages/agent/dist/tools/core/read/read.js
...
var __addDisposableResource46 = function(env, value, async) {
...
var __disposeResources46 = /* @__PURE__ */ (function(SuppressedError2) {
  return function(env) {
    function fail(e) {
...
function getReturnedLineCount(success) {
  if (success.exceededLimit) {
    return void 0;
  }
  const readRange = success.readRange;
  if (!readRange) {
    return void 0;
  }
  return Math.max(0, readRange.endLine - readRange.startLine + 1);
}
function getReturnedByteCount(success, blobBackedReturnedByteCount) {
  const output = success.output;
  if (!output || success.exceededLimit) {
    return void 0;
  }
  switch (output.case) {
    case "content":
...
function recordReadSuccessMetrics(ctx, success, readScope, repeatStatus, blobBackedReturnedByteCount, explicitDesc) {
  const labels = {
  ...
  readSuccessCounter.increment(ctx, 1, {
    ...labels,
    exceeded_limit: success.exceededLimit ? "true" : "false"
  });
  readTotalFileLinesHistogram.histogram(ctx, success.totalLines, labels);
  readFileSizeBytesHistogram.histogram(ctx, success.fileSize, labels);
  const returnedLines = getReturnedLineCount(success);
  if (returnedLines !== void 0) {
    readReturnedLinesHistogram.histogram(ctx, returnedLines, labels);
    if (success.totalLines > 0) {
      readCoveragePercentHistogram.histogram(ctx, returnedLines / success.totalLines * 100, labels);
    }
  }
  const returnedBytes = getReturnedByteCount(success, blobBackedReturnedByteCount);
...
function createOffsetSchema(options2 = {}) {
  ...
  const errorMessage4 = allowNegative ? "Offset must be >= 1 or <= -1." : "Offset must be >= 1.";
...
var createReadTool = (resourceAccessor, formattingOptions, promptVersion, options2) => {
  ...
  const execute = async (parentCtx, interactionHandler, rawArgs, meta) => {
    ...
    try {
      const spanCtxt = __addDisposableResource46(env_1, createSpan(parentCtx.withName("readExecute")), false);
      ...
      const path30 = maybeRedirectWorktriesPath(originalPath);
      trackMcpDirectoryAccessIfApplicable(spanCtxt.ctx, path30, "read");
      if (isAgentTranscriptPath(path30)) {
      ...
      if (UNSUPPORTED_BINARY_EXTENSIONS.test(path30)) {
        const ext2 = path30.toLowerCase().match(/\.([^.]+)$/)?.[1] ?? "unknown";
        return new ReadToolResult({
          result: {
            case: "error",
      ...
      if (readToolResult.result.case === "success") {
        ...
        const hasSeenReadPath = (path31) => (meta.stateHandler?.readPaths?.has(path31) ?? false) || (meta.stepReadPathDedup?.has(path31) ?? false);
        const rememberReadPath = (path31) => {
          meta.stateHandler?.recordReadPath(safeString(path31));
      ...
      if (isMcpDirectoryPath2(path30) && readToolResult.result.case === "success" && readToolResult.result.value.output?.case === "content") {
...
${body}`;
  ...
  const getRelatedCursorRules = (success) => success.relatedCursorRules ?? [];
  const relatedSkillsBySuccess = /* @__PURE__ */ new WeakMap();
  const getRelatedSkills = (success) => relatedSkillsBySuccess.get(success) ?? [];
...
${skillReminder}`;
            ...
            case "dataBlobId": {
              ...
              const { path: path30 } = result.value;
              const blobIdHex = toHex3(blobId);
              const blobData = hydratedBlobs.get(blobIdHex);
              if (!blobData) {
                throw new Error(`Image blob not hydrated for render: ${blobIdHex} (path: ${path30})`);
              }
              return renderImageBinary(blobData, path30);
            }
            case void 0:
