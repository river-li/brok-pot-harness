...
var logger35 = createLogger("remote-hooks");
function safeBuildRemoteHookContexts(ctx, toolName, toolCallId, hookEventName, additionalContext) {
  try {
    ...
    if (error3 instanceof HookAdditionalContextTooLargeError) {
      logger35.warn(ctx, `${hookEventName} additional_context exceeded max size; dropping carrier`, {
        toolName,
        toolCallId,
        hookEventName,
        actualLength: error3.actualLength,
        maxLength: error3.maxLength
      });
      return [];
    }
    throw error3;
...
function isHookExecutionTimeout(error3) {
...
function isTimeoutError(error3) {
  return error3 instanceof ToolTimeoutError || error3 instanceof Error && error3.name === "TimeoutError";
}
...
async function executeRemoteAfterAgentThoughtHook(args) {
  ...
  try {
    ...
    const durationMs = Math.round(performance.now() - startTime);
    const timedOut = isHookExecutionTimeout(error3);
    ...
    logger35.warn(ctx, "afterAgentThought hook execution failed", {
...
async function executeRemoteSubagentStartHook(args) {
  ...
  logger35.info(ctx, "subagentStart hook firing", {
    subagentId,
    subagentType,
    toolCallId: requestContext.toolCallId
  });
  ...
  try {
    ...
    if (result.response?.response.case !== "subagentStart") {
      logger35.warn(ctx, "subagentStart hook returned unexpected response", {
        subagentId,
        subagentType,
        toolCallId: requestContext.toolCallId,
    ...
    logger35.info(ctx, "subagentStart hook completed", {
      subagentId,
      subagentType,
      toolCallId: requestContext.toolCallId,
    ...
    getAgentEventTracker(ctx).trackHookExecuted(ctx, {
      ...
      hookSource: "remote",
      hookType: "subagentStart",
      status: isHookExecutionTimeout(error3) ? "timeout" : "failed",
      latencyMs: durationMs,
      failClosed: false,
      timedOut: isHookExecutionTimeout(error3)
    });
    throw error3;
  }
}
async function executeRemoteSubagentStopHook(args) {
  const { ctx, subagentId, subagentType, status, durationMs: subagentDurationMs, summary, parentConversationId: providedParentConversationId, messageCount, toolCallCount: toolCallCount2, errorMessage: errorMessage4, modifiedFiles, gitBranch, loopCount, task, description: description9, requestContext, options: options2 } = args;
  ...
  const { conversationId, generationId } = buildRequestIds(ctx, requestContext);
  const parentConversationId = providedParentConversationId ?? conversationId;
  logger35.info(ctx, "subagentStop hook firing", {
    subagentId,
    subagentType,
    toolCallId: requestContext.toolCallId,
    status
  ...
  try {
    ...
    if (result.response?.response.case !== "subagentStop") {
      logger35.warn(ctx, "subagentStop hook returned unexpected response", {
        subagentId,
        subagentType,
        toolCallId: requestContext.toolCallId,
        responseCase: result.response?.response.case,
    ...
    logger35.info(ctx, "subagentStop hook completed", {
      subagentId,
      subagentType,
      toolCallId: requestContext.toolCallId,
      status,
...
async function executeRemotePreToolUseHook(args) {
  ...
  const { conversationId, generationId } = buildRequestIds(ctx, requestContext);
  logger35.info(ctx, "preToolUse hook firing", {
    toolName,
    toolCallId: requestContext.toolCallId
  });
  const startTime = performance.now();
  try {
    ...
    if (result.response?.response.case === "preToolUse") {
      ...
      if (response.updatedInput) {
        try {
          ...
          logger35.warn(ctx, "Failed to parse preToolUse updatedInput", {
            toolName,
            toolCallId: requestContext.toolCallId
          });
        }
      ...
      logger35.info(ctx, "preToolUse hook completed", {
        toolName,
        toolCallId: requestContext.toolCallId,
        permission: response.permission ?? "none",
        durationMs,
    ...
    logger35.info(ctx, "preToolUse hook completed with no response", {
      toolName,
      toolCallId: requestContext.toolCallId,
      durationMs
    });
    ...
    if (hasFailClosed) {
      const detail = error3 instanceof Error ? error3.message : "preToolUse hook error";
      ...
      logger35.warn(ctx, "preToolUse hook failed (fail-closed)", {
        toolName,
        toolCallId: requestContext.toolCallId,
        error: error3 instanceof Error ? error3.message : String(error3),
        durationMs
    ...
    logger35.warn(ctx, "preToolUse hook execution failed (fail-open)", {
      toolName,
      toolCallId: requestContext.toolCallId,
      error: error3 instanceof Error ? error3.message : String(error3),
      durationMs
...
async function executeRemotePostToolUseHook(args) {
  ...
  const { conversationId, generationId } = buildRequestIds(ctx, requestContext);
  logger35.info(ctx, "postToolUse hook firing", {
    toolName,
    toolCallId: requestContext.toolCallId
  });
  const startTime = performance.now();
  ...
  try {
    ...
    logger35.info(ctx, "postToolUse hook completed", {
      toolName,
      toolCallId: requestContext.toolCallId,
      durationMs
    });
    ...
    logger35.warn(ctx, "postToolUse hook execution failed", {
      toolName,
      toolCallId: requestContext.toolCallId,
      error: error3 instanceof Error ? error3.message : String(error3),
      durationMs
...
async function executeRemotePostToolUseFailureHook(args) {
  const { ctx, toolName, toolInput, errorMessage: errorMessage4, failureType, durationMs: toolDurationMs, isInterrupt = false, requestContext, options: options2 } = args;
  const remoteHookExecutor = getRemoteHookExecutor(options2, "postToolUseFailure");
  if (!remoteHookExecutor) {
    return [];
  }
  const { conversationId, generationId } = buildRequestIds(ctx, requestContext);
  logger35.info(ctx, "postToolUseFailure hook firing", {
    toolName,
    toolCallId: requestContext.toolCallId,
    failureType
  });
  ...
  try {
    ...
    logger35.info(ctx, "postToolUseFailure hook completed", {
      toolName,
      toolCallId: requestContext.toolCallId,
      durationMs
    });
    ...
    logger35.warn(ctx, "postToolUseFailure hook execution failed", {
      toolName,
      toolCallId: requestContext.toolCallId,
      error: error3 instanceof Error ? error3.message : String(error3),
      durationMs
...
function withRemoteHooks(args) {
  ...
  return async (ctx, toolArgs) => {
    ...
    try {
      ...
      const errorObj = error3 instanceof Error ? error3 : new Error(String(error3));
      try {
        await executeRemotePostToolUseFailureHook({
          ...
          failureType: isTimeoutError(error3) ? "timeout" : "error",
