...
function maybeNormalizeExecBoundaryError(error3) {
  if (error3 instanceof Error && error3.message.includes("signal is aborted without reason")) {
    const normalized2 = new ToolCallAbortedError();
    setErrorCause(normalized2, error3);
    return normalized2;
  }
  if (isExecBackendUnavailableError(error3)) {
  ...
  if (isPodMigratingExecBoundaryError(error3)) {
    return new RetryableToolEnvironmentOrchestrationError("The execution environment is moving to another machine (reason=pod_migrating); it will be back in a few minutes.", { cause: error3, code: "ENVIRONMENT_MIGRATING" });
  }
  if (isPodNotFoundExecBoundaryError(error3) || isExecDaemonUnreachableExecBoundaryError(error3) || isBridgeTransportClosedExecBoundaryError(error3) || isEnvironmentUnreachableMessageError(error3)) {
    const normalized2 = new RetryableToolEnvironmentOrchestrationError("The execution environment has become unreachable.", { cause: error3 });
    return normalized2;
  }
  ...
  switch (connectErrorCode) {
    ...
    case Code.ResourceExhausted:
      if (shouldUseRateLimitMessage(error3)) {
  ...
  setErrorCause(normalized, error3);
  return normalized;
}
function maybeCreateAgentStreamStartTimeoutTurnError(error3) {
  if (!isAgentStreamStartTimeoutError(error3)) {
    return void 0;
  }
  const cause = error3 instanceof Error ? error3 : void 0;
...
function isPodNotFoundExecBoundaryError(error3) {
  return collectErrorText(error3).some((text2) => text2.toLowerCase().includes("object not found: pod"));
}
function isPodMigratingExecBoundaryError(error3) {
  return collectErrorText(error3).some((text2) => {
    const lower = text2.toLowerCase();
    return lower.includes("reason=pod_migrating") || lower.includes("pod_migrating:");
  });
}
function isExecDaemonUnreachableExecBoundaryError(error3) {
  return collectErrorText(error3).some((text2) => text2.toLowerCase().includes("exec-daemon is unreachable"));
}
function isBridgeTransportClosedExecBoundaryError(error3) {
  return collectErrorText(error3).some((text2) => text2.toLowerCase().includes("bridge transport is closed"));
}
function isEnvironmentUnreachableMessageError(error3) {
  return collectErrorText(error3).some((text2) => text2.toLowerCase().includes("execution environment has become unreachable"));
}
function isExecBackendUnavailableError(error3) {
  if (error3 instanceof Error && (error3.name === "ExecBackendUnavailableError" || error3.name === "ControlledExecDisposedError" || error3.name === AGENT_STREAM_START_TIMEOUT_ERROR_NAME)) {
    return true;
  }
  if (isAgentStreamStartTimeoutError(error3)) {
    return true;
  }
  if (isExtensionHostTimeoutConnectError(error3)) {
    return true;
  }
  if (isLegacyAgentStreamStartTimeoutError(error3)) {
    return true;
  }
  const texts = collectErrorText(error3).map((t) => t.toLowerCase());
...
function isAgentStreamStartTimeoutError(error3) {
  if (error3 instanceof Error && error3.name === AGENT_STREAM_START_TIMEOUT_ERROR_NAME) {
    return true;
  }
  return false;
}
function isAgentStreamStartTimeoutRecoveryTurnError(error3) {
  if (isAgentStreamStartTimeoutError(error3)) {
    return true;
  }
  if (!(error3 instanceof ConnectError) || error3.code !== Code.DeadlineExceeded) {
    return false;
  }
  return error3.findDetails(ErrorDetails).some((detail) => {
    return detail.error === ErrorDetails_Error.EXTENSION_HOST_TIMEOUT && detail.details?.title === AGENT_STREAM_START_TIMEOUT_TITLE;
  });
}
function isExtensionHostTimeoutConnectError(error3) {
  if (!(error3 instanceof ConnectError)) {
    return false;
  }
  for (const detail of error3.findDetails(ErrorDetails)) {
...
function isLegacyAgentStreamStartTimeoutError(error3) {
  return collectErrorText(error3).some((text2) => text2.toLowerCase().includes("agent stream start timeout"));
}
function setErrorCause(target, cause) {
  target.cause = cause;
}
function shouldUseRateLimitMessage(error3) {
  if (error3 instanceof ConnectError && error3.findDetails(ErrorDetails).some((detail) => detail.error === ErrorDetails_Error.RESOURCE_EXHAUSTED)) {
    return true;
  }
  const texts = collectErrorText(error3).map((s3) => s3.toLowerCase());
  if (texts.length === 0) {
    return false;
  }
...
function collectErrorText(error3) {
  const texts = [];
  const queue = [error3];
...
function getConnectErrorCode(error3) {
  if (error3 instanceof Error) {
    const messageMatch = /^\[([a-z_]+)\](?:\s|$)/.exec(error3.message);
    if (messageMatch) {
      return CONNECT_CODE_BY_NAME.get(messageMatch[1]);
    }
    const stackMatch = typeof error3.stack === "string" ? /(?:^|\n)\s*ConnectError:\s*\[([a-z_]+)\](?:\s|$)/.exec(error3.stack) : null;
    if (stackMatch) {
      return CONNECT_CODE_BY_NAME.get(stackMatch[1]);
    }
    const loweredTexts = collectErrorText(error3).map((text2) => text2.toLowerCase());
