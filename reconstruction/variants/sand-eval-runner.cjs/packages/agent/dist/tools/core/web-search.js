...
init_dist3();
init_zod();
var __addDisposableResource50 = function(env, value, async) {
...
var __disposeResources50 = /* @__PURE__ */ (function(SuppressedError2) {
  return function(env) {
    function fail(e) {
...
function getErrorMessage3(error3) {
  ...
  if (typeof error3 === "object" && error3 !== null && "message" in error3 && typeof error3.message === "string") {
    return error3.message;
  }
  return void 0;
}
...
function classifyWebSearchProviderError(error3) {
  const candidateMessages = [];
  const directMessage = getErrorMessage3(error3);
...
var createWebSearchTool = (webSearchService, promptVersion, options2) => {
  ...
  const execute = async (parentCtx, interactionHandler, rawArgs, meta) => {
    ...
    try {
      const spanCtxt = __addDisposableResource50(env_1, createSpan(parentCtx.withName("webSearchExecute")), false);
      const executeCore = async (ctx, args) => {
        ...
        return await interactionHandler.executeToolCall(ctx, createWebSearchToolCall(baseToolCall), meta.toolCallId, async (innerCtx) => {
          ...
          try {
            ...
            throw classifyWebSearchProviderError(error3) ?? error3;
          }
      ...
      if (error3 instanceof ToolCallRejectedError) {
        return createWebSearchToolCall(new WebSearchToolCall({
          result: new WebSearchResult({
            result: {
              case: "rejected",
              value: new WebSearchRejected({ reason: error3.message })
            }
          })
        }));
      }
      const errorMessage4 = error3 instanceof ToolCallError ? error3.clientVisibleErrorMessage : "An error occurred while searching the web";
      return createWebSearchToolCall(new WebSearchToolCall({
        result: new WebSearchResult({
          result: {
            case: "error",
            value: new WebSearchError({ error: errorMessage4 })
          }
        })
      }));
    }
