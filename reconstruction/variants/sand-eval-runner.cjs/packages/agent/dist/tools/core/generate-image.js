...
init_dist3();
init_zod();
var __addDisposableResource45 = function(env, value, async) {
...
var __disposeResources45 = /* @__PURE__ */ (function(SuppressedError2) {
  return function(env) {
    function fail(e) {
...
function isResizeImageError(error3) {
...
function getPermissionLabels(writeResult2) {
  if (writeResult2.result.case !== "permissionDenied") {
    return { operation: "n/a", isReadonly: "n/a" };
  }
  const operation = writeResult2.result.value.operation || "unknown";
  const isReadonly = writeResult2.result.value.isReadonly ? "true" : "false";
...
function getImageMimeTypeFromPath(imagePath) {
  const extension3 = import_node_path53.default.extname(imagePath).toLowerCase();
  switch (extension3) {
    case ".jpg":
    case ".jpeg":
      return "image/jpeg";
...
function isContentSafetyBlockedError(error3) {
  if (error3 instanceof Error) {
    if (error3.name === "ImageGenerationContentSafetyError") {
      return true;
    }
  }
...
function getProviderResponseStatusCode(error3) {
  if (typeof error3 !== "object" || error3 === null) {
    return void 0;
  }
  const maybeError = error3;
...
function getProviderResponseDetails(error3) {
  if (typeof error3 !== "object" || error3 === null) {
    return void 0;
  }
  const maybeError = error3;
...
function getGenerateImageBaseErrorMessage(error3) {
  return error3 instanceof Error ? error3.message : String(error3);
}
function getGenerateImageDetailedErrorMessage(error3) {
  const baseMessage = getGenerateImageBaseErrorMessage(error3);
  const providerDetails = getProviderResponseDetails(error3);
  if (!providerDetails) {
    return baseMessage;
  }
...
function crossPlatformBasename(filePath) {
  return import_node_path53.default.basename(normalizeToUnixPath(filePath));
}
function getImageOutputPath(options2) {
  const { projectFolder, filename, artifactsFolder } = options2;
  if (artifactsFolder) {
    return import_node_path53.default.join(artifactsFolder, "assets", filename);
  }
  return import_node_path53.default.join(projectFolder, "assets", filename);
}
async function writeGeneratedImage(ctx, writeExecutor, options2) {
  ...
  const writeResult2 = await writeExecutor.execute(ctx, new WriteArgs({
    path: outputPath,
    fileBytes: new Uint8Array(imageBytes),
    returnFileContentAfterWrite: false
  }));
  const writeResultLabel = writeResult2.result.case ?? "undefined";
  const permissionLabels = getPermissionLabels(writeResult2);
  ...
  if (writeResult2.result.case !== "success") {
    if (writeResult2.result.case === "permissionDenied" && writeResult2.result.value.isReadonly) {
    ...
    const resultCase = writeResult2.result.case ?? "undefined";
    let errorDetail;
    let toThrow;
    switch (writeResult2.result.case) {
      case "error":
        errorDetail = writeResult2.result.value.error;
        toThrow = new Error(`Failed to save generated image: ${errorDetail}`);
        break;
      case "permissionDenied":
        errorDetail = `permission denied (operation=${writeResult2.result.value.operation}, isReadonly=${writeResult2.result.value.isReadonly}, error=${writeResult2.result.value.error})`;
        toThrow = new ToolCallUnexpectedEnvironmentError(`Failed to save generated image: ${errorDetail}`);
        break;
      case "rejected": {
        const rejectedReason = writeResult2.result.value.reason;
      ...
      case void 0:
        ...
        break;
      default: {
        const _exhaustive = writeResult2.result;
...
var createGenerateImageTool = (resourceAccessor, generateImageService, promptVersion, requestContext, imageGenerationConcurrencyLimiter, isReadonly, suspiciousKeywords = DEFAULT_SUSPICIOUS_KEYWORDS, isModelRestricted) => {
  ...
  const execute = async (parentCtx, interactionHandler, rawArgs, meta) => {
    ...
    try {
      const spanCtxt = __addDisposableResource45(env_1, createSpan(parentCtx.withName("generateImageExecute")), false);
      ...
      try {
        ...
        const isAborted2 = parentCtx.signal.aborted || error3 instanceof ToolCallAbortedError || error3 instanceof Error && error3.name === "AbortError";
        if (isAborted2) {
        ...
        const contentSafetyBlocked = isContentSafetyBlockedError(error3);
        ...
        if (!isAborted2 && providerDetails !== void 0) {
          const baseMessage = getGenerateImageBaseErrorMessage(error3);
          const classification = classifyProviderStatus(getProviderResponseStatusCode(error3));
          throw new CustomToolCallError(classification, {
            clientVisibleErrorMessage: baseMessage,
            modelVisibleErrorMessage: baseMessage,
            error: getGenerateImageDetailedErrorMessage(error3)
          });
        }
        throw error3;
      } finally {
...
Always use this absolute path when referring to the image. Do not repeat this image as a Markdown reference; it is already displayed to the user.`);
          ...
          renderOutcome = isResizeImageError(error3) ? "resize_error" : "internal_error";
          throw error3;
        }
      }
  ...
  return createZodAgentTool("GENERATE_IMAGE", {
    ...
    serializeError: (error3) => {
      const errorMessage4 = error3 instanceof ToolCallError ? error3.clientVisibleErrorMessage : getGenerateImageBaseErrorMessage(error3);
