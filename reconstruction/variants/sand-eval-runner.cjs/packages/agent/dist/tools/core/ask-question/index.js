...
${answerDescriptions?.join("\n") ?? ""}`;
    ...
    case "error": {
      const errorMessage4 = result.result.value.errorMessage;
      if (errorMessage4 === FIRST_ASK_QUESTION_CLIENT_ERROR_MESSAGE) {
        return `Error: ${FIRST_ASK_QUESTION_MODEL_ERROR_MESSAGE}`;
      }
      if (errorMessage4 === PLACEHOLDER_ASK_QUESTION_CLIENT_ERROR_MESSAGE) {
