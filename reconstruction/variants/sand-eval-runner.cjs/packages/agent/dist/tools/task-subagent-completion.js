...
async function handleSubagentExecutionError(error3, state, iterState, completionCtx, deps, subagentCtxCanceled, abortOptions) {
  ...
  const taskErrorLogData = {
    ...
    errorStack: error3 instanceof Error ? error3.stack : void 0,
  ...
  if (shouldBubbleTaskErrorToOuterRetryLayer(ctx, error3, iterState.runStreamCompleted)) {
  ...
  const classifiedTaskError = classifyTaskProviderError(error3);
