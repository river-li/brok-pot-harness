...
function createPromptReferenceIdFromMessageId(messageId) {
  const digest = (0, import_node_crypto19.createHash)("sha256").update(messageId).digest();
  let id = "";
...
function createPromptReferenceId(messageId) {
  ...
  let id = "";
  for (let i = 0; i < PROMPT_REFERENCE_ID_LENGTH; i++) {
    id += PROMPT_REFERENCE_ID_ALPHABET[(0, import_node_crypto19.randomInt)(0, PROMPT_REFERENCE_ID_ALPHABET_LENGTH)];
  }
  return id;
}
