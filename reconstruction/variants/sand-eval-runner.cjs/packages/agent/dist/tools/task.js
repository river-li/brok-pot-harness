 ...
 * Source: ../packages/agent/dist/tools/task.js
...
var __addDisposableResource42 = function(env, value, async) {
...
var __disposeResources42 = /* @__PURE__ */ (function(SuppressedError2) {
  return function(env) {
    function fail(e) {
...
function isWritableIterableClosedError(error3) {
  return error3 instanceof WriteIterableClosedError || error3 instanceof Error && error3.message.includes("WritableIterable is closed");
}
...
async function executeSubagentStartHook(params) {
  ...
  try {
    ...
    const span = __addDisposableResource42(env_1, createSpan(parentCtx.withName("agent.lifecycleHook.subagentStart")), false);
    ...
    __disposeResources42(env_1);
  }
}
async function executeSubagentStopHook(params) {
  const env_2 = { stack: [], error: void 0, hasError: false };
  try {
    const { resourceAccessor, toolCallId, subagentId, subagentType, overriddenModelId, status, durationMs, messageCount, toolCallCount: toolCallCount2, summary, errorMessage: errorMessage4, loopCount, task, description: description9, parentCtx, enableExecuteHookExec, configuredSteps } = params;
    const span = __addDisposableResource42(env_2, createSpan(parentCtx.withName("agent.lifecycleHook.subagentStop")), false);
    const hookCtx = span.ctx;
    ...
    __disposeResources42(env_2);
  }
}
async function extractLastAssistantMessage(ctx, newTurns, blobStore, logContext) {
  const env_3 = { stack: [], error: void 0, hasError: false };
  try {
    const span = __addDisposableResource42(env_3, createSpan(ctx.withName("extractLastAssistantMessage")), false);
    ...
    __disposeResources42(env_3);
  }
}
async function collectConversationStepsFromStepBlobIds({ ctx, stepBlobIds, blobStore, logContext }) {
  const env_4 = { stack: [], error: void 0, hasError: false };
  try {
    const span = __addDisposableResource42(env_4, createSpan(ctx.withName("collectConversationStepsFromStepBlobIds")), false);
    const spanCtx = span.ctx;
    ...
    __disposeResources42(env_4);
  }
}
async function collectConversationTurnStructures({ ctx, turns, blobStore }) {
  const env_5 = { stack: [], error: void 0, hasError: false };
  try {
    const span = __addDisposableResource42(env_5, createSpan(ctx.withName("collectConversationTurnStructures")), false);
    const spanCtx = span.ctx;
    ...
    __disposeResources42(env_5);
  }
}
async function collectConversationStepsWithToolCallCount(ctx, turns, blobStore) {
  const env_6 = { stack: [], error: void 0, hasError: false };
  try {
    const span = __addDisposableResource42(env_6, createSpan(ctx.withName("collectConversationStepsWithToolCallCount")), false);
    const spanCtx = span.ctx;
...
async function getLatestParentCursorCommands(ctx, stateHandler) {
  for (let i = stateHandler.turns.length - 1; i >= 0; i--) {
    ...
    const redactedContext = userMessage.selectedContext;
    if (!redactedContext) {
      return void 0;
    }
...
function buildTaskErrorShapeSnapshot(error3) {
  ...
  const base = {
    typeof: typeof error3,
    isErrorInstance: error3 instanceof Error
  };
  if (error3 instanceof Error) {
    const maybeCode = "code" in error3 && (typeof error3.code === "string" || typeof error3.code === "number") ? error3.code : void 0;
    const maybeCause = "cause" in error3 && error3.cause !== void 0 ? error3.cause : void 0;
    return {
      ...base,
      constructorName: error3.constructor?.name,
      name: error3.name,
      message: error3.message,
      ...maybeCode !== void 0 ? { code: maybeCode } : {},
      ...maybeCause !== void 0 ? { causeTypeof: typeof maybeCause } : {},
      ...error3.stack !== void 0 ? { hasStack: true } : {}
    };
  }
  if (typeof error3 === "object" && error3 !== null) {
    const objectError = error3;
...
function parseProviderErrorFromMessage(errorMessage4) {
  const payloadCandidates = [];
  const knownPrefixes = [PROVIDER_ERROR_PREFIX, NOT_FOUND_ERROR_PREFIX];
  for (const prefix of knownPrefixes) {
    const prefixIndex = errorMessage4.indexOf(prefix);
    if (prefixIndex >= 0) {
      const rawPayload = errorMessage4.slice(prefixIndex + prefix.length).trim();
      if (rawPayload.length > 0) {
        payloadCandidates.push(rawPayload);
      }
    }
  }
  const apiErrorIndex = errorMessage4.indexOf(API_ERROR_PREFIX);
  if (apiErrorIndex >= 0) {
    const apiErrorSection = errorMessage4.slice(apiErrorIndex + API_ERROR_PREFIX.length).trim();
...
function collectTaskProviderErrorCandidateMessages(error3) {
  ...
  pushMessage(getObjectMessage(error3));
  const cause = typeof error3 === "object" && error3 !== null && "cause" in error3 ? error3.cause : void 0;
  pushMessage(getObjectMessage(cause));
  pushMessage(getObjectMessage(getNestedErrorObject(cause)));
  pushMessage(getObjectMessage(getNestedErrorObject(error3)));
  return candidateMessages;
}
function classifyTaskProviderError(error3) {
  const candidateMessages = collectTaskProviderErrorCandidateMessages(error3);
...
function shouldBubbleTaskErrorToOuterRetryLayer(ctx, error3, runStreamCompleted) {
  if (!getShouldBubbleRetryableTaskErrorsFromContext(ctx) || runStreamCompleted) {
    return false;
  }
  const providerError = classifyTaskProviderError(error3);
  if (providerError !== void 0 && providerError.classification === ToolErrorClassification.PROVIDER_ERROR) {
    return true;
  }
  const connectErrorCode = getConnectErrorCode(error3);
...
function maybeAppendInterruptRetryHint(error3, enableSubagentInterrupt, interruptAlreadyRequested) {
  if (!enableSubagentInterrupt || interruptAlreadyRequested || error3 !== RUNNING_SUBAGENT_FOLLOWUP_ERROR) {
    return error3;
  }
  return `${error3}
...
` + fullDescription;
  ...
  const parseTaskToolCallArgs = (rawArgs) => {
    ...
    try {
      ...
      const message = error3 instanceof Error ? error3.message : "Failed to parse arguments";
      throw new ToolCallArgParseError(`Invalid arguments:
argument: ${message}`);
    }
...
${errorMessages.join("\n")}`);
  ...
  const execute = async (parentCtx, interactionHandler, rawArgs, meta) => {
    ...
    try {
      const spanCtxt = __addDisposableResource42(env_7, createSpan(parentCtx.withName("taskExecute")), false);
      ...
      const taskResult = await interactionHandler.executeToolCall(executionCtx, createTaskToolCall(baseToolCall), meta.toolCallId, async (ctx) => {
        ...
        if (canUseClientSideSubagent) {
          ...
          try {
            ...
            const runInBackground2 = updatedRawArgs.run_in_background ?? subagentConfig.isBackground;
            ...
            if (steerSignal !== void 0 && runInBackground2 !== true) {
            ...
            if (result.result.case === "error") {
              ...
              throw new Error(maybeAppendInterruptRetryHint(error3.error ?? "Unknown subagent error", enableSubagentInterrupt, updatedRawArgs.interrupt === true));
            }
            ...
            const lastAgentId = effectiveEnvironment === SubagentExecutionEnvironment.CLOUD ? success.agentId : success.agentId ?? subagentId;
            if (lastAgentId !== void 0 && lastAgentId.length > 0 && (!reattachOnly || stateHandler.restoreSubagentState(ctx, lastAgentId) === void 0)) {
              stateHandler.persistSubagentState(ctx, lastAgentId, subagentConfig.subagent_type, reattachOnly ? new SubagentPersistedState() : effectiveEnvironment === SubagentExecutionEnvironment.CLOUD ? buildCloudSubagentPersistedState({
                bcId: lastAgentId,
                modelId: resolvedModelId,
                transcriptPath: success.transcriptPath,
            ...
            return new TaskResult({
              result: {
                ...
                value: new TaskSuccess({
                  ...
                  agentId: lastAgentId,
                  isBackground,
                  backgroundReason: success.backgroundReason,
                  transcriptPath: success.transcriptPath
                })
              }
            });
            ...
            const errorMessage4 = isStreamClosed ? SUBAGENT_STREAM_CLOSED_ERROR : error3 instanceof Error ? error3.message : String(error3);
            if (isStreamClosed) {
        ...
        try {
          ...
          return await handleSubagentExecutionError(error3, state, iterState, completionCtx, completionDeps, subagentCtx.canceled, registry2.lastAbortOptions);
        } finally {
