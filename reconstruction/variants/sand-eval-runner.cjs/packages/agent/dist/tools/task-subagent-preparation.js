...
async function buildClientSubagentAttachmentsContext(ctx, attachmentPaths, resourceAccessor, toolCallId, options2) {
  ...
  for (const rawFilePath of videoPaths) {
    ...
    selectedVideos.push(new SelectedVideo({
      ...
      path: filePath,
      mimeType,
      filename: import_node_path49.default.basename(filePath),
      fps: 4
    }));
  }
...
function isPathWithinPrefix2(args) {
  const resolvedTarget = import_node_path49.default.resolve(args.targetPath);
  const resolvedPrefix = import_node_path49.default.resolve(args.prefix);
  return resolvedTarget === resolvedPrefix || resolvedTarget.startsWith(resolvedPrefix + import_node_path49.default.sep);
}
...
function resolveTrustedAttachmentPath(args) {
  const posixPath = import_node_path49.default.posix.normalize(args.attachmentPath);
  ...
  const canonicalPath = import_node_path49.default.resolve(args.attachmentPath);
  const env = args.requestContext.env;
  const allowedPrefixes = [
    ...env?.projectFolder ? [import_node_path49.default.join(env.projectFolder, "uploads"), import_node_path49.default.join(env.projectFolder, "attachments")] : [],
    ...env?.workspacePaths.flatMap((workspacePath) => [
      import_node_path49.default.join(workspacePath, "uploads"),
      import_node_path49.default.join(workspacePath, "attachments")
    ]) ?? [],
    env?.artifactsFolder
  ...
  const userAttachedPaths = (args.userAttachedVideoPaths ?? []).map((attachedPath) => import_node_path49.default.resolve(attachedPath));
