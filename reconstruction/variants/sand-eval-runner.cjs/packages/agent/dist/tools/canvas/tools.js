...
function renderWriteResult(result) {
  ...
  if (value.case === "success") {
    ...
    const title = success.title !== void 0 && success.title.length > 0 ? success.title : "Untitled canvas";
    return [
      `Canvas saved: "${title}".`,
      `URL: ${success.url}`,
      `Reference it to the user as a markdown link: [${title}](${success.url}).`
    ].join("\n");
  }
  if (value.case === "failure") {
    const failure = value.value;
    if (isWriteArgValidationDetail(failure.detail)) {
      return [
        `Canvas was not saved: ${failure.detail}`,
        "No url was produced; do not present a canvas link to the user."
      ].join("\n");
    }
    const lines2 = [`Canvas was not saved: ${describeWriteFailReason(failure.reason)}.`];
    if (failure.detail !== void 0 && failure.detail.length > 0) {
      lines2.push(failure.detail);
    }
...
function renderReadResult(result) {
  ...
  if (value.case === "success") {
    const success = value.value;
    const title = success.title !== void 0 && success.title.length > 0 ? success.title : "Untitled canvas";
    return [`Canvas "${title}" (${success.url}):`, "", success.source].join("\n");
  }
  if (value.case === "failure") {
    const failure = value.value;
    if (failure.reason === ReadCanvasFailReason.INVALID_REFERENCE && failure.detail !== void 0 && failure.detail.length > 0) {
      return `Could not read canvas: ${failure.detail}`;
    }
    const detail = failure.detail !== void 0 && failure.detail.length > 0 ? ` ${failure.detail}` : "";
    return `Could not read canvas: ${describeReadFailReason(failure.reason)}.${detail}`;
  }
...
function serializeWriteToolError(error3) {
  if (error3 instanceof ToolCallArgParseError) {
    const detail = isWriteArgValidationDetail(error3.message) ? error3.message : `Invalid arguments:
${error3.message}`;
    return writeFailureResult(WriteCanvasFailReason.REFUSED, detail);
  }
  return writeFailureResult(WriteCanvasFailReason.UNAVAILABLE, errorMessage3(error3));
}
function buildWriteCanvasTool(port) {
  ...
  return createZodAgentTool("WRITE_CANVAS", {
    ...
    render: render2,
    serializeError: (error3) => wrapWriteToolCall(new WriteCanvasToolCall({
      result: serializeWriteToolError(error3)
    }))
  });
}
function buildReadCanvasTool(port) {
  ...
  return createZodAgentTool("READ_CANVAS", {
    ...
    execute: withSafeParsedArgs(readCanvasParametersSchema, async (ctx, interactionHandler, parsedArgs, meta) => {
      ...
      return await interactionHandler.executeToolCall(ctx, wrapReadToolCall(new ReadCanvasToolCall({ args: argsProto })), meta.toolCallId, async () => {
        ...
        try {
          ...
          return readFailureResult(ReadCanvasFailReason.UNAVAILABLE, errorMessage3(error3));
        }
    ...
    render: render2,
    serializeError: (error3) => wrapReadToolCall(new ReadCanvasToolCall({
      result: readFailureResult(ReadCanvasFailReason.UNAVAILABLE, errorMessage3(error3))
    }))
  });
}
