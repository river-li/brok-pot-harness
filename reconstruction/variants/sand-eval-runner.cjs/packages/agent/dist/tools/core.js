...
async function executeToolResultOrError(tool, parentCtx, interactionHandler, argsStream, meta) {
  ...
  try {
    const spanCtxt = __addDisposableResource23(env_1, createSpan(parentCtx.withName("executeToolResultOrError")), false);
    ...
    try {
      ...
      if (error3 instanceof RetryableToolOrchestrationError && shouldBubbleRetryableTaskErrors) {
        throw error3;
      }
      const streamStartTimeoutTurnError = maybeCreateAgentStreamStartTimeoutTurnError(error3);
      ...
      const normalized = maybeRewriteFusedStepGuardAbort(maybeNormalizeExecBoundaryError(error3), interactionAbortSignal !== void 0 ? [interactionAbortSignal, ctx.signal] : [ctx.signal], tool.name);
...
function shouldHideToolCallErrorFromClient(error3) {
  return error3 instanceof CustomToolCallError && "hideFromClientToolCall" in error3 && error3.hideFromClientToolCall === true;
}
function isAbortLikeError(error3) {
  return error3 instanceof ToolCallAbortedError || error3 instanceof Error && error3.name === "AbortError";
}
function maybeRewriteFusedStepGuardAbort(error3, abortSignals, toolName) {
  if (!isAbortLikeError(error3)) {
    return error3;
  }
  const reason = findFusedStepGuardTimeoutReason(abortSignals, error3);
  if (reason === void 0) {
    return error3;
  }
  ...
  timeoutError.cause = error3;
  return timeoutError;
}
function findFusedStepGuardTimeoutReason(abortSignals, error3) {
  for (const signal of abortSignals) {
    if (signal.aborted && isFusedStepGuardTimeoutReason(signal.reason)) {
      return signal.reason;
    }
  }
  if (isFusedStepGuardTimeoutReason(error3)) {
    return error3;
  }
  if (error3 instanceof Error && isFusedStepGuardTimeoutReason(error3.cause)) {
    return error3.cause;
  }
  return void 0;
}
function classifyError(error3) {
  if (error3 instanceof RetryableToolOrchestrationError) {
    return error3.classification ?? ToolErrorClassification.OTHER_ERROR;
  }
  if (error3 instanceof CustomToolCallError) {
    return error3.classification;
  }
  if (error3 instanceof ToolCallArgParseError) {
    return error3.classification ?? ToolErrorClassification.INVALID_ARGS;
  }
  if (isAbortLikeError(error3)) {
    return ToolErrorClassification.ABORTED;
  }
  if (error3 instanceof ToolCallRejectedError) {
    if (error3.message.includes(HOOK_DENIAL_AGENT_NOTE)) {
      return ToolErrorClassification.HOOK_DENIED;
    }
    return ToolErrorClassification.USER_REJECTED;
  }
  if (error3 instanceof ToolCallUnexpectedEnvironmentError) {
    return ToolErrorClassification.UNEXPECTED_ENVIRONMENT;
  }
  if (error3 instanceof ToolTimeoutError || error3 instanceof Error && error3.name === "TimeoutError" || isAxiosTimeoutError(error3)) {
    return ToolErrorClassification.TIMEOUT;
  }
  if (isBadUserDeviceStateError(error3)) {
    return ToolErrorClassification.BAD_USER_DEVICE_STATE;
  }
  if (isUnexpectedEnvironmentErrno(error3)) {
    return ToolErrorClassification.UNEXPECTED_ENVIRONMENT;
  }
  if (error3 instanceof RemoteHookBlockedError) {
    return ToolErrorClassification.HOOK_DENIED;
  }
  if (error3 instanceof Error && error3.message.includes(HOOK_DENIAL_AGENT_NOTE)) {
    return ToolErrorClassification.HOOK_DENIED;
  }
  return ToolErrorClassification.OTHER_ERROR;
}
function isAxiosTimeoutError(error3) {
  ...
  if (code === "ECONNABORTED" && error3.message.toLowerCase().includes("timeout")) {
    return true;
  }
  return false;
}
function isBadUserDeviceStateError(error3) {
  if (!(error3 instanceof Error)) {
    return false;
  }
  const errnoCode = error3.code;
...
function isUnexpectedEnvironmentErrno(error3) {
  if (!(error3 instanceof Error)) {
    return false;
  }
  const errnoCode = error3.code;
