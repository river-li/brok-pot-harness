...
function extractCompleteTagRangesMatching(promptContent, matches) {
  ...
  while (searchStart < promptContent.length) {
    ...
    ranges.push({ start: openIndex, end: sectionEnd, tagName });
...
function isMcpServerTagName(tagName) {
  return MCP_SERVER_TAG_NAMES.has(tagName);
}
...
function extractCompleteTagRanges(promptContent, tagName) {
