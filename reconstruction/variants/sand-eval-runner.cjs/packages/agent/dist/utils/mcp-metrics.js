...
function emitGetMcpToolsMetrics(ctx, input) {
  const { mode, durationMs, success, failureReason, resultCount, responseBytes, payloadBytes, wroteToFile } = input;
...
function emitCallMcpToolMetrics(ctx, input) {
  const { mcpMode, durationMs, success, failureReason, retryable, responseBytes, calledWithoutReadDef } = input;
...
function reportMcpMetaToolFailure(ctx, error3, data) {
  const errorMessage4 = error3 instanceof Error ? error3.message : typeof error3 === "string" ? error3 : String(error3);
  const logData = {
    tool: data.tool,
    ...
    error_message: data.errorMessage ?? errorMessage4
  };
...
function isMcpDirectoryPath2(path30) {
  return path30?.includes(MCP_DIR_SEGMENT) ?? false;
}
function trackMcpDirectoryAccessIfApplicable(ctx, path30, tool) {
  if (isMcpDirectoryPath2(path30)) {
