...
async function executeSmartModeClassifierWithMeasurement(ctx, executor, args, mode = "enforce", workspacePaths, options2) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      const failureReason = classifySmartModeClassifierException(error3);
...
function getSmartModeClassifierSurfaceLabel(args) {
  const json2 = args.target?.arguments?.toJson();
  if (json2 === null || Array.isArray(json2) || typeof json2 !== "object") {
    return void 0;
  }
  const surface = json2.execution_surface;
...
function classifySmartModeClassifierException(error3) {
  if (error3 instanceof Error && (error3.name === "TimeoutError" || error3.message.includes("Smart Mode classifier timed out"))) {
