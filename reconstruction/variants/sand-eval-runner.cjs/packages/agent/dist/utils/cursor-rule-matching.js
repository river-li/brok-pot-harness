...
var normalizePath2 = (value) => {
  const normalized = import_node_path46.posix.normalize(normalizeToUnixPath(value));
  if (normalized === SEP2) {
    return SEP2;
  }
...
function getRuleDir(mdcPath) {
  ...
  const literalRuleDir = import_node_path46.posix.normalize(import_node_path46.posix.dirname(normalizedPath));
...
function getSkillScopeRoot(skillPath) {
  ...
  const segments = import_node_path46.posix.normalize(import_node_path46.posix.dirname(normalizedPath)).split(SEP2);
  for (let i = segments.length - 2; i >= 0; i--) {
  ...
  return import_node_path46.posix.normalize(import_node_path46.posix.dirname(normalizedPath));
}
