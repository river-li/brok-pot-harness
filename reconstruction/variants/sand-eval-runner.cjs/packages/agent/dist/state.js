...
var __addDisposableResource33 = function(env, value, async) {
...
var __disposeResources33 = /* @__PURE__ */ (function(SuppressedError2) {
  return function(env) {
    function fail(e) {
...
function shouldExposeIncomingMessageId(userMessage) {
  ...
  if (userMessage.simulatedMsgReason === SimulatedMsgReason.USER_QUICK_ACTION) {
    return true;
  }
  if (userMessage.simulatedMsgReason !== SimulatedMsgReason.SUBSCRIPTION) {
    return false;
  }
...
function getExplicitUserMessageThreadId(userMessage) {
  return normalizeNonEmptyString2(userMessage.threadId);
}
function resolveUserMessageThreadId(userMessage) {
  return getExplicitUserMessageThreadId(userMessage) ?? normalizeNonEmptyString2(userMessage.promptReferenceId) ?? normalizeNonEmptyString2(userMessage.messageId);
}
function withUserMessageThreadId(userMessage, threadId) {
  return {
    ...userMessage,
    threadId
  };
...
function buildDateStringForTimestampMs(timestampMs, timeZone) {
  const timestamp2 = new Date(Number(timestampMs));
  ...
  const parts = formatter.formatToParts(timestamp2);
  const partLookup = {};
...
function recordFileStateInMap(fileStates, blobStore, path30, content, prevContent, skipReprioritization) {
  const existing = fileStates.get(path30);
  ...
  if (skipReprioritization) {
    fileStates.set(path30, next);
    return;
  }
  const otherEntries = Array.from(fileStates).filter(([key]) => key !== path30);
  fileStates.clear();
  fileStates.set(path30, next);
...
function extractWorkspaceUris(_ctx, requestContext) {
  ...
  for (const workspacePath of workspacePaths) {
    ...
    try {
      uris.push((0, import_node_url5.pathToFileURL)(trimmedPath).toString());
    } catch {
    }
  }
...
var AgentConversationTurnHandle = class extends Writeable {
  ...
  completeOpenMessageStep(completedAtMs) {
    const open3 = this.openMessageStep;
    if (open3 === void 0) {
      return;
    }
    const message = open3.step.message;
    switch (message.case) {
      case "assistantMessage":
      case "thinkingMessage": {
        if (message.value.completedAtMs === void 0) {
          message.value.completedAtMs = completedAtMs;
          open3.reference.set(open3.step);
          this.markDirty();
        }
        break;
      }
...
function ensureUserMessageTiming(message) {
  const timestampMs = message.startedAtMs !== void 0 && message.startedAtMs > BigInt(0) ? message.startedAtMs : message.completedAtMs !== void 0 && message.completedAtMs > BigInt(0) ? message.completedAtMs : BigInt(Date.now());
  message.startedAtMs = timestampMs;
  message.completedAtMs = timestampMs;
  return timestampMs;
}
function computeSubagentTracking(subagentStates) {
  ...
  for (const [subagentId, state] of subagentStates) {
    const timestamp2 = state.lastUsedTimestampMs;
    ...
    if (subagentType) {
      ...
      if (!existing || timestamp2 > existing.timestamp) {
        typeMaxTimestamps.set(typeName, { id: subagentId, timestamp: timestamp2 });
      }
    }
  }
...
var ConversationStateHandle = class _ConversationStateHandle {
  ...
  static async fromConversationStateStructure(ctx, blobStore, conversationStateStructure, rootPromptBuilder, formattingOptions, modelId, runtimeAgentType, options2) {
    ...
    try {
      const span = __addDisposableResource33(env_1, createSpan(ctx.withName("fromConversationStateStructure")), false);
      ...
      if (loadRootPromptBlobs) {
        ...
        const rootPromptMessageLoads = await asyncMapValues(conversationStateStructure.rootPromptMessagesJson, async (blobId) => {
          ...
          try {
            ...
            return { kind: "failed", error: error3 };
          }
      ...
      for (const [pathString, fileStateStructure] of fileStateEntries) {
        const path30 = pathString.unwrap(PrivacyCapability.UNSAFE_ALWAYS_ALLOWED);
      ...
      for (const path30 of conversationStateStructure.readPaths) {
        state.recordReadPath(path30);
      }
  ...
  recordFileState(path30, content, prevContent, skipReprioritization) {
    recordFileStateInMap(this.fileStates, this.blobStore, path30, content, prevContent, skipReprioritization);
  }
  async getFileState(ctx, path30) {
    const contentRef = this.fileStates.get(path30)?.content;
    if (contentRef === void 0) {
      return void 0;
    }
    return contentRef.get(ctx);
  }
  async hydrateUserMessageBlobText(ctx, userMessage) {
    const [textBytes, richTextBytes] = await Promise.all([
      userMessage.text.length === 0 && userMessage.textBlobId !== void 0 && userMessage.textBlobId.length > 0 ? this.blobStore.getBlob(ctx, userMessage.textBlobId) : Promise.resolve(void 0),
      (userMessage.richText === void 0 || userMessage.richText.length === 0) && userMessage.richTextBlobId !== void 0 && userMessage.richTextBlobId.length > 0 ? this.blobStore.getBlob(ctx, userMessage.richTextBlobId) : Promise.resolve(void 0)
    ]);
    if (textBytes !== void 0) {
      userMessage.text = textDecoder2.decode(textBytes);
    } else if (userMessage.text.length === 0 && userMessage.textBlobId !== void 0 && userMessage.textBlobId.length > 0) {
    ...
    } else if ((userMessage.richText === void 0 || userMessage.richText.length === 0) && userMessage.richTextBlobId !== void 0 && userMessage.richTextBlobId.length > 0) {
  ...
  async assignPromptReferenceMatchesToThread(ctx, matches, threadId) {
    for (const match2 of matches) {
      ...
      const userMessage = await turn.userMessage.get(ctx);
      if (getExplicitUserMessageThreadId(userMessage) === threadId) {
        continue;
      }
   ...
   */
  recordReadPath(path30) {
    this.readPaths.add(path30.unwrap(PrivacyCapability.UNSAFE_ALWAYS_ALLOWED));
  }
  /**
   ...
   */
  hasReadPath(path30) {
    return this.readPaths.has(path30.unwrap(PrivacyCapability.UNSAFE_ALWAYS_ALLOWED));
  }
  /**
  ...
  async doComputeNewStructure(parentCtx) {
    ...
    try {
      const span = __addDisposableResource33(env_2, createSpan(parentCtx.withName("computeNewStructure")), false);
      const ctx = span.ctx;
      const quietCtx = withSuppressedChildSpans(ctx);
      const serializeStart = performance.now();
      let setBlobCount = 0;
      const serializeRootPromptMessages = async (messages) => {
      ...
      const fileStatesEntriesPromise = Promise.all(Array.from(this.fileStates.entries()).map(async ([path30, fileState]) => {
        ...
        return [path30, fileStateStructure];
      }));
  ...
  resolveTurnMode(userMessage) {
    const mode = resolveCurrentTurnMode(this.mode, userMessage.mode);
  ...
  resolveStepMode(userMessage) {
    const mode = resolveCurrentStepMode(this.mode, userMessage.mode);
    return __classPrivateFieldGet3(this, _ConversationStateHandle_instances, "m", _ConversationStateHandle_suppressesMultitask).call(this, mode) ? AgentMode.AGENT : mode;
  }
  ...
  async createAgentTurn(parentCtx, userMessage, requestContext, config2, resourceAccessor, options2) {
    const { turn } = await __classPrivateFieldGet3(this, _ConversationStateHandle_instances, "m", _ConversationStateHandle_createAgentTurn).call(this, parentCtx, userMessage, requestContext, config2, resourceAccessor, options2, false);
    return turn;
  }
  /**
   ...
   */
  async createBackgroundTaskCompletionTurn(parentCtx, userMessage, requestContext, config2, resourceAccessor, options2) {
    return await __classPrivateFieldGet3(this, _ConversationStateHandle_instances, "m", _ConversationStateHandle_createAgentTurn).call(this, parentCtx, userMessage, requestContext, config2, resourceAccessor, options2, true);
  }
};
...
}, _ConversationStateHandle_createAgentTurn = async function _ConversationStateHandle_createAgentTurn2(parentCtx, userMessage, requestContext, config2, resourceAccessor, options2, callerAppendsNotificationPrompt) {
  const env_3 = { stack: [], error: void 0, hasError: false };
  try {
    const span = __addDisposableResource33(env_3, createSpan(parentCtx.withName("createAgentTurn")), false);
    const ctx = span.ctx;
    ...
    const isRootProjectMessage = userMessage.projectDetails !== void 0 && projectSubagentDetails === void 0 && projectSideChatDetails === void 0;
    ...
    const projectName = isProjectKickoff ? normalizeProjectName(unwrapPossiblyRedactedCodeString(userMessage.projectDetails?.name)) : void 0;
    const projectChildName = projectSubagentDetails !== void 0 || projectSideChatDetails !== void 0 ? normalizeProjectName(unwrapPossiblyRedactedCodeString(userMessage.projectDetails?.name)) : void 0;
    ...
    if (__classPrivateFieldGet3(this, _ConversationStateHandle_instances, "m", _ConversationStateHandle_suppressesMultitask).call(this, userMessage.mode)) {
      userMessage.mode = AgentMode.AGENT;
    }
    if (stateBeforeMessageBlobId !== void 0) {
      userMessage.conversationStateBlobId = new Uint8Array(stateBeforeMessageBlobId);
    }
    if (!overlapPreTurnStateSnapshot && userMessage.projectDetails !== void 0) {
    ...
    const modeForContextProcessing = this.resolveTurnMode(userMessage);
    const { userContent, selectedImages, selectedVideos, selectedDocuments, imageFilePaths } = await processSelectedContext(ctx, userMessage.selectedContext ?? new SelectedContext(), this.blobStore, config2, requestContext, resourceAccessor, modeForContextProcessing, this.modelId, userMessage.text, userMessage.simulatedMsgReason, this.privacyMode);
    if (overlapPreTurnStateSnapshot) {
      ...
      if (userMessage.projectDetails !== void 0) {
        const nameToKeep = projectName ?? projectChildName;
        if (nameToKeep !== void 0) {
          userMessage.projectDetails.name = nameToKeep;
        } else {
    ...
    const customModeIntent = userMessage.customModeIntent?.intent;
    if (customModeIntent?.case === "exit") {
      this.durableSkillBlocks = [];
      this.durableCustomModeId = void 0;
    } else if (customModeIntent?.case === "enter") {
      const { selectedSkills } = resolveSelectedContextSkillSections(userMessage.selectedContext ?? new SelectedContext());
...
${imageFilePaths.map((path30, i) => `${i + 1}. ${path30}`).join("\n")}
...
${branchReminder}` : branchReminder
    ...
    const currentModeForReminder = this.resolveTurnMode(userMessage);
...
${userTurnSystemReminder}` : userTurnSystemReminder
    ...
    const subagentReminder = userMessage.subagentSystemReminder !== void 0 && userMessage.subagentSystemReminder !== "" ? `<system_reminder>
...
${recentlyAddedPluginReminder}` : recentlyAddedPluginReminder
    ...
    if (enableHookAdditionalContext || enableAgentStoreConflictNotices) {
      const contexts = enableHookAdditionalContext ? userMessage.hookAdditionalContexts : userMessage.hookAdditionalContexts.filter((c) => c.hookEventName === "agentStoreConflict");
...
` : "";
    ...
    const promptReferenceIdResolution = resolvePromptReferenceId(config2.featureFlags?.glassMetaParentAgent === true, userMessage.promptReferenceId, userMessage.messageId, userContent);
    ...
    const incomingMessageIdTag = sendMessageEnabled && shouldExposeIncomingMessageId(userMessage) ? renderIncomingMessageIdTag(userMessage.messageId) : void 0;
...
${eagerEditingNote}${userMessage.text}
    ...
    const promptReferenceId = !promptReferenceIdResolution.hasStructuredPromptReferenceId && promptReferenceIdResolution.promptReferenceId !== void 0 ? promptReferenceIdResolution.promptReferenceId : userMessage.promptReferenceId;
    ...
    const isProjectSendMessageRequestBoundary = sendMessageEnabled && userMessage.isSimulatedMsg !== true;
    const message = {
      role: "user",
      content: userContent,
      ...(requestId !== void 0 || isProjectSendMessageRequestBoundary) && {
        providerOptions: {
          cursor: {
    ...
    const redactedUserMessage = toRedactedUserMessage2(userMessage, this.privacyMode);
    ...
    const inner = new AgentConversationTurnStructure({
      ...
      ...userMessage.messageId.trim().length > 0 ? { userMessageId: userMessage.messageId.trim() } : {}
    });
    ...
    this.olderAgentTurnCount += appendRecentUserMessageId(this.recentUserMessageIds, userMessage.messageId, config2.recentUserMessageIdIndexLimit ?? RECENT_USER_MESSAGE_ID_INDEX_LIMIT);
    return { turn, systemReminder: callerOffloadReminder };
  } catch (e_3) {
    env_3.error = e_3;
    env_3.hasError = true;
  } finally {
    __disposeResources33(env_3);
  }
};
