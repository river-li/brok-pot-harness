 ...
 * Source: ../packages/agent/dist/interaction-handler.js
...
var __disposeResources25 = /* @__PURE__ */ (function(SuppressedError2) {
  return function(env) {
    function fail(e) {
...
var logger37 = createLogger("interaction-handler");
...
var InteractionHandler = class {
  ...
  isConsumerSideError(error3) {
    if (!(error3 instanceof Error)) {
      return false;
    }
    if (error3.name === "CacheRuntimeError" || error3.message.includes("Cache operation") || error3.message.includes("Redis") || error3.message.includes("xadd")) {
      return true;
    }
  ...
  async consumeStream(ctx, fullStream, textHandler) {
    ...
    const sealCurrentThinking = async () => {
      ...
      void this.onThinkingCompleted?.(ctx, {
        ...
        logger37.warn(ctx, "afterAgentThought hook callback failed", {
    ...
    try {
      ...
      if (error3 instanceof AgentLoopError && error3.loopType === "singleMessage") {
        throw error3;
      }
      if (error3 instanceof InputTokenLimitError) {
        logger37.warn(ctx, "InputTokenLimitError will trigger blocking summarization", { error: error3 });
        return;
      }
      ...
      if (isStreamIntentionalAbort) {
        logger37.info(ctx, "consumeStream aborted (intentional cancellation)", {
          error: error3,
          intentionalAbort: true,
          abortReason: streamAbortReason?.reason
        });
      } else {
        logger37.error(ctx, "Error in consumeStream", error3);
      }
      if (this.isConsumerSideError(error3)) {
        throw error3;
      }
    } finally {
      await sealCurrentThinking();
  ...
  async executeToolCall(parentCtx, toolCall, callId, promiseFn, resultMergeFn, hookContextCollector) {
    ...
    try {
      const spanCtxt = __addDisposableResource25(env_1, createSpan(parentCtx.withName("executeToolCall")), false);
      ...
      const startSendStartMs = Date.now();
      {
        ...
        try {
          const startSendSpan = __addDisposableResource25(env_2, createSpan(ctx.withName("executeToolCall.sendStartUpdate")), false);
          const toolCallWithId = this.withToolCallMetadata(callId, toolCall, {
          ...
          __disposeResources25(env_2);
        }
      }
      const startSendMs = Date.now() - startSendStartMs;
      if (startSendMs > 1e3) {
        logger37.warn(ctx, "nal.await_stall.start_send_slow", {
          callId,
      ...
      const completionSendStartMs = Date.now();
      {
        ...
        try {
          const completionSendSpan = __addDisposableResource25(env_3, createSpan(ctx.withName("executeToolCall.sendCompletionUpdate")), false);
          ...
          __disposeResources25(env_3);
        }
      }
      const completionSendMs = Date.now() - completionSendStartMs;
      if (completionSendMs > 1e3) {
        logger37.warn(ctx, "nal.await_stall.completion_send_slow", {
          callId,
          toolName,
          toolExecutionMs,
          completionSendMs
