...
async function requestPromptSuggestion(parentCtx, invocationId, model, executor, interactionListener, tools, messagesSnapshot, priceOptions) {
  ...
  try {
    const spanCtxt = __addDisposableResource34(env_1, createSpan(parentCtx.withName("requestPromptSuggestion")), false);
    ...
    try {
      ...
      const settledResponse = streamResult.response.then(() => ({ didReject: false }), (error3) => ({ didReject: true, error: error3 }));
      let responseText = "";
      ...
      logger57.error(ctx, "Error during prompt suggestion request", error3);
    } finally {
      executor.clearMessages();
