...
async function collectCurrentIntentComputerToolCalls(ctx, stateHandler, userMessageFilter) {
  ...
  for (let turnIndex = stateHandler.turns.length - 1; turnIndex >= 0; turnIndex--) {
    ...
    const text2 = getUserMessageContextText(userMessage, {
...
function extractCurrentTurnInvokedSkillPathsFromRootPrompt(stateHandler) {
  ...
  for (let index = rootPromptMessages.length - 1; index >= 0; index--) {
    ...
    for (const text2 of getCoreMessageTextParts(message)) {
      for (const path30 of extractPathsFromManuallyAttachedSkillsText(text2)) {
        paths.add(path30);
      }
...
function getUserMessageContextText(userMessage, options2) {
  const plainUserMessage = fromRedactedUserMessage2(userMessage, PrivacyCapability.UNSAFE_ALWAYS_ALLOWED);
...
function applySmartModeClassifierMessageCountLimits(messages, limits) {
  ...
  const computerMessages = messages.filter((message) => message.role === "computer").slice(-SAND_AUTO_REVIEW_CLASSIFIER_MAX_COMPUTER_MESSAGES);
...
function truncateSandAutoReviewClassifierContext(messages, options2 = {}) {
  ...
  return applySmartModeClassifierMessageCountLimits(messages, limits).map((message) => ({
    ...message,
    content: truncateSandAutoReviewClassifierMessageContent(message.content, maxCharsPerMessage)
  })).filter((message) => message.content.length > 0);
}
function truncateSmartModeClassifierContext(messages, options2 = {
  ...
  let assistantMessages = messages.filter((message) => message.role === "assistant").slice(-options2.maxAssistantMessages);
  let userMessages = messages.filter((message) => message.role === "user").slice(-options2.maxUserMessages);
...
async function extractSmartModeClassifierConversationContext(ctx, stateHandler, limits = IDE_SMART_MODE_CLASSIFIER_CONTEXT_LIMITS, truncateMessages = (messages) => truncateSmartModeClassifierContext(messages), options2) {
  ...
  return truncateMessages(messages).map((message) => new SmartModeClassifierConversationMessage({
    role: message.role,
