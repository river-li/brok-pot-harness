...
function getSafeErrorType(error3) {
  return error3 instanceof Error ? error3.name || "Error" : typeof error3;
}
...
function isPathWithinPrefix({ targetPath, prefix }) {
  const resolvedTarget = import_node_path45.default.resolve(targetPath);
  const resolvedPrefix = import_node_path45.default.resolve(prefix);
  return resolvedTarget === resolvedPrefix || resolvedTarget.startsWith(resolvedPrefix + import_node_path45.default.sep);
}
...
function isAttachmentStoragePath(targetPath, projectFolder) {
  ...
  const relativePath = import_node_path45.default.relative(import_node_path45.default.resolve(projectFolder), import_node_path45.default.resolve(targetPath));
  const segments = relativePath.split(import_node_path45.default.sep).filter(Boolean);
...
function getTrustedSelectedDocumentPath(documentPath, requestContext) {
  const canonicalPath = import_node_path45.default.resolve(documentPath);
  const env = requestContext?.env;
  const allowedPrefixes = [
    env?.projectFolder ? import_node_path45.default.join(env.projectFolder, "uploads") : void 0,
    ...env?.workspacePaths.map((workspacePath) => import_node_path45.default.join(workspacePath, "uploads")) ?? [],
    env?.artifactsFolder
...
async function resolveTrustedPathOnlyAttachmentPath(args) {
  ...
  const hasPathOnlyPayload = readablePath.length > 0 && (treatNonAbsoluteAsPathOnly || import_node_path45.default.isAbsolute(readablePath));
...
async function resolveTrustedPathOnlyAttachmentCandidate(args) {
  if (!import_node_path45.default.isAbsolute(args.readablePath)) {
    return void 0;
  }
  if (args.attachmentKind === "video") {
    return import_node_path45.default.resolve(args.readablePath);
  }
...
function appendRandomUploadSuffix(filename, fallbackFilename) {
  ...
  const suffix = (0, import_node_crypto18.randomUUID)().replace(/[^a-zA-Z0-9]/g, "").slice(0, 4) || "file";
  const extension3 = import_node_path45.default.extname(sanitizedFilename);
...
function normalizeCustomModeSkillPath(skillPath) {
  ...
  if (trimmed.startsWith("file:")) {
    try {
      comparable = (0, import_node_url4.fileURLToPath)(trimmed);
    } catch {
      comparable = trimmed;
    }
  }
...
function customModeSkillPathLooksLike(fullPath, identity) {
  const path30 = normalizeCustomModeSkillPath(fullPath);
  const needle = normalizeCustomModeSkillPath(identity);
  if (path30.length === 0 || needle.length === 0) {
    return false;
  }
  if (path30 === needle) {
    return true;
  ...
  if (needle.includes("/")) {
    ...
    return path30.endsWith(suffix);
  }
  return customModeSkillFolderName(path30) === needle;
}
...
${pr2.ciFailures}` : ""}
      ...
      if (recentlyViewedFiles.length > 0) {
        ...
        for (const file of reversedFiles) {
          ...
          if (file.activeCommand && config2.enableTerminalFiles) {
...
`);
    ...
    if (imgData) {
      if (config2.enableImageFiles && resourceAccessor && requestContext?.env !== void 0 && requestContext.env.projectFolder !== void 0 && requestContext.env.projectFolder !== "") {
        try {
          ...
          const extensionFromPath = originalPath !== void 0 ? import_node_path45.default.extname(originalPath).replace(/^\./, "").toLowerCase() : void 0;
          let extension3 = "png";
          ...
          const originalFilename = originalPath !== void 0 ? import_node_path45.default.basename(originalPath) : void 0;
          const baseFilename = originalFilename !== void 0 && originalFilename !== "" ? import_node_path45.default.basename(originalFilename, import_node_path45.default.extname(originalFilename)) : selectedImg.uuid || `image-${Date.now()}-${i}`;
          ...
          if (workspaceRoot !== void 0 && originalPath !== void 0 && originalPath !== "") {
            const normalizedOriginalPath = import_node_path45.default.normalize(originalPath);
            const normalizedWorkspaceRoot = import_node_path45.default.normalize(workspaceRoot);
            const relativePath = import_node_path45.default.relative(normalizedWorkspaceRoot, normalizedOriginalPath);
            if (!relativePath.startsWith("..") && !import_node_path45.default.isAbsolute(relativePath)) {
              imageFilePath = normalizedOriginalPath;
            }
          }
          if (imageFilePath === void 0) {
            const projectFolder = requestContext.env.projectFolder;
            const assetsDir = import_node_path45.default.join(projectFolder, "assets");
            const filePath = import_node_path45.default.join(assetsDir, fileName);
  ...
  const videoProcessingPromise = Promise.all(selectedContext.selectedVideos.map(async (selectedVideo, i) => {
    ...
    if (pathOnlyVideo.hasPathOnlyPayload) {
      ...
      return {
        ...
        filename: displayFilename || import_node_path45.default.basename(trustedPathOnlyVideoPath)
      };
    }
    ...
    let videoFilePath;
    const filename = selectedVideo.filename || (selectedVideo.path.length > 0 ? import_node_path45.default.basename(selectedVideo.path) : `video-${Date.now()}-${i}`);
    if (materializeToFilesystem && videoData) {
      ...
      if (resourceAccessor && videoRootPath !== void 0) {
        try {
          ...
          const uploadsDir = import_node_path45.default.join(videoRootPath, "uploads");
          const filePath = import_node_path45.default.join(uploadsDir, safeFilename);
          const writeExecutor = resourceAccessor.get(writeExecutorResource);
          ...
          logger54.warn(ctx, "Failed to write video to filesystem", {
            errorType: getSafeErrorType(error3)
          });
          processedSelectedVideo = void 0;
        }
      }
  ...
  const documentProcessingPromise = Promise.all(selectedContext.selectedDocuments.map(async (selectedDoc, i) => {
    ...
    const readablePath = selectedDocPath || (import_node_path45.default.isAbsolute(trimmedFilename) ? trimmedFilename : "");
    const displayFilename = trimmedFilename && !import_node_path45.default.isAbsolute(trimmedFilename) ? trimmedFilename : import_node_path45.default.basename(readablePath);
    ...
    if (docData) {
      ...
      if (resourceAccessor && documentRootPath !== void 0) {
        try {
          ...
          const uploadsDir = import_node_path45.default.join(documentRootPath, "uploads");
          const filePath = import_node_path45.default.join(uploadsDir, safeFilename);
          const writeExecutor = resourceAccessor.get(writeExecutorResource);
          await writeExecutor.execute(ctx, new WriteArgs({
            path: filePath,
  ...
  const writeCodeSelectionToFile = async (opts) => {
    ...
    try {
      const originalBasename = import_node_path45.default.basename(originalPath) || "selection";
      const originalExt = import_node_path45.default.extname(originalBasename);
  ...
  const writeExternalLinkContentToFile = async (opts) => {
    ...
    try {
      ...
      try {
        ...
        basename11 = lastSegment ? decodeURIComponent(lastSegment) : urlObj.hostname;
      } catch {
        basename11 = "link";
      }
      const knownTextExtensions = /\.(md|txt|html|htm|json|xml|csv|tsv|yaml|yml|rst|tex|log)$/i;
      if (!knownTextExtensions.test(basename11)) {
        basename11 = `${basename11}.md`;
      }
      const sanitized = sanitizeFilename(basename11) || "link.md";
