...
function enrichPendingToolCallJson(pendingMessage, options2) {
  try {
    const parsed = JSON.parse(pendingMessage);
    const contracts = {
      ...parsed.providerOptions?.cursor?.pendingToolExecutionContracts ?? {}
    };
    ...
    const existingStartedAtMs = parsed.providerOptions?.cursor?.pendingToolCallStartedAtMs;
