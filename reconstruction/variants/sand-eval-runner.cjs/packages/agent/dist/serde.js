...
function parseWithUint8Arrays(json2) {
  const parsed = JSON.parse(json2);
  if (json2.includes(UINT8ARRAY_MARKER)) {
    return reviveUint8ArraysInPlace(parsed);
  }
  return parsed;
}
function parseCoreMessageJson(json2) {
  if (fastCoreMessageParseEnabled) {
    return parseWithUint8Arrays(json2);
  }
  return JSON.parse(json2, jsonReviver2);
}
var CoreMessageSerde2 = class {
  serialize(value) {
    const json2 = JSON.stringify(value, jsonReplacer2);
    return utf8Serde.serialize(json2);
  }
  deserialize(blob) {
    const json2 = utf8Serde.deserialize(blob);
    return parseCoreMessageJson(json2);
  }
  getBlobType() {
    return { kind: "json" };
  }
...
var RedactedCoreMessageSerde = class {
  ...
  serialize(value) {
    ...
    const json2 = JSON.stringify(plainMessage, jsonReplacer2);
    return utf8Serde.serialize(json2);
  }
  deserialize(blob) {
    const json2 = utf8Serde.deserialize(blob);
    const plainMessage = parseCoreMessageJson(json2);
