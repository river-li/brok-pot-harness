...
var logger32 = createLogger("@anysphere/agent");
function findLastUserMessageIndex2(messages, options2) {
...
function shouldPerformSelfSummary(messages, tokenDetails, ctx, configTokenLimit) {
  const tokenCount = tokenDetails?.usedTokens !== void 0 && Number.isFinite(tokenDetails.usedTokens) && tokenDetails.usedTokens > 0 ? tokenDetails.usedTokens : estimateTokenCount2(messages, { includeNonTextContent: true });
  ...
  if (evalOverrideLimit !== void 0 && ctx !== void 0) {
    logger32.info(ctx, "[self-summary] using eval override token limit", {
      evalOverrideLimit,
      configTokenLimit
    });
  }
