...
function getBuiltinExtensibilitySkillRoot(userHomeDirectory) {
  return {
    dirPath: (0, import_node_path24.join)(userHomeDirectory, BUILTIN_SKILL_CONFIG_DIR.configDir, BUILTIN_SKILL_CONFIG_DIR.subdir),
    scope: "builtin",
    source: "builtin"
  };
}
...
function getProjectExtensibilitySkillRoots(projectRoot, thirdPartyExtensibilityEnabled) {
  return getDiscoverableSkillConfigDirs(thirdPartyExtensibilityEnabled).map((dir) => ({
    dirPath: (0, import_node_path24.join)(projectRoot, dir.configDir, dir.subdir),
    scope: "project",
    source: "workspace"
  }));
}
function getUserExtensibilitySkillRoots(userHomeDirectory, thirdPartyExtensibilityEnabled) {
  return getDiscoverableSkillConfigDirs(thirdPartyExtensibilityEnabled).map((dir) => ({
    dirPath: (0, import_node_path24.join)(userHomeDirectory, dir.configDir, dir.subdir),
    scope: "user",
    source: "user"
  }));
}
