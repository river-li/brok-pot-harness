...
var MacComputerUseActionRunner = class _MacComputerUseActionRunner {
    ...
    try {
      ...
      for (const action of this.actions) {
        ...
        switch (a.case) {
          ...
          case "drag": {
            const path30 = a.value.path.map((point) => ({
              x: point.x,
              y: point.y
            }));
            await this.ensureGeometryCached();
      ...
      return this.errorResult(error3);
    } finally {
      if (needsControl) {
  ...
  async errorResult(error3) {
    ...
    const guidance = userFacingMacComputerUseError(error3);
    return new ComputerUseResult({
      result: {
        case: "error",
  ...
  async normalizeScreenshot(image2, frame) {
    ...
    try {
      ...
      const reason = redactMacComputerUseError(error3);
