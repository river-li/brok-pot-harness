...
var createCurrentModuleRequire = () => {
  ...
  try {
    ...
    return getCreateRequire()(typeof currentModuleUrl === "string" && currentModuleUrl.length > 0 ? currentModuleUrl : (0, import_node_path25.join)(process.cwd(), "package.json"));
  } finally {
...
function fileNameToPath(fileName) {
  return fileName.startsWith("file:") ? (0, import_node_url3.fileURLToPath)(fileName) : fileName;
}
function loadRuntimeImporter() {
  ...
  for (const moduleFile of stackFileNames()) {
    const dir = (0, import_node_path25.dirname)(fileNameToPath(moduleFile));
    add2((0, import_node_path25.join)(dir, MAC_WEBP_RUNTIME_HELPER));
    add2((0, import_node_path25.join)(dir, "..", "..", "src", "computer-use", MAC_WEBP_RUNTIME_HELPER));
  }
  if (typeof process.argv[1] === "string" && process.argv[1].length > 0) {
    add2((0, import_node_path25.join)((0, import_node_path25.dirname)(process.argv[1]), MAC_WEBP_RUNTIME_HELPER));
  }
  add2((0, import_node_path25.join)(process.cwd(), "src", "computer-use", MAC_WEBP_RUNTIME_HELPER));
  add2((0, import_node_path25.join)(process.cwd(), "computer-use", MAC_WEBP_RUNTIME_HELPER));
  try {
    const packageRoot = (0, import_node_path25.dirname)(getNodeRequire().resolve("@anysphere/local-exec/package.json"));
    add2((0, import_node_path25.join)(packageRoot, "src", "computer-use", MAC_WEBP_RUNTIME_HELPER));
    add2((0, import_node_path25.join)(packageRoot, "dist", "computer-use", MAC_WEBP_RUNTIME_HELPER));
  } catch {
  }
...
async function importJsquash(...parts) {
  return await importSpecifier((0, import_node_url3.pathToFileURL)(resolveJsquashFile(...parts)).href);
}
...
var loadDecoder = memoizedAsync(async () => {
  ...
  await mod.init({
    wasmBinary: await import_node_fs14.promises.readFile(resolveJsquashFile("codec", "dec", "webp_dec.wasm"))
  });
  return mod;
});
var loadEncoder = memoizedAsync(async () => {
  ...
  const module2 = await utils2.initEmscriptenModule(enc.default, void 0, {
    wasmBinary: await import_node_fs14.promises.readFile(resolveJsquashFile("codec", "enc", "webp_enc.wasm"))
  });
