...
function userFacingMacComputerUseError(error3) {
  if (error3 instanceof MacComputerUseRPCError && error3.userFacing && error3.message.trim().length > 0 && !macComputerUseErrorContainsPath(error3.message)) {
    return error3.message.trim();
  }
  return void 0;
}
function isMacComputerUseCaptureBlocked(error3) {
  if (!(error3 instanceof MacComputerUseRPCError)) {
    return false;
  }
  return error3.tool === "computer_use_screenshot" || error3.code === "permission_required";
}
function redactMacComputerUseError(error3) {
  if (typeof error3 === "object" && error3 !== null && "code" in error3 && typeof error3.code === "string") {
    return error3.code;
  }
  if (error3 instanceof Error) {
    if (macComputerUseErrorContainsPath(error3.message)) {
      return error3.name;
    }
    return error3.message.length > 0 ? `${error3.name}: ${error3.message}` : error3.name;
  }
  return "unknown";
}
...
var MacComputerUseRPCClient = class _MacComputerUseRPCClient {
   ...
   */
  static isInstalled() {
    if ((0, import_node_fs13.existsSync)(_MacComputerUseRPCClient.serviceExecutablePath())) {
      return true;
    }
    return (0, import_node_fs13.existsSync)(_MacComputerUseRPCClient.serviceStatePath());
  }
  screenshot(ctx) {
  ...
  async releaseControl(ctx) {
    ...
    try {
      ...
      const response = await _MacComputerUseRPCClient.sendOnce(_MacComputerUseRPCClient.socketPathFrom(state), {
        ...
        requestID: (0, import_node_crypto7.randomUUID)(),
        mode: this.mode,
        sessionId
      ...
      _MacComputerUseRPCClient.logger.warn(ctx, "local-cua control release failed", {
        error: redactMacComputerUseError(error3)
      });
    }
  }
  ...
  async sendWithRelaunchRetry(ctx, buildRequest, label) {
    ...
    try {
      ...
      if (!_MacComputerUseRPCClient.isPreDeliveryConnectionError(error3)) {
        throw error3;
      }
      _MacComputerUseRPCClient.logger.warn(ctx, "local-cua callTool failed; relaunching and retrying", {
        tool: label,
        error: _MacComputerUseRPCClient.spawnErrorCode(error3)
      });
  ...
  async launchAndWait(ctx) {
    ...
    while (Date.now() < deadline) {
      ...
      await delay(_MacComputerUseRPCClient.launchPollIntervalMs);
    }
    throw new MacComputerUseRPCError("local-cua sidecar did not become ready in time");
  }
  static isPreDeliveryConnectionError(error3) {
    const code = redactMacComputerUseError(error3);
  ...
  static appSupportDirectory() {
    ...
    return path11.join(os9.homedir(), "Library", "Application Support", _MacComputerUseRPCClient.appSupportFolder);
  }
  static serviceStatePath() {
    return path11.join(_MacComputerUseRPCClient.appSupportDirectory(), "service.json");
  }
  /**
  ...
  static serviceAppPath() {
    ...
    return path11.join(os9.homedir(), ".cursor", "cursor-computer-use", `${_MacComputerUseRPCClient.serviceAppBundleName}.app`);
  }
  /**
  ...
  static sidecarLaunchLogFields(appPath) {
    ...
    return {
      appName: path11.basename(appPath) === expectedName ? expectedName : "custom",
      appPathHash: (0, import_node_crypto7.createHash)("sha256").update(appPath).digest("hex"),
      usingOverride: Boolean(process.env[_MacComputerUseRPCClient.serviceAppOverrideEnv])
    };
  }
  static spawnErrorCode(error3) {
    return redactMacComputerUseError(error3);
  }
  static socketError(error3) {
    const code = redactMacComputerUseError(error3);
    return new MacComputerUseRPCError(`local-cua RPC connection failed: ${code}`, { code });
  }
  static serviceExecutablePath() {
    return path11.join(_MacComputerUseRPCClient.serviceAppPath(), "Contents", "MacOS", "CUCursorService");
  }
  /**
  ...
  static spawnSidecarProcess(ctx, command, args, options2) {
    return new Promise((resolve14, reject2) => {
      ...
      const fail = (error3) => {
        reject2(new MacComputerUseRPCError(`local-cua sidecar launch failed: ${_MacComputerUseRPCClient.spawnErrorCode(error3)}`));
      };
  ...
  static async readServiceState() {
    try {
      const raw = await (0, import_promises14.readFile)(_MacComputerUseRPCClient.serviceStatePath(), "utf8");
      return MacRPCProtocol.parseServiceState(raw);
    } catch (error3) {
      if (error3 instanceof Error && "code" in error3 && error3.code === "ENOENT") {
        return void 0;
      }
      if (error3 instanceof MacRPCProtocolError) {
        return void 0;
      }
  ...
  static sendOnce(socketPath, request3, timeoutMs, signal) {
    ...
    return new Promise((resolve14, reject2) => {
      ...
      const socket = (0, import_node_net.createConnection)({ path: socketPath });
      const finish = (fn) => {
        if (settled) {
          return;
        }
...
`, (error3) => {
          if (error3 != null) {
            finish(() => reject2(_MacComputerUseRPCClient.socketError(error3)));
            return;
          }
      ...
      socket.on("data", (chunk) => {
        ...
        bufferedBytes = 0;
        try {
          const parsed = MacRPCProtocol.parseResponse({
            line,
