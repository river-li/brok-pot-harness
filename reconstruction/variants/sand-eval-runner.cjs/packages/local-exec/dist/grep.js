...
function unresolvedTargetPathError(path30) {
  return `Path '${path30}' could not be verified while resolving symlinks. This can happen when access is denied or the path contains a symlink loop.`;
}
function findOwningWorkspaceRoot(targetPath, workspaceRoots) {
  let owner;
  for (const candidateRoot of workspaceRoots) {
    if (isPathWithin({ basePath: candidateRoot, targetPath }) && (owner === void 0 || candidateRoot.length > owner.length)) {
      owner = candidateRoot;
    }
  }
  return owner;
...
function fileBelongsToCurrentWorkspace(currentRoot, filePath, workspaceRoots) {
  const absoluteFilePath = (0, import_node_path32.resolve)(currentRoot, filePath);
...
function deduplicateCrossWorkspaceResults(workspaceResults, workspacePaths) {
  ...
  const resolvedRoots = [...new Set(workspacePaths.map((root) => (0, import_node_path32.resolve)(root)))];
  const deduped = {};
  for (const [root, unionResult] of Object.entries(workspaceResults)) {
    const resolvedRoot = (0, import_node_path32.resolve)(root);
...
function parseContentOutput(stdout, clientLimitLines, ripgrepHardCuttoffLines, ctx) {
  ...
  try {
    const _span = __addDisposableResource15(env_1, createSpan(ctx.withName("parseContentOutput")), false);
...
var logger16 = createLogger("local-exec/grep");
function isEnoentSpawnError(e) {
  if (!(e instanceof Error))
    return false;
  return e.message.includes("ENOENT") && e.message.includes("spawn");
}
function safeExists(path30) {
  try {
    return (0, import_node_fs15.existsSync)(path30);
  } catch {
    return "check_failed";
  }
}
function buildRipgrepEnoentMetadata(rgPath, cwd, error3, logContext) {
  return {
    ...
    errno: error3.errno,
    syscall: error3.syscall,
    errorPath: error3.path
  };
}
function createPathDoesNotExistError(path30) {
  return new Error(`Path does not exist: ${path30}`);
}
function probeRipgrepPath(rgPath) {
  ...
  try {
    probe.rgPathExists = (0, import_node_fs15.existsSync)(rgPath);
  } catch (e) {
    probe.rgPathExists = false;
    probe.rgPathExistsError = e instanceof Error ? e.message : String(e);
  }
  try {
    const lst = (0, import_node_fs15.lstatSync)(rgPath);
  ...
  const parentDir = (0, import_node_path32.dirname)(rgPath);
  try {
    probe.parentDirExists = (0, import_node_fs15.existsSync)(parentDir);
    if (probe.parentDirExists) {
      probe.parentDirContents = (0, import_node_fs15.readdirSync)(parentDir).slice(0, MAX_DIR_ENTRIES);
    }
  } catch (e) {
    probe.parentDirError = e instanceof Error ? e.message : String(e);
  }
  const grandparentDir = (0, import_node_path32.dirname)(parentDir);
  try {
    probe.grandparentDirExists = (0, import_node_fs15.existsSync)(grandparentDir);
    if (probe.grandparentDirExists) {
      probe.grandparentDirContents = (0, import_node_fs15.readdirSync)(grandparentDir).slice(0, MAX_DIR_ENTRIES);
    }
  } catch (e) {
  ...
  if (probe.rgPathExists) {
    try {
      const st2 = (0, import_node_fs15.statSync)(rgPath);
  ...
  if (rgPath.includes(asarUnpackedSegment)) {
    ...
    if (prefix) {
      ...
      try {
        probe.asarExists = (0, import_node_fs15.existsSync)(asarPath);
      } catch {
        probe.asarExists = "check_failed";
      }
      const asarUnpackedDir = prefix.concat(asarUnpackedSegment);
      try {
        probe.asarUnpackedDirExists = (0, import_node_fs15.existsSync)(asarUnpackedDir);
        if (probe.asarUnpackedDirExists) {
          const ripgrepPkgDir = (0, import_node_path32.join)(asarUnpackedDir, "@vscode", "ripgrep");
          probe.ripgrepPkgDirExists = (0, import_node_fs15.existsSync)(ripgrepPkgDir);
        }
      } catch {
...
var LocalGrepExecutor = class _LocalGrepExecutor {
  ...
  computeRelativeTarget(path30, root) {
    const resolvedPath = resolvePath(path30, this.singleWorkspacePath);
    if (resolvedPath === root) {
      return ".";
    }
    const result = (0, import_node_path32.relative)(root, resolvedPath);
    return result;
  }
  ...
  async runFilesMode(ctx, root, params, sandboxPolicy, logContext) {
    ...
    try {
      const _span = __addDisposableResource15(env_2, createSpan(ctx.withName("LocalGrepExecutor.runFilesMode")), false);
      ...
      const filteredFiles = (await this.filterBlockedFiles(files.map((file) => ({ file })), root)).map(({ file }) => file);
      ...
      __disposeResources15(env_2);
    }
  }
  async runCountMode(ctx, root, params, sandboxPolicy, logContext) {
    const env_3 = { stack: [], error: void 0, hasError: false };
    try {
      const _span = __addDisposableResource15(env_3, createSpan(ctx.withName("LocalGrepExecutor.runCountMode")), false);
      ...
      __disposeResources15(env_3);
    }
  }
  async runContentMode(ctx, root, params, sandboxPolicy, logContext) {
    const env_4 = { stack: [], error: void 0, hasError: false };
    try {
      const _span = __addDisposableResource15(env_4, createSpan(ctx.withName("LocalGrepExecutor.runContentMode")), false);
    ...
    return new Promise((resolve14, reject2) => {
      ...
      try {
        ...
        if (isEnoentSpawnError(e)) {
          ...
          logger16.error(ctx, "rg_diagnostics.spawn_throw", void 0, buildRipgrepEnoentMetadata(rgPath, cwd, errnoErr, logContext));
      ...
      proc.on("error", (err) => {
        ...
        if (isEnoentSpawnError(err)) {
          const rgPath = getRipgrepBinaryPath();
          const errnoErr = err;
          logger16.error(ctx, "rg_diagnostics.spawn_enoent", void 0, buildRipgrepEnoentMetadata(rgPath, cwd, errnoErr, logContext));
          if (safeExists(cwd) === false) {
  ...
  async execute(ctx, args, options2) {
    ...
    try {
      const _span = __addDisposableResource15(env_5, createSpan(ctx.withName("LocalGrepExecutor.execute")), false);
      ...
      try {
          ...
          for (const [root, unionResult] of Object.entries(workspaceResults)) {
            ...
            switch (result.case) {
              case "content":
                ...
                break;
              case "files":
                result.value.files = (await this.filterBlockedFiles(result.value.files.map((file) => ({ file })), root)).map(({ file }) => file);
                break;
              case "count":
        ...
        if (args.path) {
          ...
          try {
            await (0, import_promises20.stat)(resolvedPath);
          } catch (_e2) {
            return new GrepResult({
              result: {
                case: "error",
