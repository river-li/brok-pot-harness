...
var execFileAsync3 = (0, import_node_util8.promisify)(import_node_child_process11.execFile);
function hasFfmpegExited(child) {
  return child.exitCode !== null || child.signalCode !== null || child.killed;
}
function isErrnoCode(error3, code) {
...
var LocalRecordScreenExecutor = class _LocalRecordScreenExecutor {
  ...
  async dispose() {
    ...
    if (this.activeRecording) {
      ...
      try {
        await fs10.rm(this.activeRecording.sessionDir, {
          force: true,
          recursive: true
        });
      } catch {
  ...
  async ensureRecordingDirectoryExists() {
    await fs10.mkdir(this.stagingDir, { recursive: true });
    await fs10.mkdir(this.artifactsDir, { recursive: true });
  }
  /**
  ...
  buildSessionDir(toolCallId) {
    ...
    const dirname16 = `session-${timestamp2}-${sanitized}`;
    return path17.join(this.stagingDir, dirname16);
  }
  /**
  ...
  async movePolishedVideo(sourcePath, destinationPath) {
    try {
      await fs10.rename(sourcePath, destinationPath);
      return;
    } catch (error3) {
      if (!isErrnoCode(error3, "EXDEV")) {
        throw error3;
      }
    }
    try {
      await fs10.copyFile(sourcePath, destinationPath);
    } catch (error3) {
      await fs10.rm(destinationPath, { force: true }).catch(() => void 0);
      throw error3;
    }
    try {
      await fs10.unlink(sourcePath);
    } catch (error3) {
      await fs10.rm(destinationPath, { force: true }).catch(() => void 0);
      throw error3;
    }
  ...
  async getUniqueArtifactVideoPath(preferredFilename) {
    const parsed = path17.parse(preferredFilename);
    const baseName = parsed.name || "recording";
    const extension3 = parsed.ext || ".mp4";
    let candidateFilename = `${baseName}${extension3}`;
    let suffix = 2;
    for (; ; ) {
      const candidatePath = path17.join(this.artifactsDir, candidateFilename);
      try {
        const handle = await fs10.open(candidatePath, "wx");
        await handle.close();
        return candidatePath;
      } catch (error3) {
        if (!isErrnoCode(error3, "EEXIST")) {
          throw error3;
        }
  ...
  async resolveArtifactVideoPath(options2) {
    ...
    if (saveAsFilename) {
      ...
      if (validationResult.success) {
        ...
        preferredFilename = `${path17.basename(stagingSessionDir)}.mp4`;
      }
    } else {
      preferredFilename = `${path17.basename(stagingSessionDir)}.mp4`;
    }
  ...
  async renderPolished(stagingSessionDir) {
    const recordingDir = path17.join(stagingSessionDir, "recording");
    const polishedVideoPath = path17.join(recordingDir, "recording_full.mp4");
    ...
    await fs10.access(polishedVideoPath, fs10.constants.R_OK);
  ...
  async getVideoFramerate(videoPath) {
    try {
      ...
      console.warn(`[record-screen] Failed to read video framerate: ${error3 instanceof Error ? error3.message : String(error3)}, defaulting to ${_LocalRecordScreenExecutor.PROXY_TARGET_FPS}fps`);
    }
  ...
  async startFfmpegRecording(resolution, sessionDir) {
    const recordingDir = path17.join(sessionDir, "recording");
    await fs10.mkdir(recordingDir, { recursive: true });
    const proxyPath = path17.join(recordingDir, "recording_render_proxy_1080p.mp4");
    let refreshRate;
    try {
      const displayResult = await detectDisplay(this.display);
      refreshRate = displayResult.display.refreshRate;
    } catch (error3) {
      console.warn(`[record-screen] Failed to detect refresh rate: ${error3 instanceof Error ? error3.message : String(error3)}, using default ${_LocalRecordScreenExecutor.DEFAULT_REFRESH_RATE}Hz`);
  ...
  async runFfmpegCommand(label, args) {
    return await new Promise((resolve14, reject2) => {
      ...
      child.once("error", (error3) => {
        reject2(new Error(`ffmpeg ${label} failed to start: ${error3 instanceof Error ? error3.message : String(error3)}`));
      });
  ...
  async saveRecordingDataPackage(options2) {
    ...
    const recordingDir = path17.join(sessionDir, "recording");
    const dataPath = path17.join(recordingDir, "recording-data.json");
    ...
    await fs10.writeFile(dataPath, JSON.stringify(dataPackage, null, 2));
  }
  /**
  ...
  async stopFfmpegRecording(state) {
    ...
    return new Promise((resolve14, reject2) => {
      ...
      childProcess.once("error", (error3) => {
        clearTimeout(timeout2);
        reject2(new Error(`Error stopping ffmpeg: ${error3.message}`));
      });
      try {
        try {
  ...
  async performSave(args, state) {
    ...
    try {
      ...
      try {
        ...
        stopError = error3;
        console.warn(`[record-screen] Failed to stop ffmpeg: ${errorMessage2(error3)}`);
      }
      const ffmpegStartedEpochMs = state.ffmpegStartedEpochMs ?? recordingStartEpochMs;
      const stagingSessionDir = state.sessionDir;
      const stagingRecordingDir = path17.join(stagingSessionDir, "recording");
      const proxyPath = path17.join(stagingRecordingDir, "recording_render_proxy_1080p.mp4");
      let actualVideoDurationMs;
      let videoDimensions;
      let sourceFps;
      try {
        await fs10.access(proxyPath, fs10.constants.R_OK);
      } catch (error3) {
        await this.cleanupRecording(state);
        this.clearActiveRecordingIfCurrent(state);
        const message = isErrnoCode(error3, "ENOENT") ? `Recording proxy not found at ${proxyPath}` : `Recording proxy at ${proxyPath} could not be validated: ${errorMessage2(error3)}`;
      ...
      try {
        ...
        await this.cleanupRecording(state);
        const detail = errorMessage2(error3);
      ...
      try {
        if (this.disablePolishedRendering) {
          await fs10.copyFile(proxyPath, finalVideoPath);
        } else {
        ...
        await fs10.rm(finalVideoPath, { force: true }).catch(() => void 0);
        throw error3;
      }
      try {
        await fs10.rm(stagingSessionDir, { force: true, recursive: true });
      } catch (error3) {
        console.warn(`[record-screen] Failed to remove staging session ${stagingSessionDir}: ${errorMessage2(error3)}`);
      }
      this.clearActiveRecordingIfCurrent(state);
      return new RecordScreenResult({
        result: {
  ...
  async execute(parentCtx, args) {
    ...
    try {
      const _span = __addDisposableResource18(env_1, createSpan(parentCtx.withName("LocalRecordScreenExecutor.execute")), false);
      try {
        ...
        return new RecordScreenResult({
          result: {
            case: "failure",
            value: new RecordScreenFailure({
              error: `Failed to create recording directory: ${error3 instanceof Error ? error3.message : String(error3)}. Ensure the parent directory exists and has write permissions, or run: sudo mkdir -p ${this.stagingDir} ${this.artifactsDir} && sudo chmod 777 ${this.stagingDir} ${this.artifactsDir}`
            })
          }
        });
      }
      ...
      switch (args.mode) {
        case RecordingMode.START_RECORDING: {
          ...
          if (this.activeRecording) {
            ...
            if (!this.inFlightSaves.some((entry) => entry.state === prior)) {
              ...
              try {
                ...
                await fs10.rm(prior.sessionDir, {
                  force: true,
                  recursive: true
                }).catch(() => {
                });
          ...
          try {
            ...
            try {
              ...
              throw new Error(`ffmpeg failed to start encoding: ${error3 instanceof Error ? error3.message : String(error3)}`);
            }
            const state = {
              childProcess,
              sessionDir,
            ...
            if (sessionDir) {
              await fs10.rm(sessionDir, { force: true, recursive: true }).catch(() => {
              });
            }
            this.activeRecording = null;
            return new RecordScreenResult({
        ...
        case RecordingMode.DISCARD_RECORDING: {
          ...
          try {
            ...
            await fs10.rm(state.sessionDir, { force: true, recursive: true });
            this.activeRecording = null;
            return new RecordScreenResult({
              result: {
                case: "discardSuccess",
            ...
            let removed = false;
            try {
              await fs10.rm(sessionDir, { force: true, recursive: true });
              removed = true;
            } catch {
            }
            this.activeRecording = null;
