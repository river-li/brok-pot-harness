...
function parseCloudCanvasWebLink(href) {
  try {
    ...
    if (parsed.protocol !== "https:" || !CURSOR_WEB_HOSTNAMES.has(parsed.hostname) || parsed.port.length > 0 || parsed.username.length > 0 || parsed.password.length > 0 || parsed.search.length > 0 || parsed.hash.length > 0) {
      return void 0;
    }
    const segments = parsed.pathname.split("/").filter(Boolean);
    if (segments.length !== 3 || segments[0] !== "canvas" || parsed.pathname !== `/${segments.join("/")}`) {
      return void 0;
    }
