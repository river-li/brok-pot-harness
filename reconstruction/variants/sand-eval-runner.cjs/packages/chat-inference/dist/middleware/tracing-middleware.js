...
function analyzeMessages(messages) {
  ...
  for (const message of messages) {
    const textContent2 = getMessageTextContent(message);
    const charCount = textContent2.length;
    switch (message.role) {
      case "user":
        stats.userCount++;
  ...
  stream(ctx, invocationId, tools, options2) {
    ...
    const tracedFullStream = function() {
      return __asyncGenerator4(this, arguments, function* () {
        ...
        try {
          try {
            for (var _d = true, _e2 = __asyncValues3(streamResult.fullStream), _f; _f = yield __await4(_e2.next()), _a20 = _f.done, !_a20; _d = true) {
              _c2 = _f.value;
              _d = false;
              const part = _c2;
              if (!seenFirstToken) {
              ...
              yield yield __await4(part);
            }
          } catch (e_1_1) {
            e_1 = { error: e_1_1 };
          } finally {
            try {
              if (!_d && !_a20 && (_b2 = _e2.return)) yield __await4(_b2.call(_e2));
            } finally {
