...
function errorLabel(error3) {
  if (error3 instanceof Error) {
    return error3.message.length > 0 ? error3.message : error3.name;
  }
  ...
  return {
    ...
    async executeTool(ctx, args, options3) {
      try {
        ...
        recordMcpExecErrorClass(ctx, error3);
        return errorResult(`Box MCP execution failed for "${args.name}": ${errorLabel(error3)}`);
      }
    }
  };
}
