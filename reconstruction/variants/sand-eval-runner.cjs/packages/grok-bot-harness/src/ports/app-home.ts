...
function parseSandAppHomeDocument(value) {
  const parsed = sandAppHomeDocumentSchema.safeParse(value);
  if (!parsed.success) {
    const issue2 = parsed.error.issues[0];
    const path30 = issue2 === void 0 || issue2.path.length === 0 ? "" : `${issue2.path.join(".")}: `;
    return { ok: false, reason: `${path30}${issue2?.message ?? "invalid board"}` };
  }
  const bytes = sandAppHomeDocumentByteLength(parsed.data);
