...
function countToolCallsSinceLastSendMessage(messages) {
  let count = 0;
...
function hasSendMessageSinceRealTurnStart(messages) {
  for (let index = messages.length - 1; index >= 0; index--) {
    const message = messages[index];
...
function hasReminderFiredThisSilentStreak(messages) {
  for (let index = messages.length - 1; index >= 0; index--) {
    const message = messages[index];
    if (isInjectedReminderMessage2(message)) {
      return true;
