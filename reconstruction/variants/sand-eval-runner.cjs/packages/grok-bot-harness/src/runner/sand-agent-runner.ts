...
var SandAgentRunner = class _SandAgentRunner {
  ...
  constructor(options2) {
    ...
    this.runShell = (options2.createTurnRunShell ?? createTurnRunShell)({
      ...
      setActiveTurnAutomationWakeEmail: (email2) => {
        this.activeTurnAutomationWakeEmail = email2;
      },
      streamTuning: this.streamTuning,
      ...
      noteMcpToolDiscoveryFailed: (error3) => this.noteMcpToolDiscoveryFailed(error3),
  ...
  resolveBoxBrowser() {
    ...
    return {
      display: `:${windowIndex}`,
      cdpUrl: `http://127.0.0.1:${BOX_CDP_PORT_BASE3 + windowIndex}`
    };
  }
  ...
  async drainNavigationAudit(timeoutMs = 1e4) {
    ...
    await Promise.race([
      probe.flush().catch((error3) => {
        process.stderr.write(
          `sand.turn.navigation_audit_drain_failed error_class=${errorLogTag(error3)}
`
        );
      }),
      delay2(timeoutMs)
  ...
  stampInheritableRunAttributes(runCtx, inferenceRequestId, turnType) {
    try {
      ...
      process.stderr.write(
        `sand.turn.run_attribute_stamp_failed error_class=${errorLogTag(error3)}
`
      );
    }
    return runCtx;
  ...
  stampCallerTurnRootRequestId(span, inferenceRequestId) {
    try {
      ...
      process.stderr.write(
        `sand.turn.caller_span_attribute_failed error_class=${errorLogTag(error3)}
`
      );
    }
  }
