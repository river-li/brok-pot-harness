...
function matchesHost(criteria, hostname2) {
  return (criteria.equals?.includes(hostname2) ?? false) || (criteria.suffix?.some((suffix) => hostname2.endsWith(suffix)) ?? false);
}
...
function classifyBotBlockPage(page) {
  ...
  const hostname2 = url2.hostname.toLowerCase().replace(/^www\./, "");
  ...
  for (const signature of BOT_BLOCK_SIGNATURES) {
    if (matchesSignature(signature, {
      ...
      return {
        ...
        blockedHost: hostname2.slice(0, MAX_BLOCKED_HOST_LENGTH),
