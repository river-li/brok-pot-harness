...
function buildAttachedFilesNote({
  ...
  const cleaned = entries.flatMap((entry) => {
    const path30 = entry.path.trim();
    return path30.length > 0 ? [{ path: path30, kind: entry.kind }] : [];
  });
  if (cleaned.length === 0) return "";
  const list = cleaned.map(({ path: path30, kind }) => {
    const size = sizeByPath.get(path30);
    const sizeSuffix = size != null ? ` (${formatAttachedFileSize(size)})` : "";
    return `
- ${path30}${sizeSuffix}${ATTACHED_MEDIA_LINE_SUFFIX[kind]}`;
  }).join("");
