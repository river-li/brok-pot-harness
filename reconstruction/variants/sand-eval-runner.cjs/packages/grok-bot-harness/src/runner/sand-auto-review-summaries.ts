...
function humanizeMcpToolAction(toolName) {
  const words = compact(toolName, 80).replace(/([a-z0-9])([A-Z])/g, "$1 $2").replace(/[-_.]+/g, " ").trim().toLowerCase().split(/\s+/).filter((word) => word.length > 0);
  if (words.length === 0) return void 0;
  if (words.length === 2 && /^(send|create|update|delete|post|write|read|add|remove|open|close|list|search|fetch|get|set|upload|download|invite|share)$/.test(
    words[0] ?? ""
  ) && !/s$/i.test(words[1] ?? "")) {
...
function summarizeSandEmailSendAction(args) {
  ...
  const files = args.attachments.length === 0 ? " with no attachments" : ` attaching ${args.attachments.length} file${args.attachments.length === 1 ? "" : "s"}: ${args.attachments.map((file) => {
    const size = `${Math.max(1, Math.round(file.sizeBytes / 1024))} KB`;
    const excerpt = file.textPreview === void 0 ? " [binary, content not shown]" : ` \u201C${compactHeadAndTail(redactSandAutoReviewInlineSecrets(file.textPreview), 160)}\u201D`;
    return `${compact(file.filename, 80)} (${size})${excerpt}`;
  }).join(", ")}`;
