...
async function pollShellTerminalFile(host, id, isCancelled, settle) {
  ...
  while (!isCancelled()) {
    try {
      ...
      if (error3 instanceof SandLocalToolPermissionDeniedError) {
        settle(
          "error",
...
async function readShellTerminalSnapshot(host, accessor, path30, ctx = host.ctx) {
  const readExecutor = accessor.get(readExecutorResource);
  const result = await readExecutor.execute(ctx, new ReadArgs({ path: path30, toolCallId: "" }));
