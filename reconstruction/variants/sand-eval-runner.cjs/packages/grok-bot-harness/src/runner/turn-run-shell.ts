...
function createTurnRunShell(host) {
      ...
      tools = [
        ...await traceSendPhase(ctx, "mcp.getTools", async (childCtx) => {
          ...
          try {
            ...
            cancelDiscovery(error3);
            throw error3;
          }
        })
      ];
    } catch (error3) {
      if (ctx.canceled) return { kind: "canceled", error: error3 };
      host.noteMcpToolDiscoveryFailed(error3);
    }
  ...
  async function run(prompt, options2 = {}) {
    ...
    try {
      ...
      const tags = resolveTurnOutcomeTags({ conversationId, outcome, error: failure });
  ...
  async function runTurn(prompt, options2, hostReceiptPerfMs, performanceObservation) {
    ...
    try {
      ...
      const completeThisRun = (requestId = inferenceRequestId) => {
        if (!ctx.canceled && cancelActiveRun === cancelRun && stopRequest.kind === "none") {
          stopRequest = { kind: "complete", requestId };
        }
      };
      const pauseThisRun = (requestId = inferenceRequestId) => {
        if (ctx.canceled) return;
        if (cancelActiveRun === cancelRun) {
          stopRequest = { kind: "awaiting-user", requestId };
          return;
        }
        cancelRun(
          new SandRunAbortError({
      ...
      try {
        ...
        const emitRequestId = (requestId) => {
          options2.onRequestId?.(requestId);
          idleCompaction?.noteRequestId(requestId);
          host.emitUpdate({ type: "request-id", requestId });
        };
        ...
        const userFormVaultKeysPromise = host.resolveUserFormVaultKeysForRun?.().catch((error3) => {
          console.warn(
            `[sand:user-form-vault] keys-catalog fetch failed; request_user_form carries no catalog this run: ${errorLogTag(error3)}`
          );
          return void 0;
        ...
        const boxConnection = await traceSendPhase(ctx, "box.ensureReady", async (childCtx) => {
          try {
            ...
            if (error3 instanceof SandBoxDaemonUnreachableError) {
              const reason = boxNotReadyReasonForError(error3);
              throw new SandBoxNotReadyError(reason.errorKind, reason.message, { cause: error3 });
            }
            throw error3;
          }
        });
        ...
        endRunLifecycle();
        if (!ctx.canceled) {
          markTurnTraceError(runSpan, error3);
          markTurnTraceError(turnTrace, error3);
          throw error3;
        }
