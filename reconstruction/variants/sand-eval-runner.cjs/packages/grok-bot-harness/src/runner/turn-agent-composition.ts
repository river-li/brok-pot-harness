...
function createTurnAgentComposition(host) {
  ...
  function buildAgentForRun(sessions, boxConnection, turnScope) {
    ...
    const localToolPermission = host.localToolPermission;
    ...
    const hostShellApprovalProvider = autoReviewModes.hostShell === "enforce" && host.autoReviewController !== void 0 ? createSandShellApprovalProvider({
      ...
      ...localToolPermission !== void 0 ? {
        beforeApproval: (request3) => localToolPermission.awaitDesktopStandingDecision({
          agentId: host.getConversationId(),
        ...
        resourceEntry(mcpExecutorResource, {
          execute: async (ctx, args, options2) => {
            ...
            try {
              ...
              settleMcpExecObservation({ kind: "error", errorClass: mcpErrorClassOf(error3) });
              throw error3;
            }
...
${note}`;
    ...
    const config2 = {
      ...
      messageHistoryModifier: profilePromptSnapshot != null && onProfileUpdateAppended != null ? (messages) => host.promptGlue.appendProfileUpdateToHistory(
        messages,
        profilePromptSnapshot,
        onProfileUpdateAppended
      ) : void 0,
