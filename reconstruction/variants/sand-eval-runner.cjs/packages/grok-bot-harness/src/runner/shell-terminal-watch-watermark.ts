 ...
 * Source: ../packages/grok-bot-harness/src/runner/shell-terminal-watch-watermark.ts
...
function decodeConfirmedUserTurnWatermark(bytes) {
  ...
  try {
    const parsed = storedWatermarkSchema.safeParse(JSON.parse(new TextDecoder().decode(bytes)));
...
async function findConfirmedUserTurnWatermark(host, state) {
  ...
  for (let index = turnCount - 1; index >= stopIndex; index--) {
    ...
    try {
      ...
      userMessage = UserMessage.fromBinary(userMessageBlob);
    } catch {
      return { lastUserMessageId: void 0, hasUserTurn: true };
    }
    if (userMessage.text.startsWith(SAND_HIDDEN_PROMPT_MARKER)) continue;
    if (isOffRecordMessageId(userMessage.messageId)) continue;
    if (userMessage.messageId.length === 0 && isGroupTurnPromptText(userMessage.text)) {
      continue;
    }
    return cacheResult({
