 ...
 * Source: ../packages/grok-bot-harness/src/runner/turn-memory.ts
...
async function runTurnMemory(memoryStore, episodeProgress, session, ctx, turnTs, exchange) {
  ...
  try {
    ...
    process.stderr.write(`sand.memory.episode_summary_failed error_class=${errorLogTag(error3)}
`);
  }
}
...
${exchange.agent}`
    ...
    process.stderr.write(`sand.memory.extraction_failed error_class=${errorLogTag(error3)}
`);
  }
}
