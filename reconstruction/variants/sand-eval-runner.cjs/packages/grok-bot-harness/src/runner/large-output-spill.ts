 ...
 * Source: ../packages/grok-bot-harness/src/runner/large-output-spill.ts
  ...
  return async (ctx, result) => {
    ...
    const materialized = await materializeMcpTextOutput({
      ...
      write: async (aggregateText) => {
        try {
          const relativePath = import_node_path56.posix.join(AGENT_TOOLS_DIR, `${(0, import_node_crypto30.randomUUID)()}.txt`);
    ...
    if (materialized === void 0 || materialized === result.result.value.content) {
      return result;
    }
