...
function shellRewatchPollMs() {
  ...
  return Number.isFinite(parsed) && parsed > 0 ? parsed : SHELL_REWATCH_POLL_DEFAULT_MS;
}
function parseShellTerminalFooter(content) {
  ...
  if (exitCodeLine != null) {
    ...
    return {
      kind: "exited",
      exitCode: raw.length > 0 && Number.isInteger(parsed) ? parsed : null
    };
  }
...
function createBackgroundWatches(host) {
  ...
  const watchCloudAgent = (bcId, options2) => {
    ...
    const settle = (completion) => {
      if (watchedCloudAgentBcIds.get(id) !== watch) return;
