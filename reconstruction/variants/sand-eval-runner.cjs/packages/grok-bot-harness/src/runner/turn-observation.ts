...
function createTurnObservation(host, timing) {
  ...
  function listAsyncTasks() {
    ...
    for (const [bcId, watch] of host.cloudAgentWatchEntries()) {
      tasks.push({
        kind: "cloud-agent",
        id: bcId,
        label: watch.title,
        labelKind: watch.labelKind,
        labelParams: watch.labelParams,
        status: "running",
    ...
    if (deps.enterEpochMs !== void 0 && Number.isFinite(deps.enterEpochMs)) {
      try {
        ...
        process.stderr.write(
          `sand.turn.send_dispatch_observation_failed error_class=${errorLogTag(error3)}
`
        );
      }
    }
    ...
    const observeFirstToken = (chunkType) => {
      ...
      try {
        ...
        process.stderr.write(
          `sand.turn.first_token_observation_failed error_class=${errorLogTag(error3)}
`
        );
      }
    };
