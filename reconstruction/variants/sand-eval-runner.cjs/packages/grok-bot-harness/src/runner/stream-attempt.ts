...
function createStreamAttempt(host) {
  ...
  const runStreamOnce = async () => {
    ...
    const persistCheckpoint = async (checkpointCtx, checkpoint) => {
      ...
      try {
        ...
        checkpointFailure ??= { status: "rejected", reason: error3 };
        throw error3;
      } finally {
    ...
    if (policy.firstTokenDeadlineMs <= 0) {
      try {
        ...
        const failed = await drainCheckpoints();
    ...
    return new Promise((resolve14, reject2) => {
      ...
      const onDeadline = () => {
        ...
        void drainCheckpoints().then((failed) => {
          if (failed != null) {
            reject2(failed.reason);
            return;
          }
          reject2(
      ...
      void streamPromise.then(
        ...
        (error3) => {
          ...
          hooks.disarm();
          void drainCheckpoints().then((failed) => {
            reject2(failed == null ? error3 : failed.reason);
          });
        }
      );
    });
  ...
  const runBoundedTurn = async () => retryPolicy.maxAttempts > 1 ? await runWithTransientRetry(runStreamOnce, {
    ...retryPolicy,
    isRetryable: (error3) => shouldRetryTurnAttempt({
      error: error3,
  ...
  const run = async () => {
    try {
      ...
      if (!ctx.canceled) {
        ...
        } else if (retriesPerformed > 0 || isRetryableProviderError(error3)) {
          host.reportTurnRetry({
