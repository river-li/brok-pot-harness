...
function isMemorableExchange(userMessage) {
...
function formatMemoryDate(createdAtMs) {
...
function buildExtractionUserPrompt(userMessage, agentMessage, existingMemories) {
  ...
  return [
    ...
    "",
    "Latest exchange:",
    `User: ${userMessage.trim().length > 0 ? userMessage.trim() : "(no message)"}`,
...
async function extractMemories(args) {
  const { executor, ctx, userMessage, agentMessage, existingMemories } = args;
  executor.appendMessages([
    { role: "system", content: buildExtractionSystemPrompt() },
    {
      role: "user",
      content: buildExtractionUserPrompt(userMessage, agentMessage, existingMemories),
...
function isRecentlyActive(session, nowMs) {
  const lastActivityAtMs = session.lastActivityAtMs;
  if (lastActivityAtMs == null || lastActivityAtMs <= 0) return true;
  return nowMs - lastActivityAtMs <= GROK_BOT_ACTIVE_SESSION_MAX_AGE_MS;
}
function renderActiveSessionsDigest(sessions, nowMs = Date.now()) {
  const active = sessions.filter((session) => isRecentlyActive(session, nowMs));
