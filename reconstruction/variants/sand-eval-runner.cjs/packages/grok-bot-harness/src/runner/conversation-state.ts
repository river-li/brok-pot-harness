...
init_dist();
var logger97 = createLogger("sand:conversation-state");
...
function buildUnansweredQuestionsNote(prompts) {
  ...
  const skipped = clean(prompts.skipped);
...
Treat these as settled and already handled on the user's side: take them into account together with the message below, follow the guidance each one carries, and don't raise the same cards again.`
  ...
  if (skipped.length === 1) {
    sections.push(
      `Earlier you prompted the user and they moved on without responding ("${skipped[0]}") \u2014 treat it as skipped. Don't wait for or assume a response; continue with what you already know, and only ask again if you still genuinely need it.`
    );
  } else if (skipped.length > 1) {
    const list = skipped.map((prompt) => `
- "${prompt}"`).join("");
    sections.push(
...
function safelyObservePromptStream(ctx, callback) {
  try {
    ...
    logger97.warn(ctx, `Prompt stream observer failed (${errorLogTag(error3)})`);
    return false;
  }
}
