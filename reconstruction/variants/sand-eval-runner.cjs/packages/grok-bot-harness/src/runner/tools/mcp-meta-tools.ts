...
function dynamicToolCallErrorClass(error3) {
  if (error3 instanceof CustomToolCallError) return error3.classification;
  if (error3 instanceof ToolTimeoutError) return "timeout";
  return errorLogTag(error3);
}
...
function invokesDesignatedDangerousMcpTool(rawArgs) {
  ...
  let invoked = invokedToolNameOf(parsed);
  if (invoked !== void 0 && MCP_INVOCATION_TOOL_NAMES.has(invoked.trim().toLowerCase().replaceAll("_", ""))) {
    const nested = typeof parsed === "object" && parsed !== null && "arguments" in parsed ? parsed.arguments : void 0;
...
function wrapDynamicInvocationToolWithTimeout({
  ...
  return {
    ...
    execute: async (ctx, interactionHandler, argsStream, meta) => {
      ...
      const reportDynamicOutcome = (outcome, error3) => {
        ...
        observeDynamicToolCall?.({
          ...
          outcome,
          ...outcome === "error" ? { errorClass: dynamicToolCallErrorClass(error3) } : {},
      ...
      try {
        ...
        if (!(callCtx.canceled && isIntentionalAbortReason(callCtx.reason))) {
          reportDynamicOutcome("error", error3);
        }
        throw error3;
      } finally {
        unregisterPauseCancel?.();
