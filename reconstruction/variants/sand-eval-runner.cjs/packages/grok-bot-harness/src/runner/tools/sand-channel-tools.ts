...
var createChannelParameters = external_exports.object({
  ...
  member_ids: external_exports.array(external_exports.string().trim().min(1)).min(1).describe(
    `The agent ids to seat in the channel, up to ${GROUP_MAX_MEMBERS}. Use ids from your teammates list \u2014 not names \u2014 and never a channel id (channels cannot nest).`
  )
});
...
function createUpdateChannelTool(management) {
  return defineCommunicateTool(management, {
    ...
    description: `Add and/or remove member agents of an existing channel (a group chat) by its id. Only the ids you pass change; the rest of the roster stays exactly as it was, and a channel holds at most ${GROUP_MAX_MEMBERS} members. A channel always keeps at least one member \u2014 a removal that would empty it is refused. Members you add see the channel in their next channel turn; removed members simply stop receiving its posts. You can only change channels you are currently a member of; any other channel is reported as not found.`,
