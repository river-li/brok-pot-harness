...
var logger103 = createLogger("sand:messages-tools");
function messagesToolAction(toolIdentifier) {
  return toolIdentifier === "SEND_IMESSAGE" ? "send-imessage" : "read-messages";
}
var MESSAGES_ATTACHMENT_BOX_DIR = import_node_path64.posix.join(SAND_BOX_UPLOADS_DIR, "messages");
...
function decodeAttachmentEnvelope(raw) {
  ...
  try {
    ...
    if (!(error3 instanceof SyntaxError)) throw error3;
    return { text: raw };
  }
  if (parsed === null || typeof parsed !== "object") return { text: raw };
  const text2 = "text" in parsed ? parsed.text : void 0;
  if (typeof text2 !== "string") return { text: raw };
  const imageKey = "imageKey" in parsed ? parsed.imageKey : void 0;
  return {
    text: text2,
...
function boxFileName(filename) {
  const cleaned = import_node_path64.posix.basename(filename).replace(/[^a-zA-Z0-9._-]/g, "_");
...
async function fetchAttachment(ctx, args, deps, inflightImages) {
  ...
  const boxPath = import_node_path64.posix.join(MESSAGES_ATTACHMENT_BOX_DIR, boxFileName(filename));
...
function boundErrorClass(error3) {
  const name17 = errorClassOf(error3);
...
function failedToolUse(op, error3, durationMs) {
  if (isMessagesDecline(error3)) return { op, outcome: "declined", durationMs };
  return {
    op,
    outcome: "error",
    errorClass: boundErrorClass(error3),
    errorCode: sandMessagesErrorCode(error3),
    durationMs
  };
}
function reportMessagesToolUse(ctx, report, makeUse) {
  try {
    ...
    logger103.warn(ctx, `Messages tool-use report failed (${errorLogTag(error3)})`);
  }
}
function instrumentMessages(messages, report) {
  return {
    ...
    run: async (ctx, op, display) => {
      ...
      let result;
      try {
        result = await messages.run(ctx, op, display);
      } catch (error3) {
        const paused = error3 instanceof DeferredInteractionResponseError;
        const interrupted = ctx.canceled && isIntentionalAbortReason(ctx.reason);
        if (!paused && !interrupted) {
          reportMessagesToolUse(
            ctx,
            report,
            () => failedToolUse(op.kind, error3, performance.now() - startedAt)
          );
        }
        throw error3;
      }
...
function createMessagesTools(deps) {
  const gated = gateMessagesOnGrants(deps.messages, deps.messagesGrants, deps.reportGrantsAsk);
  const toolDeps = {
    ...deps,
    messages: deps.reportToolUse === void 0 ? gated : instrumentMessages(gated, deps.reportToolUse)
  };
  return [
    defineCommunicateTool(toolDeps, {
      id: "FIND_CONTACTS",
