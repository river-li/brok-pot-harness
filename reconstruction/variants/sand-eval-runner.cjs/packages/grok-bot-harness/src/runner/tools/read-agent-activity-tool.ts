...
function formatAgentActivityAge(iso, nowMs) {
...
function renderSummary(summary, nowMs) {
  ...
  const lastUser = summary.lastUserMessageAt === null ? "none" : formatAgentActivityAge(summary.lastUserMessageAt, nowMs);
  const lastAssistant = summary.lastAssistantMessageAt === null ? "none" : formatAgentActivityAge(summary.lastAssistantMessageAt, nowMs);
  ...
  if (summary.pendingQuestions.length > 0) {
    ...
    for (const question of summary.pendingQuestions) {
      lines2.push(`- ${formatAgentActivityAge(question.askedAt, nowMs)}: ${question.text}`);
    }
  }
  lines2.push(
    "Recent excerpt:",
...
function renderAgentActivity(result, bounds, nowMs) {
  ...
  return [header, ...result.agents.map((summary) => renderSummary(summary, nowMs))].join("\n\n");
}
