...
function normalizeSourcePath(raw) {
  ...
  if (!import_node_path61.posix.isAbsolute(raw)) {
    throw new SandToolInputError(
  ...
  const normalized = import_node_path61.posix.normalize(raw);
...
function createUploadFileTool(deps) {
  return defineCommunicateTool(deps, {
    ...
    describeActivity: (args) => ({
      detail: import_node_path61.posix.basename(args.sourcePath),
