...
function normalizeDestinationPath(raw) {
  ...
  if (!import_node_path60.posix.isAbsolute(path30)) {
    throw new SandToolInputError(
      `destination.path ${JSON.stringify(path30)} is not absolute. Pass the full path on your computer, e.g. "/home/box/agent-data/agents/<id>/downloads/report.pdf", or omit it.`
    );
  }
  const normalized = import_node_path60.posix.normalize(path30);
...
function createDownloadFileTool(deps) {
  return defineCommunicateTool(deps, {
    ...
    describeActivity: (args) => ({
      detail: args.source.path === void 0 ? args.source.fileId ?? "" : import_node_path60.posix.basename(args.source.path),
