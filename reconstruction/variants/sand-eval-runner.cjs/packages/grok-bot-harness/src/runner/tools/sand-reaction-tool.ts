...
function createReactToMessageTool(deps) {
  return defineCommunicateTool(deps, {
    ...
    execute: async (_ctx, args, d) => {
      ...
      const emoji2 = args.emoji.trim();
      d.react({ messageAddress: address, emoji: emoji2 });
      return `Reacted ${emoji2} on ${address}. (Reactions toggle: react the same emoji again to take it back.)`;
    }
  });
}
