...
async function resolveAttachmentSource(ctx, sourceUrl, deps) {
  ...
  const fileName = sourcePath != null ? (0, import_node_path66.basename)(sourcePath) : void 0;
  const resolved = (url2) => ({
    url: url2,
  ...
  if (ingest != null) {
    try {
      return resolved((0, import_node_url9.pathToFileURL)(await ingest(sourcePath)).href);
    } catch (error3) {
      if (error3 instanceof SandToolInputError || error3 instanceof AttachmentTooLargeError) {
        throw error3;
      }
      ingestError = error3;
    }
