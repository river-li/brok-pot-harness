...
function botTemplateShareRefusalReason(error3) {
  if (error3 instanceof ConnectError) {
    const header = error3.metadata.get(BOT_TEMPLATE_SHARE_REASON_HEADER);
...
function botTemplateShareRefusalToolResult(error3) {
  const reason = botTemplateShareRefusalReason(error3);
  if (reason === "none") {
