 ...
 * Source: ../packages/grok-bot-harness/src/runner/tools/virtual-card-tool.ts
...
function normalizeMerchantUrl(raw) {
  const parsed = parseMerchantUrl(raw);
  switch (parsed.kind) {
    case "ok":
      return parsed.url;
    case "not-a-url":
    ...
    case "bad-protocol":
      throw new SandToolInputError(
        `merchantUrl must be an http(s) URL; got ${JSON.stringify(parsed.protocol)}.`
      );
  }
}
...
function normalizeTitle(raw) {
  ...
  if (words.length > VIRTUAL_CARD_MAX_TITLE_WORDS) {
    throw new SandToolInputError(
      `title is ${words.length} words; keep it to ${VIRTUAL_CARD_MAX_TITLE_WORDS} or fewer, naming the payment and nothing else, e.g. "Pay Letterform Archive for Type Selection".`
    );
  }
  return title;
}
