...
async function stopAllRunningSubagents(controller, args) {
  ...
  for (const outcome of outcomes) {
    ...
    switch (outcome.result) {
      case "ok":
        stopped.push(subagent);
        break;
      case "not-running":
        alreadyFinished.push(subagent);
        break;
      case "failed":
        failed.push({ ...subagent, error: outcome.error });
        break;
    }
  }
  return {
...
function formatDelegatedWorkLines(delegated) {
  ...
  const stopped = delegated.filter((outcome) => outcome.result === "stopped").length;
  return [
    `Work you handed to other agents (${stopped} of ${delegated.length} stopped):`,
