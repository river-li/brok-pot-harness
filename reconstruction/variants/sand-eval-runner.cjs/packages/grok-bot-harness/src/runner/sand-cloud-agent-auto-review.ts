...
function describeSandCloudAgentReviewImages(urls, images) {
  return images.map((image2, index) => ({
    ...
    sha256: (0, import_node_crypto35.createHash)("sha256").update(image2.data).digest("hex")
  }));
}
function describeSandCloudAgentReviewFiles(files) {
  return files.map((file) => ({
    ...
    sha256: (0, import_node_crypto35.createHash)("sha256").update(file.data).digest("hex")
  }));
}
