...
var logger29 = createLogger("summarization-handler");
...
function executeSummarizationStream(ctx, promptSession, summaryPrompt, maxOutputTokens) {
  return __awaiter40(this, void 0, void 0, function* () {
    ...
    result.extendedUsage.catch((e) => {
      logger29.error(ctx, "[summarization-handler] Error getting extended usage", e, {
        summarization: streamLogFields
      });
    });
    result.usage.catch((e) => {
      logger29.error(ctx, "[summarization-handler] Error getting usage", e, {
        summarization: streamLogFields
      });
    });
    result.providerMetadata.catch((e) => {
      logger29.error(ctx, "[summarization-handler] Error getting provider metadata", e, {
        summarization: streamLogFields
      });
    });
    result.invocationId.catch((e) => {
      logger29.error(ctx, "[summarization-handler] Error getting invocation id", e, {
        summarization: streamLogFields
      });
    });
    result.response.catch((e) => {
      logger29.error(ctx, "[summarization-handler] Error getting response", e, {
        summarization: streamLogFields
      });
    });
    try {
      for (var _d = true, _e2 = __asyncValues10(result.fullStream), _f; _f = yield _e2.next(), _a20 = _f.done, !_a20; _d = true) {
        _c2 = _f.value;
        _d = false;
        const _chunk = _c2;
      }
      ...
      try {
        if (!_d && !_a20 && (_b2 = _e2.return)) yield _b2.call(_e2);
      } finally {
    ...
    if (assistantMessages.length > 1) {
      logger29.warn(ctx, "[summarization-handler] Summarization response contained more than one assistant message; using only the last to avoid duplicated summaries", {
...
function turnMessageToString(redactedMessage, enablePromptSizeGuards) {
  ...
  return content.map((c) => {
    ...
    if (c.type === "text") {
      ...
      const argsStr = (_a20 = JSON.stringify(c.args)) !== null && _a20 !== void 0 ? _a20 : "null";
...
function buildSummarizationPrompt(ctx, messages, options2) {
  var _a20, _b2;
  var _c2, _d, _e2;
  const suffix = `
<summarization_request>${MORE_PROMPT}${(options2 === null || options2 === void 0 ? void 0 : options2.requestShorterOutput) ? SHORTER_OUTPUT_RETRY_PROMPT : ""}</summarization_request>`;
  const serialized = messages.map((m2) => `${m2.role}: ${turnMessageToString(m2, true)}`);
  ...
  if (fullPromptBytes > MAX_PROMPT_BYTES) {
    ...
    if (anticipatedBytesPerChar > currentBytesPerChar * 0.95) {
      ...
      logger29.info(ctx, "[summarization-handler] Reducing prompt budget due to byte limit", {
        summarization: {
          byteAdjustment: {
  ...
  if (fullPrompt.length > promptBudget) {
    logger29.info(ctx, "[summarization-handler] Would truncate summarization prompt", {
      summarization: {
        promptTruncation: {
          totalMessages: messages.length,
          fullPromptLength: fullPrompt.length,
          lastSerializedMessageLength: (_d = (_a20 = serialized.at(-1)) === null || _a20 === void 0 ? void 0 : _a20.length) !== null && _d !== void 0 ? _d : 0,
          didByteAdjust,
          promptBudget
        }
      }
  ...
  if (result === null) {
    logger29.warn(ctx, "[summarization-handler] failed to build summarization prompt", {
      summarization: {
        totalMessages: messages.length,
  ...
  if (joinedResult.length < 500) {
    logger29.warn(ctx, "[summarization-handler] Suspiciously short joined result length for summarization", {
      summarization: {
  ...
  if (finalPromptBytes > MAX_PROMPT_BYTES) {
    logger29.warn(ctx, "[summarization-handler] Final prompt exceeded byte budget", {
      summarization: {
        adjustmentInfo: {
          promptBudget,
  ...
  logger29.info(ctx, "[summarization-handler] Summarization prompt exceeded char budget; truncated", {
    summarization: {
      promptTruncation: {
        totalMessages: messages.length,
...
function executeSummarizationWithRetry(ctx, promptSession, messages, options2) {
  return __awaiter40(this, void 0, void 0, function* () {
    var _a20, _b2, _c2;
    var _d, _e2, _f, _g;
    ...
    for (let attempt = 1; attempt <= MAX_SUMMARIZATION_RETRIES; attempt++) {
      if ((_a20 = options2.cancellationToken) === null || _a20 === void 0 ? void 0 : _a20.cancelled) {
        break;
      }
      attemptsPerformed++;
      try {
        logger29.info(ctx, "[summarization-handler] Attempt started", {
        ...
        let { prompt: summaryPrompt, didTruncate } = buildSummarizationPrompt(ctx, messages, buildPromptOptions);
        if (expectTruncationFromInputReduction && attempt === MAX_SUMMARIZATION_RETRIES) {
          ...
          if (strippedChars > 0) {
            logger29.info(ctx, "[summarization-handler] Stripped non-printable characters from prompt on final input-limit retry", {
              summarization: {
                attempt,
                originalLength,
        ...
        if (expectTruncationFromInputReduction && !didTruncate) {
          logger29.warn(ctx, "[summarization-handler] Expected prompt truncation after input limit reduction but prompt fit within reduced budget", {
            summarization: {
              attempt,
              promptBudget,
              promptLength: summaryPrompt.length,
        ...
        logger29.info(ctx, "[summarization-handler] Attempt succeeded", {
          summarization: Object.assign(Object.assign({}, orchestrationLogFields), {
            attempt,
        ...
        if (retryDirective.reduceInputs) {
          logger29.info(ctx, "[summarization-handler] prompt guard did not reduce inputs enough", {
            summarization: {
              attempt,
              promptBudget,
              lastFailedPromptLength,
        ...
        logger29.error(ctx, "[summarization-handler] Error summarizing messages", error3, {
        ...
        if (retryDirective.requestShorterOutput) {
          ...
          logger29.info(ctx, "[summarization-handler] Retrying with shorter-output instruction", {
        ...
        if (retryDirective.reduceInputs) {
          ...
          logger29.info(ctx, "[summarization-handler] Retrying with reduced input budget", {
            summarization: Object.assign(Object.assign({}, orchestrationLogFields), {
              attempt,
    ...
    if (wasCancelled) {
      logger29.info(ctx, "[summarization-handler] Summarization retries cancelled", {
        summarization: Object.assign(Object.assign({}, orchestrationLogFields), {
      ...
      logger29.warn(ctx, "[summarization-handler] All retries exhausted", {
        summarization: Object.assign(Object.assign({}, orchestrationLogFields), {
          attempts: attemptsPerformed,
          maxRetries: MAX_SUMMARIZATION_RETRIES,
          errorKind,
...
var DEBUG_SUMMARIZATION_STRATEGY_FILE = import_node_path39.default.join(import_node_os12.default.homedir(), "debug-summarization-strategy.txt");
...
function shouldForceSummarizationForTesting(messages, evalCompletionMode) {
  var _a20, _b2;
  ...
  try {
    const strategy = import_node_fs18.default.readFileSync(DEBUG_SUMMARIZATION_STRATEGY_FILE, "utf8").trim();
    if (strategy.startsWith("every-human")) {
      const numHumanMessages = messages.filter((message) => message.role === "user").length;
      const numHumanMessagesLimit = Number.parseInt(strategy.includes(":") ? (_a20 = strategy.split(":")[1]) !== null && _a20 !== void 0 ? _a20 : "1" : "1", 10);
    ...
    if (strategy === "next-human") {
      if (lastMessageIsUser) {
        import_node_fs18.default.rmSync(DEBUG_SUMMARIZATION_STRATEGY_FILE);
      }
      return lastMessageIsUser ? BackgroundSummarizationMode.WaitForCompletion : void 0;
    }
...
var SummarizationHandler = class {
  constructor(promptSession, isFallbackToMainModel = false, options2) {
    ...
    this.enableReduceInputsRetry = (_a20 = options2 === null || options2 === void 0 ? void 0 : options2.enableReduceInputsRetry) !== null && _a20 !== void 0 ? _a20 : false;
  ...
  partitionMessages(messages, options2) {
    ...
    if (preserveLastUser && preservedTailMessages.length === 0 && messagesToSummarize.length > 0) {
      ...
      for (let i = messagesToSummarize.length - 1; i >= 0; i--) {
        ...
        if (m2.role === "user" && ((_b2 = (_a20 = m2.providerOptions) === null || _a20 === void 0 ? void 0 : _a20.cursor) === null || _b2 === void 0 ? void 0 : _b2.isSummary) !== true && !isInjectedReminderMessage(m2)) {
          lastUserIndex = i;
          break;
        }
      }
  ...
  generateSummary(ctx, partitioned, options2) {
    return __awaiter40(this, void 0, void 0, function* () {
      ...
      const result = yield executeSummarizationWithRetry(ctx, this.promptSession, partitioned.messagesToSummarize, {
        ...
        enableRetryOutputTokenLimit: (_a20 = options2.enableRetryOutputTokenLimit) !== null && _a20 !== void 0 ? _a20 : this.alwaysRetryOutputTokenLimit,
  ...
  buildDeterministicFallbackText(ctx, partitioned, options2, errorKind) {
    ...
    if (built === null) {
      logger29.warn(ctx, "[summarization-handler] Deterministic fallback could not fit within budget; falling through to placeholder", {
        summarization: {
          model: metricsModel,
          triggerReason: options2.triggerReason,
          backgroundSummarizationMode: options2.backgroundSummarizationMode,
    ...
    logger29.info(ctx, "[summarization-handler] Deterministic fallback produced summary", {
      summarization: {
        model: metricsModel,
        triggerReason: options2.triggerReason,
        backgroundSummarizationMode: options2.backgroundSummarizationMode,
  ...
  buildSummaryMessage(rawSummary, partitioned, enrichments, options2) {
    ...
    const privacyMode = (_e2 = (_d = (_c2 = options2 === null || options2 === void 0 ? void 0 : options2.privacyMode) !== null && _c2 !== void 0 ? _c2 : (_a20 = partitioned.messagesToSummarize[0]) === null || _a20 === void 0 ? void 0 : _a20._privacyMode) !== null && _d !== void 0 ? _d : (_b2 = partitioned.systemMessage) === null || _b2 === void 0 ? void 0 : _b2._privacyMode) !== null && _e2 !== void 0 ? _e2 : PrivacyMode.UNSPECIFIED;
  ...
  summarize(ctx, messages, options2) {
    return __awaiter40(this, void 0, void 0, function* () {
      ...
      try {
        const pipelineResult = yield runSummarizationPipeline(this, ctx, messages, options2);
        ...
        logger29.error(ctx, "[summarization-handler] Summarize pipeline failed", error3, {
  ...
  recordMetrics(ctx, params) {
    ...
    if (rawSummary.inputTokens !== void 0) {
      ...
      if (rawSummary.inputTokens > INPUT_TOKENS_WARN_THRESHOLD) {
        logger29.warn(ctx, "[summarization-handler] Summarization input tokens are very high", {
          summarization: {
    ...
    if (rawSummary.outputTokens !== void 0) {
      ...
      if (rawSummary.outputTokens > OUT_TOKENS_WARN_THRESHOLD) {
        logger29.warn(ctx, "[summarization-handler] Summarization output tokens are very high", {
          summarization: {
            inputTokens: rawSummary.inputTokens,
            outputTokens: rawSummary.outputTokens,
            durationMs,
  ...
  totalRawSerializedCharLength(messages) {
    let total = 0;
   ...
   */
  static isTokenLimitError(error3) {
    return error3 instanceof InputTokenLimitError;
  }
  /**
   ...
   */
  static isInputOrOutputTokenLimitError(error3) {
    return error3 instanceof InputTokenLimitError || hasErrorName(error3, "InputTokenLimitError") || error3 instanceof OutputTokensLimitExceededError || hasErrorName(error3, "OutputTokensLimitExceededError");
  }
};
