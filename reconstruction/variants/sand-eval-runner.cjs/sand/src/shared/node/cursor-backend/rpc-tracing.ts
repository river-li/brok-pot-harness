...
init_esm();
var import_sdk_trace_node = __toESM(require_src7(), 1);
var UNTRACED_SERVICE_TYPE_NAMES = /* @__PURE__ */ new Set(["aiserver.v1.AnalyticsService"]);
var RPC_ROOT_SAMPLER = new import_sdk_trace_node.AlwaysOnSampler();
var SAND_RPC_TRACE_WINDOW_DURATION_MS = 2 * 60 * 1e3;
var traceWindowExpiry = createExpiryPolicy({
  name: "sand-rpc-trace-window",
  ttlMs: SAND_RPC_TRACE_WINDOW_DURATION_MS
});
var configured;
var activeTraceWindow;
...
function createSandRpcTracingInterceptor() {
  return (next) => async (req) => {
    ...
    const requestId = pinRequestId(req.header);
    ...
    try {
      ...
      const label = (() => {
        ...
        if (error3 instanceof Error) return error3.name;
        return typeof error3;
      })();
