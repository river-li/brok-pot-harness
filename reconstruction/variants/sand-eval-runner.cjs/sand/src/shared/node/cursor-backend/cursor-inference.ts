...
var cachedPrivacyMode;
function privacyLookupErrorLabel(error3) {
  if (error3 instanceof ConnectError) return Code[error3.code];
  return error3 instanceof Error ? error3.name : typeof error3;
}
...
async function settlePrivacyMode(fetchPrivacyMode, options2) {
  try {
    ...
    console.info(
      `[sand:privacy] privacy-mode lookup failed, using privacy-safe fallback backend=${options2.backend.backendUrl} error=${privacyLookupErrorLabel(error3)}`
    );
    return void 0;
  }
}
...
function createSandInferenceInterceptor(options2) {
  return (next) => async (req) => {
    ...
    const requestId = pinnedRequestId != null && pinnedRequestId !== "" ? pinnedRequestId : crypto.randomUUID();
    ...
    try {
      ...
      if (onUnauthenticatedResponse != null && sessionBearer != null && error3 instanceof ConnectError && error3.code === Code.Unauthenticated) {
