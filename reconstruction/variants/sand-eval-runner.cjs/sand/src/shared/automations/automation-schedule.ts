...
function cronIntervalIsAtLeast(matcher, minimumIntervalMs) {
  ...
  for (let dayMs = Date.UTC(2e3, 0, 1); dayMs <= Date.UTC(2400, 0, 1); dayMs += 24 * 60 * MINUTE_MS) {
    const date5 = new Date(dayMs);
    const matches = cronDayMatches(matcher, {
      month: date5.getUTCMonth() + 1,
      dayOfMonth: date5.getUTCDate(),
      dayOfWeek: date5.getUTCDay()
    });
...
function englishCronDays(days) {
  switch (days.kind) {
    ...
    case "yearly-date": {
      const date5 = `${MONTHS[days.month - 1]} ${days.day}`;
      return { lead: `Every ${date5}`, on: ` on ${date5}` };
    }
    case "every-day":
...
function isCalendarDate({ month, day }) {
  const date5 = new Date(2e3, month - 1, day);
  return date5.getMonth() === month - 1 && date5.getDate() === day;
}
...
function describeTrigger(trigger) {
  ...
  const [first, ...rest] = triggerList(trigger);
  return [
