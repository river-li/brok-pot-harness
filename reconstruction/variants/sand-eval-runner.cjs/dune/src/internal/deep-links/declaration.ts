...
function deepLinkRoute(path30, params) {
  const versionMatch = ROUTE_PATH.exec(path30);
  if (versionMatch?.[1] === void 0) {
    throw new DeepLinkDeclarationError(
      `route path "${path30}" must be /v<version>/<segment> with lowercase kebab segments`
    );
  }
  ...
  for (const [name17, validator2] of Object.entries(params)) {
    if (!PARAM_NAME.test(name17) || RESERVED_PARAM_NAMES.has(name17)) {
      throw new DeepLinkDeclarationError(
        `route path "${path30}" has an invalid param name "${name17}"`
      );
    }
    if (typeof validator2?.check !== "function" || typeof validator2.expects !== "string") {
      throw new DeepLinkDeclarationError(`route path "${path30}" param "${name17}" is not a validator`);
    }
  }
