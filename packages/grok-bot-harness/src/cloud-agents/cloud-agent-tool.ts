var SandCloudAgentToolInputError = class extends SandModelVisibleError {
};
...
var cloudAgentParameters = external_exports.object({
  ...
  images: external_exports.array(
    ...
    "Optional image(s) to attach to a launch or reply, such as a screenshot, mock, or chart the cloud agent needs to see. The agent actually sees them (they go through its vision channel), so never paste an image as markdown in the prompt. Pass an absolute file:// URL to a file in your box: a path under /workspace (file:///workspace/shot.png) or one in your own attachments/assets folder (file:///home/box/agent-data/agents/<your id>/attachments/\u2026). https:// is not supported here. Download it to a file first. Describe what each image shows in the prompt itself; there is no caption field. A non-image file goes in files, not here."
  ),
  ...
  repo: external_exports.string().trim().optional().describe(
    `Required for launch unless new_repo is true or environment.type is "environment" (a saved environment supplies its own repos). Full repository URL on an SCM provider connected to the user's Cursor account, or of an existing Cursor Origin repository. Supported providers are GitHub, GitLab, Bitbucket, and Azure DevOps (e.g. https://github.com/owner/repo or https://gitlab.com/group/project). An Origin repository is passed as its https://cursor.com/codebase/owner/repo page or its origin.cursor.com clone URL (Origin needs no separate connection, since the signed-in Cursor account is the Origin account). A bare owner/name is rejected because it does not identify the provider; when the user gives one, look the repository up or ask where it lives, then pass the URL.`
  ),
...
function renderPrLookup(lookup3) {
  ...
  const agentLine = (agent) => {
    ...
    return `${summaryLine(agent)}
  This agent is a thread of ${project2}.`;
  };
...
It runs on ${cloudAgentRuntimeDescription(args.environment)}${modeClause}.${prClause} ${followup}`;
    ...
    case "models": {
      ...
      return [
        `Available cloud-agent models (${catalog.length}). Pass 'model' (id) and optional 'model_params' (a map from param id to value) to launch/reply:`,
...
${note}`;
      ...
      const footers = [
        ...
        ...result.unreachableProviders.length > 0 ? [
          `Could not check ${result.unreachableProviders.join(", ")} (listing failed). Their state is unknown, not necessarily disconnected.`
        ] : []
      ];
    ...
    case "dump": {
      ...
      return [
        ...
        progressNote,
        "Format: JSONL, one JSON message per line, with role, text, reasoning/thinking, tool calls with args, and tool results. Use Shell to grep it or Read to read it.",
...
var DURABLE_WATCH_DESCRIPTION_CLAUSE = " You stay subscribed to the agent afterwards: every later run it does, whether a follow-up from the user, one of its own subscriptions or CI watches, or anything else, revives you again when it finishes, and that revival says whether you started the run. It never expires on its own; it stops only when you call unwatch, or archive or delete the agent. Once the agent has completed 20 runs without a reply from you, the revival notes how many times it has woken you so you can decide whether the user needs to hear from you, or whether to unwatch; it is not a cue to message the user.";
...
function cloudAgentDescription({
  ...
  return [
    ...
    "- rename: retitle an existing agent (agent_id and title). The title is used verbatim and shows on cursor.com, in the IDE sidebar, and on mobile. On an agent you did not launch, rename needs confirm: true.",
