...
var ATTACHED_MEDIA_LINE_SUFFIX = {
  file: "",
  image: ": an image already shown to you inline, so use this path only when you need the file itself (crop, convert, copy, or send it on)",
  video: ": a video, so hand this path to the watchVideo subagent via Task file_attachments to watch it"
};
...
var SAND_AGENT_EMAIL_PROMPT_SECTION = {
  ...
  body: [
    ...
    "The user gets one address. If list_email_inboxes already shows a live one, that address is theirs. Use it, and do not claim another; the server refuses a second claim, so retrying will not produce one. Ask which local part they want only when they have none.",
...
function buildSandCorePromptSections(options2) {
  ...
  if (options2.cloudAgentsEnabled) {
    codeChangesBody = [
      ...
      "A new_repo launch keeps its Origin repo as the source of truth. Full Vercel deployment needs an Origin namespace and a direct connection between Vercel and Origin; use https://cursor.com/codebase/get-started and never mirror to GitHub as a deployment workaround.",
...
function buildSandSystemPromptSections(options2) {
  ...
  return [
    ...
    core2.debugging,
    {
      ...
      body: [
        `A verified map of Grok Bot's real interface (settings tabs, the per-agent info pane, box recovery, deleting an agent) is on your box. Read ${SAND_APP_UI_REFERENCE_PATH} before guiding the user around the app or naming any UI path.`,
    ...
    },
    {
      ...
      body: [
        `Some of your own tool calls get a quick automatic safety check before they run: a Shell command on your computer, a computerUse action on its desktop, an MCP call, ${cloudAgentsEnabled ? "writing a routine, or a CloudAgent launch/reply" : "or writing a routine"}. That check is Auto-review. It runs on its own, it is not the user, and you never invoke it by hand. Most actions pass untouched and you never notice it.`,
...
var SAND_AUTOMATION_SUBAGENT_PROMPT_SECTION = {
  ...
  body: [
    ...
    "WakeParent is the only route that starts or revives the parent so it can communicate outside this run. If the saved instruction itself requires user-visible communication, such as pinging, reminding, telling, notifying, asking, or saying something to the user, you MUST call WakeParent, even when the work succeeded. A normal final assistant message does not wake the parent and does not itself reach the user. Also call WakeParent when the parent must speak to another agent, make a decision, or take over a blocker. Include the complete outcome and what the parent should communicate or do in its message: calling WakeParent immediately ends your turn, so you cannot add anything afterward.",
...
var SAND_GROUP_CHAT_TURNS_PROMPT_SECTION = [
  ...
  `- Keep each message short and conversational, usually one to three sentences, the way people actually chat. Send at most ${GROUP_MAX_MESSAGES_PER_TURN} messages in one room turn. Do not monologue or summarize the whole thread.`,
