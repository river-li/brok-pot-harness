...
function mergeUserMemoryShards(shards, limits) {
  const tag = (records2, via) => records2.map((record2) => ({ ...record2, via }));
  const profile = shards.flatMap((shard) => tag(shard.recall.profile, shard.via));
  const recent = shards.flatMap((shard) => tag(shard.recall.recent, shard.via));
  return {
...
function mergeUserMemoryShardRecords(shards) {
  const tagged = shards.flatMap(
    (shard) => shard.records.map((record2) => ({ ...record2, via: shard.via }))
  );
...
var MEMORY_PROJECT_PROFILE_PROMPT_LIMIT = 25;
var MEMORY_PROJECT_PROFILE_CHAR_BUDGET = 2500;
var MEMORY_PROJECT_RECENT_PROMPT_LIMIT = 10;
var MEMORY_PROJECT_RECENT_CHAR_BUDGET = 1500;
var MEMORY_PROJECT_INJECTED_CAP = 3;
function projectMemoryActivity(recall) {
  let newest = 0;
...
function selectProjectMemoryBlocks(blocks, injectedCap) {
...
function hasProjectFacts(block) {
  return block.recall.profile.length > 0 || block.recall.recent.length > 0;
}
function projectMemoryHasFacts(recall) {
  return recall.injected.some(hasProjectFacts);
}
function renderProjectMemorySystemPrompt(recall, ctx) {
  ...
  for (const block of recall.injected) {
    const header = block.ownShardDir != null ? `Project "${block.name}" (${block.slug}) \u2014 your shard: ${block.ownShardDir}:` : `Project "${block.name}" (${block.slug}):`;
    lines2.push(header);
  ...
  if (recall.alsoMemberOf.length > 0) {
    const pointers = recall.alsoMemberOf.map((pointer) => `${pointer.name} (${pointer.slug})`).join(", ");
    lines2.push(`Also a member of: ${pointers} \u2014 grep those project folders for their memory.`);
  }
  return lines2.join("\n");
}
