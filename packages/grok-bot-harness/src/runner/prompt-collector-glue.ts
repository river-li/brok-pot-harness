...
function createPromptCollectorGlue(host) {
  ...
  function getComputerSection(skillify = false) {
    ...
    if (host.isComputerUseSubagent) {
      ...
      return [
        ...
        "- Don't loop, and know when to stop. If the same approach hasn't moved you forward after a couple of tries, change approach: scroll to find the element, reload the page, or take a different route. The moment the goal is met, or you hit something you can't get past, end the turn and report rather than poking at a finished or blocked screen.",
        `- You can't talk to the user or hand off the box. If a step needs a human, such as a password, 2FA, a puzzle or image captcha, or a payment, stop and say so clearly in your final report (name the site/step) so the parent can hand them the box; never try to enter their credentials. Exception: when your task says 1Password fills the one-time code for this login, a verification-code page is not a human step. The code is filled and submitted for you, so wait for the page to move on and continue. If it is still asking for a code after about a minute, report that so the parent can hand off. A press-and-hold "I'm human" button is not a human step. Use Computer click with holdDurationMs, holding until the widget completes (the page rarely says how long, so start near 8000 and hold longer, up to 30000, when it asks you to try again), then check the screenshot.`,
    ...
    if (host.isBrowserUseSubagent) {
      return [
        ...
        BOX_DRIVER_PRIVACY_LINE,
        "- Don't loop, and know when to stop. If the same approach hasn't moved you forward after a couple of tries, change approach: scroll to find the element, reload the page, or take a different route. The moment the goal is met, or you hit something you can't get past, end the turn and report rather than poking at a finished or blocked page.",
        `- You can't talk to the user or hand off the box. If a step needs a human, such as a password, 2FA, a puzzle or image captcha, or a payment, stop and say so clearly in your final report (name the site/step) so the parent can hand them the box; never try to enter their credentials. Exception: when your task says 1Password fills the one-time code for this login, a verification-code page is not a human step. The code is filled and submitted for you, so wait for the page to move on and continue. If it is still asking for a code after about a minute, report that so the parent can hand off. A press-and-hold "I'm human" button is not a human step. Use browser_click with holdDurationMs, holding until the widget completes (the page rarely says how long, so start near 8000 and hold longer, up to 30000, when it asks you to try again), then check the result.`,
        BOX_DRIVER_NARRATION_LINE,
        BOX_DRIVER_FINAL_REPORT_LINE
      ].join("\n");
    }
  ...
  async function assembleTurnAction(args) {
    ...
    const unansweredQuestionsNote = buildUnansweredQuestionsNote({
      ...
      isComposerAnswerPath: options2.hidden !== true && host.gates.widgetV0(),
