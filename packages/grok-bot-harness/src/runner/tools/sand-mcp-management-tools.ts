  ...
  return {
    ...
    setInstructionsParameters: external_exports.object({
      ...
      instructions: external_exports.string().describe(
        `The custom instructions to follow whenever you use this connector. They say how the user wants it used (e.g. "Reply in threads on Slack."). Pass an empty string to clear them and fall back to the connector's default.`
      )
    }),
...
function describeInstalled(server) {
  ...
  if (server.pluginId != null) {
    parts.push(
      `plugin=${server.pluginId} (remove via UninstallPlugin, which removes the whole plugin)`
    );
  }
...
function describePluginFields(detail) {
  ...
  const lines2 = detail.fields.map((field) => {
    const flags = [
      ...
      ...field.isSecret ? ["secret, so ask the user and never guess"] : []
    ].join(", ");
...
function describePluginDetail(detail) {
  ...
  if (detail.servers.length > 0) {
    sections.push(
      [
        "Its installed MCP server(s), with statuses in GetMcpServerStatus:",
        ...detail.servers.map(describeInstalled)
      ].join("\n")
    );
  }
  if (detail.isInstalled && detail.installMode === "team-required") {
    sections.push("It is required by the user's team, so it cannot be uninstalled.");
  }
...
function emitAndDescribeAuthResult(result, isForceReauth, serverId, emitConnectorCard) {
  ...
  switch (result.kind) {
    ...
    case "unreachable":
      return `Sign-in for "${name17}" never started. The server or its configuration failed the check: "${result.message}". That is not a missing credential, so the user authenticating in Settings would hit the same error. Tell them what it reported instead of sending them to Settings.${isForceReauth ? "" : ` If "${name17}" used to work and its auth now seems stuck, signing out and back in often clears errors like this: ask the user whether to sign "${name17}" out and start over, and only then call this again with force_reauth: true.`}`;
    default: {
...
var PERSONAL_MCP_MANAGEMENT_COPY = {
  ...
  uninstallServerDescription: (multiAccount) => "Remove ONE custom MCP server by its server identifier. A custom server is one added with AddMcpServer, not one that came from a plugin. This is destructive and deletes the server with all of its accounts, so confirm with the user via a question widget first. A server the listing marks `plugin=<id>` came from a marketplace plugin. Removing it would uninstall that WHOLE plugin, which this tool refuses. Use UninstallPlugin for those so the confirmation can disclose the full scope." + (multiAccount ? " To remove just one account and keep the server, use RemoveMcpAccount instead." : ""),
  ...
  return [
    ...
    defineCommunicateTool(management, {
      ...
      execute: guardMutation(async (_ctx, args, deps) => {
        ...
        if (row.isTeamServer) {
          return `${row.name} is provided by the user's team, so it can't be removed here. A team admin manages it. The user can hide it in Settings under Plugins.`;
        }
        ...
        return [
          `The removal request for ${row.name} completed, but it still reads as installed. Re-check with GetMcpServerStatus; the user can also remove it in Settings under Plugins.`,
          describeInstalledList(result.servers)
        ].join("\n");
      })
    }),
    defineCommunicateTool(management, {
      ...
      execute: guardMutation(async (_ctx, args, deps) => {
        ...
        return `The uninstall request for ${detail.displayName} completed, but it still reads as installed. Re-check with GetPlugin; the user can also remove it in Settings under Plugins.`;
      })
    }),
    defineCommunicateTool(management, {
      id: "GET_MCP_SERVER_STATUS",
      name: "GetMcpServerStatus",
      description: `The runtime status of the user's installed MCP servers (connected / needsAuth / needsGrant / error, per account). needsGrant means the user already connected it but has not granted it this turn: call AuthenticateMcpServer for it and nothing else; never treat it as missing, never install anything, and never open the browser for the same service. Pass server_id (the server identifier, NEVER a display name) for one server; omit it to list everything. Use this to see which connectors still need authentication, to find the identifier a lifecycle tool needs, or to check a connector after installing or authenticating. That identifier is the same one ${mcpMetaToolNames.discovery} and ${mcpMetaToolNames.invocation} address. Read-only and never needs the user's permission.`,
    ...
    defineCommunicateTool(management, {
      ...
      description: "Restart (reconnect) the installed MCP servers. This is useful when a server is stuck, errored, or you just finished authenticating one. Confirm with the user first if a server is mid-task.",
