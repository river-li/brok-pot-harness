...
function createSortMemoriesTool(deps) {
  return defineCommunicateTool(deps, {
    ...
    execute: async (_ctx, args, d) => {
      ...
      const refused2 = promoted.refused + demoted.refused;
      const duplicated = promoted.duplicated + demoted.duplicated;
      const settled = promote.settled.length + demote.settled.length;
      const failure2 = promoted.failure ?? demoted.failure;
      return `Moved ${promoted.moved} fact${promoted.moved === 1 ? "" : "s"} to team memory and ${demoted.moved} back to private memory${settled > 0 ? `; ${settled} had already moved` : ""}${refused2 > 0 ? `; ${refused2} could not be written and stayed put` : ""}${duplicated > 0 ? `; ${duplicated} still also show where they were until that removal is retried` : ""}${failure2 === void 0 ? "" : ` (${failure2})`}. ${refused2 + duplicated > 0 ? "Tell the owner in one line which facts did not settle." : "The card already told the owner, so send no message about the memories."}`;
    }
  });
}
