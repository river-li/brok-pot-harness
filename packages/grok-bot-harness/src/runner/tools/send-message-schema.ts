...
var sendMessageObjectSchemaWithCredentialRequest = external_exports.object({
  ...
  channel: external_exports.string().trim().optional().describe(
    "Optional. A connected messaging channel address to deliver this to instead of the in-app Grok Bot chat, shaped platform:chat, the address shown to you in an [inbound] wake. On Slack a channel thread is slack:<channel id>:<thread ts> (never a bare channel id, which is refused) and a person is slack:<user id>. Omit to send to the in-app chat (the default). Only valid with type:text or type:attachment."
  ),
