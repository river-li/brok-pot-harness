...
var pickReason = external_exports.string().trim().max(REASON_MAX_CHARS).describe(
...
var secretPick = external_exports.object({
  name: pickName.describe("The secret's name, exactly as listed in the inventory."),
  reason: pickReason
});
...
var secretPickList = external_exports.array(secretPick).max(PICK_MAX).default([]);
...
function carryOverProposalOf(args) {
  ...
  const reasons = {};
  for (const pick2 of [...secrets, ...plugins, ...suggestions]) {
    if (pick2.reason.length > 0) reasons[pick2.name] = pick2.reason;
  }
  return {
    secrets: secrets.map((pick2) => pick2.name),
