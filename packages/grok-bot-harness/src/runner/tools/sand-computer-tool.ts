...
function reportComputerAction(onComputerAction, sequence) {
  ...
  for (const args of sequence) {
    const reported2 = toReportedAction(args);
    if (reported2 !== void 0) onComputerAction(reported2);
  }
}
function describeOutcome(result, operation) {
  ...
  if (success2.cursorPosition != null) {
    lines2.push(`Cursor is at (${success2.cursorPosition.x}, ${success2.cursorPosition.y}).`);
  }
  return lines2.join("\n");
}
