...
function buildCredentialResolvedAck(args) {
  const { request: request3, resolution, detail } = args;
  switch (resolution) {
    case "approved":
      return [
        `[1Password filled the login into the sign-in page at ${request3.targetSite}; no credential value entered this conversation.]`,
    ...
    case "denied":
      return `[The user declined to fill the 1Password login ${request3.credentialId}. Do not send that credential-request again right away; continue without it or ask the user how they'd like to proceed.]`;
    case "failed":
