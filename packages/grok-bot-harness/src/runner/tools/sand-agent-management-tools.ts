...
var sendToAgentParameters = external_exports.object({
  ...
  images: external_exports.array(
    ...
    "Optional image(s) to send with the message, such as a screenshot, chart, or photo the other agent needs. They are delivered with your message. A 1:1 recipient actually sees them (like an image the user sends), and they render with your text in the exchange. They are not delivered to groups."
  ),
...
function createCreateAgentTool(management) {
  return defineCommunicateTool(management, {
    ...
    description: `Create a new agent (a new teammate assistant) for your user, with a name, an optional persona/description, and an optional sidebar section id from ${SAND_LIST_SECTIONS_TOOL_NAME}. Returns the new agent's id so you can immediately message it with SendToAgent. Use this to spin up a focused teammate for a job. You have no tool to delete an agent, so only create one when it is genuinely useful; the user can delete an agent themselves from the sidebar (right-click the agent, then "Delete").`,
