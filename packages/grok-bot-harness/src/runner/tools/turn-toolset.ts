...
var SAND_WEB_FETCH_DESCRIPTION_SUFFIX = process.env.GROKBOT_LOCAL_MODE === "1" ? `Some sites block automated HTTP clients with a 403 or a block page (e.g. "Request Rejected"). That is not evidence the page does not exist. A browser subagent can render pages that require JavaScript or browser interaction.` : `Some sites' firewalls block this tool's fetch provider with a 403 or a block page (e.g. "Request Rejected"). That is not evidence the page does not exist: fall back to a browser subagent when one is available, or \`curl\` via Shell (your box egresses from a different network).`;
...
${descriptionSuffix}`,
    execute: async (ctx, interactionHandler, argsStream, meta) => {
      ...
      try {
        ...
        targetsUserComputer = parsed2 !== null && typeof parsed2 === "object" && !Array.isArray(parsed2) && Object.prototype.hasOwnProperty.call(parsed2, "machineId");
      } catch {
...
function buildTurnTools(host, turn, props) {
  ...
  if (hasUserFacingChat) {
    tools.push(
      createSendMessageTool2({
        ...
        onSendMessage: (message, timestampMs2, deliverTo) => {
          host.emitUpdate(
            {
              type: "send-message",
              message,
  ...
  if (hasParentToolParity && !host.isParentMediatedAutomationSubagent) {
    tools.push(
      createReactToMessageTool({
        react: ({ messageAddress, emoji: emoji3 }) => host.emitUpdate({ type: "react-to-message", messageAddress, emoji: emoji3 }, updateObservers),
    ...
    tools.push(
      createSandStateTool({
        ...
        ...host.isParentMediatedAutomationSubagent ? {} : {
          onListenerAutomationSaved: (trigger2) => surfaceListenerConnectCards({
            trigger: trigger2,
  ...
  const messages2 = host.messages;
  if (!host.isSubagentRunner && host.gates.messagesTools() && messages2 !== void 0 && messages2.enabled() === true) {
    tools.push(
      ...createMessagesTools({
        messages: messages2,
  ...
  if (hasParentToolParity && !host.isParentMediatedAutomationSubagent && host.remoteBoxHasDesktop && host.getRemoteBoxAvailable() && host.boxHandoff != null) {
    ...
    tools.push(
      createRequestBoxHelpTool({
        ...
        onSendMessage: (message, timestampMs2, boxHandoffStamp) => host.emitUpdate(
          {
            type: "send-message",
            message,
            timestampMs: timestampMs2,
  ...
  const emitCard = (message, timestampMs2) => {
    host.emitUpdate(
      {
        type: "send-message",
        message,
