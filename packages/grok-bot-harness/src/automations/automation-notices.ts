...
var fiveMinuteAutomationFloorNotice = {
  ...
  lines: [
    `NOTICE ${SAND_FIVE_MINUTE_AUTOMATION_FLOOR_NOTICE_ID} (raised once for this routine, and only here, so tell the user now): the minimum routine interval is now 5 minutes. This routine's old cadence was below that floor, so the app changed it to run every 5 minutes. Send the user a plain message naming this routine and explaining those three facts. Do not change the schedule back below 5 minutes.`
  ]
};
var SAND_AUTOMATION_NOTICES = [
  githubListenerScopeNotice,
  fiveMinuteAutomationFloorNotice
];
function isAutomationNoticeId(value) {
  return SAND_AUTOMATION_NOTICES.some((notice) => notice.id === value);
}
function automationNoticesToRaise(automation) {
  const raised = automation.raisedNotices ?? [];
  return SAND_AUTOMATION_NOTICES.filter(
    (notice) => notice.applies(automation) && !raised.includes(notice.id)
  );
}
