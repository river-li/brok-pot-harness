...
var CoreMessageSerde = class {
  serialize(value) {
    const json3 = JSON.stringify(value, jsonReplacer);
    return textEncoder.encode(json3);
  }
  deserialize(blob) {
    const json3 = textDecoder.decode(blob);
    if (json3.includes(SERIALIZED_UINT8_ARRAY_MARKER)) {
      return JSON.parse(json3, jsonReviver);
    }
    ...
    for (const blobId of blobIds) {
      ...
      if (blob) {
        if (blob.length > OVERSIZE_TRANSCRIPT_BLOB_THRESHOLD_BYTES) {
          messages2.push(createOversizeBlobOmittedMessage(blob.length));
...
function formatToolArgs(args) {
  ...
  const lines2 = entries.map(([key, value]) => {
    let valueStr;
...
function stripXmlTags(text2) {
  return stripContextTags(text2);
}
function stripHiddenThinkingTags(text2) {
  return text2.replace(/<think>[\s\S]*?<\/think>/gi, "").replace(/<thinking>[\s\S]*?<\/thinking>/gi, "").replace(/\n{3,}/g, "\n\n").trim();
}
...
function formatTranscript(messages2) {
  let filteredMessages = messages2.filter((msg) => msg.role !== "system");
...
function formatMessageToJson(message, stripTags2) {
  ...
  if (typeof content === "string") {
    ...
    if (maybeStrippedThinking.trim()) {
      json3.text = maybeStrippedThinking;
    }
    ...
    if (textParts.length > 0) {
      json3.text = textParts.join("\n");
    }
    if (thinkingParts.length > 0) {
      json3.thinking = thinkingParts.join("\n");
    }
    if (toolCalls3.length > 0) {
      json3.toolCalls = toolCalls3;
    }
    if (toolResult) {
      json3.toolResult = toolResult;
    }
  }
  return json3;
}
function formatTranscriptJson(messages2) {
  let filteredMessages = messages2.filter((msg) => msg.role !== "system");
  if (filteredMessages.length >= 2 && filteredMessages[0].role === "user" && filteredMessages[1].role === "user") {
    filteredMessages = filteredMessages.slice(1);
  }
...
function getTranscriptTerminalMarkers(options2) {
  const { turnEnded } = options2;
  const textSuffixes = [];
  const jsonFields = {};
  const jsonlLines = [];
  if (turnEnded !== void 0) {
...
function formatTranscriptJsonl(messages2) {
  let filteredMessages = messages2.filter((msg) => msg.role !== "system");
  if (filteredMessages.length >= 2 && filteredMessages[0].role === "user" && filteredMessages[1].role === "user") {
    filteredMessages = filteredMessages.slice(1);
  }
...
var TranscriptStore = class {
  constructor(projectDir, blobStore, writeFile19, options2 = {}) {
    var _a19, _b2, _c2, _d;
    this.projectDir = projectDir;
    this.blobStore = blobStore;
    this.writeFile = writeFile19;
    this.options = {
      writeText: (_a19 = options2.writeText) !== null && _a19 !== void 0 ? _a19 : true,
      writeJson: (_b2 = options2.writeJson) !== null && _b2 !== void 0 ? _b2 : false,
      writeJsonl: (_c2 = options2.writeJsonl) !== null && _c2 !== void 0 ? _c2 : false,
      pathResolver: options2.pathResolver,
      appendFile: options2.appendFile,
      fallbackToFullWriteOnIncrementalFailure: (_d = options2.fallbackToFullWriteOnIncrementalFailure) !== null && _d !== void 0 ? _d : true
    };
  }
  /**
   ...
   */
  resolveFilePath(conversationId, ext2) {
    if (this.options.pathResolver) {
      return this.options.pathResolver(conversationId, ext2);
    }
  ...
  writeFromStateFull(ctx_1, state_1, conversationId_1) {
    return __awaiter25(this, arguments, void 0, function* (ctx, state, conversationId, options2 = {}) {
      ...
      try {
        ...
        try {
          const messages2 = yield hydrateMessages(span.ctx, this.blobStore, state);
          const { overviewFactory } = options2;
          const terminalMarkers = getTranscriptTerminalMarkers(options2);
          if (messages2.length === 0 && terminalMarkers.jsonlLines.length === 0) {
            return true;
          }
          let formattedConversation;
          if (this.options.writeText || overviewFactory) {
            formattedConversation = formatTranscript(messages2);
          }
...
${terminalMarkers.textSuffixes.join("\n")}` : terminalMarkers.textSuffixes.join("\n") : base;
          ...
          if (this.options.writeJson) {
            const jsonContent = formatTranscriptJson(messages2);
          ...
          if (this.options.writeJsonl) {
            const body = formatTranscriptJsonl(messages2);
  ...
  writeFromStateIncremental(ctx_1, state_1, conversationId_1, previousRootPromptCount_1) {
    return __awaiter25(this, arguments, void 0, function* (ctx, state, conversationId, previousRootPromptCount, options2 = {}) {
      ...
      try {
        ...
        if (currentCount === previousRootPromptCount && !hasTerminalMarker && !options2.overviewFactory) {
          return currentCount;
        }
        const appendFile3 = this.options.appendFile;
        const canAppend = appendFile3 && this.options.writeJsonl && !this.options.writeText && !this.options.writeJson && previousRootPromptCount > 0 && currentCount >= previousRootPromptCount && (currentCount > previousRootPromptCount || hasTerminalMarker) && !options2.overviewFactory;
        if (!canAppend) {
          const ok = yield this.writeFromStateFull(ctx, state, conversationId, options2);
        ...
        try {
          ...
          lines2.push(...terminalMarkers.jsonlLines);
          if (lines2.length > 0) {
            const content = ensureTrailingNewline(lines2.join("\n"));
