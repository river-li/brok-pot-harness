...
function convertCoreMessagesToLoopFormat(messages2) {
  const result = [];
...
function collectRunOccurrenceResults(params) {
  const { keys, messages: messages2, patternStartIdx, patternLength, normalize: normalize5 } = params;
  ...
  for (let start = patternStartIdx; start + patternLength <= keys.length; start += patternLength) {
    ...
    const texts = [];
    for (let offset = 0; offset < patternLength; offset++) {
      for (const text2 of messages2[start + offset].toolResultTexts ?? []) {
...
function checkForAgentSingleMessageLooping(params) {
  ...
  if (!loopDetected && params.detector.timedOut()) {
    ...
    logger9.info(params.ctx, "NAL single-message loop detection timed out; failing open", {
      caller: params.caller,
      channel,
  ...
  logger9.warn(params.ctx, "NAL single-message looping detected", {
...
function checkForAgentOutboundMessageFlood(params, reporting) {
  ...
  if (params.ctx) {
    logger9.warn(params.ctx, "NAL outbound message flood detected", {
      loopKind,
...
function checkForAgentMessageLooping(params, reporting) {
  ...
  if (params.ctx) {
    logger9.warn(params.ctx, "NAL assistant message looping detected", {
      loopKind,
      isLoopReoccurrence,
