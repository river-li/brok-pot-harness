...
function isLoopbackIpHost(host) {
  const hostname3 = host.toLowerCase();
  const ipVersion = (0, import_node_net3.isIP)(hostname3);
  if (ipVersion === 4)
    return isLoopbackIpv4(hostname3);
  if (ipVersion === 6) {
    if (hostname3 === "::1")
      return true;
    const mapped = extractIpv4FromMappedIpv6(hostname3);
...
function isPrivateIpHost(host) {
  ...
  const ipVersion = (0, import_node_net3.isIP)(hostname3);
  if (ipVersion === 4)
    return isPrivateIpv4(hostname3);
  if (ipVersion === 6)
    return isPrivateIpv6(hostname3);
  return false;
}
