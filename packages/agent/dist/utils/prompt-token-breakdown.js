...
var CACHEABLE_CATEGORY_IDS = PROMPT_TOKEN_BREAKDOWN_CATEGORIES.map((c) => c.id).filter((id) => id !== "conversation");
function buildPromptTokenBreakdownSnapshot(ctx, params) {
...
function buildSummarizeRefreshSnapshot(params) {
...
function collectAllCategorySources(messages2, tools, descriptionProps) {
  const { tools: toolSegments, subagents: subagentSegments } = collectToolAndSubagentSegments(tools, descriptionProps);
  return {
    system_prompt: collectSystemPromptSegments(messages2),
    tools: toolSegments,
    rules: collectRulesSegments(messages2),
    skills: collectSkillsSegments(messages2),
    mcp: collectMcpPromptBlockSegments(messages2),
    subagents: [
      ...subagentSegments,
      ...collectAvailableSubagentModelsSegments(messages2),
      ...collectAvailableSubagentTypesSegments(messages2)
    ],
    summarized_conversation: collectSummarizedConversationSegments(messages2),
    conversation: collectConversationSegments(messages2)
  };
}
function collectSystemPromptSegments(messages2) {
  const segments = [];
...
function collectRulesSegments(messages2) {
  const initialUserMessage = findInitialUserPromptMessage(messages2);
...
function collectSkillsSegments(messages2) {
  const initialUserMessage = findInitialUserPromptMessage(messages2);
  if (initialUserMessage === void 0) {
    return [];
  }
...
function collectAvailableSubagentModelsSegments(messages2) {
  const initialUserMessage = findInitialUserPromptMessage(messages2);
  if (initialUserMessage === void 0) {
    return [];
  }
...
function collectAvailableSubagentTypesSegments(messages2) {
  const initialUserMessage = findInitialUserPromptMessage(messages2);
  if (initialUserMessage === void 0) {
    return [];
  }
...
function collectMcpPromptBlockSegments(messages2) {
  const segments = [];
  for (const message of messages2) {
...
var INITIAL_PROMPT_SCAFFOLD_TAGS_NON_MCP = [
  "user_info",
  "rules",
  "agent_skills",
...
function collectConversationSegments(messages2) {
  const segments = [];
...
function collectSummarizedConversationSegments(messages2) {
  const segments = [];
  for (const message of messages2) {
    if (!isSummaryCarrierMessage(message)) {
      continue;
...
function resolveCachedCategory(characterCount, cached2, avgRatio) {
  if (characterCount === 0) {
    return { characterCount: 0, estimatedTokens: 0 };
  }
  if (cached2?.characterCount !== void 0 && cached2.characterCount > 0) {
    if (cached2.characterCount === characterCount) {
      return { characterCount, estimatedTokens: cached2.estimatedTokens };
    }
    const scaled = Math.round(cached2.estimatedTokens * (characterCount / cached2.characterCount));
...
function findInitialUserPromptMessage(messages2) {
  return messages2.find((message) => message.role === "user" && !isSummaryCarrierMessage(message));
}
var promptTokenBreakdownCategoryEstimatedTokens = createHistogram("agent.prompt_token_breakdown.category_estimated_tokens", {
