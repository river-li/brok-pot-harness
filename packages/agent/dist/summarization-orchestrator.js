...
var SummarizationOrchestrator = class {
  ...
  shouldStartBackgroundSummarization(tokenDetails, messages2, ctx) {
    ...
    const selfResult = this.selfSummarizerFactory && (this.canUseSelfSummaryNow?.() ?? true) && shouldPerformSelfSummary(messages2, tokenDetails, ctx);
    if (bgResult)
      return true;
    if (selfResult)
      return true;
  ...
  async handleSummarization(parentCtx, stateHandler, rootPromptExecutor, interactionListener, config2, requestContext, options2) {
    ...
    try {
      const spanCtxt = __addDisposableResource21(env_1, createSpan(parentCtx.withName("handleSummarization")), false);
...
${formatProjectCompactionPrompt({
      ...
      if (config2.enableExecuteHookExec && isHookStepConfigured(requestContext.hooksConfig?.configuredSteps, "preCompact")) {
        try {
          ...
          try {
            const span = __addDisposableResource21(env_2, createSpan(ctx.withName("agent.lifecycleHook.preCompact")), false);
            ...
            if (remoteHookExecutor) {
              ...
              if (result.response?.response.case === "preCompact") {
                const userMessage2 = result.response.response.value.userMessage;
                if (userMessage2) {
                  hookMessage = userMessage2;
