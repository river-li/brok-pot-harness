...
Command: ${result.value.command}` : `Error: Command failed to spawn: ${result.value.error}`;
  ...
  switch (result.case) {
    ...
    case "failure": {
      ...
      const abortReason3 = result.case === "failure" ? result.value.abortReason : void 0;
      const aborted2 = result.case === "failure" && result.value.aborted;
      const wasTimeout = abortReason3 === ShellAbortReason.TIMEOUT;
      const wasAborted = abortReason3 === ShellAbortReason.USER_ABORT || aborted2 && abortReason3 === void 0;
      if (wasTimeout) {
