...
function buildOutputNotificationConfig(rawArgs, ctx, toolCallId, notificationLimit) {
  ...
  if (pattern.length > MAX_OUTPUT_NOTIFICATION_PATTERN_CHARS) {
    throw rejectConfig("bad_pattern", `notify_on_output.pattern must be ${MAX_OUTPUT_NOTIFICATION_PATTERN_CHARS} characters or fewer`);
  }
  try {
...
var createShellTool = (resourceAccessor, options2, promptVersion = "latest") => {
  ...
  const defaultTimeoutMsForBlockUntil = isBackgroundAgentForBlockUntil ? BACKGROUND_SHELL_DEFAULT_BLOCK_UNTIL_MS : DEFAULT_TIMEOUT_MS2;
  ...
  const execute = async (parentCtx, interactionHandler, rawArgs, meta) => {
    ...
    try {
      const spanCtxt = __addDisposableResource33(env_1, createSpan(parentCtx.withName("shellExecute")), false);
      ...
      const requestId2 = getRequestId(spanCtxt.ctx);
      ...
      if (options2?.sandboxEnabled) {
        ...
        const set3 = new Set(perms);
        const requestsAll = set3.has("all");
        const wantsNetwork = set3.has("full_network") || set3.has("network");
        ...
        execResult = await interactionHandler.executeToolCall(spanCtxt.ctx, createShellToolCall(baseToolCall), meta.toolCallId, async (ctx) => {
          ...
          for await (const shellStream of wrappedShellStream()) {
            if (shellStream.event.case === "stdout") {
              ...
              abortReason3 = shellStream.event.value.abortReason;
              await streamingHandler.handleShellEvent(spanCtxt.ctx, {
                type: "exit",
                code: finalExitCode
              });
  ...
  const executeWithExclusiveLock = async (ctx, interactionHandler, argsStream, meta) => {
    ...
    try {
      ...
      const _lockHandle = __addDisposableResource33(env_2, await fileOperationLockManager.waitForExclusiveLock(ctx), false);
