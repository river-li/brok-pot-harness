...
function stripDsv3TokensFromArgs(opts) {
  ...
  for (const token of DSV3_TOOL_TOKENS_TO_STRIP) {
    if (result.includes(token)) {
      logger16.warn(opts.ctx, "nal.tool_args.dsv3_token_leaked", {
...
function createZodAgentTool(toolIdentifier, tool) {
  const fullSchema = esm_default2(tool.parameters);
  ...
  return {
    ...
    execute: async (parentCtx, interactionHandler, argsStream, meta) => {
      ...
      try {
        const spanCtxt = __addDisposableResource4(env_1, createSpan(parentCtx.withName("execute")), false);
...
var withSafeParsedArgs = (parametersSchema29, execute, initialToolCall, options2) => {
  return async (ctx, interactionHandler, argsStream, meta) => {
    ...
    try {
      ...
      const coreExecuteSpan = __addDisposableResource4(env_2, createSpan(ctx.withName("coreExecute")), false);
...
function wrapToolWithTimeout(tool, options2) {
  ...
  return {
    ...
    execute: async (parentCtx, interactionHandler, argsStream, meta) => {
      ...
      let suspendCount = 0;
      const arm = () => {
        if (finished || suspendCount > 0 || timer !== void 0) {
          return;
        }
        armedAtMs = Date.now();
        timer = setTimeout(() => {
