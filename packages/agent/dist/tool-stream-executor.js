...
function collectRawErrorMessages(error42) {
  ...
  while (queue.length > 0) {
    ...
    if (current instanceof Error) {
      if (current.message.length > 0) {
        messages2.push(truncateOutput(current.message, RAW_ERROR_MESSAGE_CHARACTER_BUDGET, true).output);
      }
  ...
  return Array.from(new Set(messages2));
}
...
var ToolCallStream = class {
  constructor(toolCallId, toolName, inner) {
    ...
    this.completedArgs = new Promise((resolve29) => {
      this.resolveCompletedArgs = resolve29;
    });
    this.completedArgsText = new Promise((resolve29) => {
      this.resolveCompletedArgsText = resolve29;
    });
  }
...
async function executeDeferredToolCall(ctx, descriptor2, toolMap, interactionHandler, extraT, recordToolCallResult, renderProps, streamingArgsIterable, completedArgs, directDynamicToolNames = /* @__PURE__ */ new Set()) {
  ...
  try {
    ...
    const isIntentionalAbort = ctx.canceled && abortReason3?.intentional === true;
...
function hasMeaningfulResponseMessageContent(messages2) {
  return messages2.some((message) => {
...
function responseOmitsStreamedToolCall(messages2, contentBuffer) {
  const responseToolCallIds = new Set(messages2.flatMap((message) => message.role === "assistant" && Array.isArray(message.content) ? message.content.flatMap((part) => part.type === "tool-call" ? [part.toolCallId] : []) : []));
...
function executeToolStream(ctx, executor, interactionHandler, tools, extraT, recordToolCallResult, descriptionProps, firstToolCallHook) {
  ...
  const responsePromise = (async () => {
    const toolPromises = [];
    const trackToolPromise = (promise2) => {
      promise2.catch(() => void 0);
      toolPromises.push(promise2);
    };
...
var BaseRedactedPromptBuilder = class {
  ...
  appendMessages(messages2) {
    const arr = Array.isArray(messages2) ? messages2 : [messages2];
