...
function buildInterruptedPendingToolCallMessages(stateHandler, tools, interruptedPendingToolCallResolutions, terminalsFolder) {
  ...
  for (const parsed2 of parsedMessages) {
    if (completedMessages.has(parsed2)) {
      continue;
    }
    messagesToAppend.push(parsed2);
    if (parsed2.role !== "assistant" || !Array.isArray(parsed2.content)) {
      continue;
    }
    const elapsedMs3 = getPendingToolCallElapsedMs(parsed2);
    let toolResults = [];
    ...
    for (const content of parsed2.content) {
...
var UserMessageActionHandler = class extends AbstractUserMessageActionHandler {
  async initializeConversation(ctx, params) {
    ...
    const userMessage2 = action.userMessage;
    const isRootProject = isRootProjectUserMessage(userMessage2);
    if (isRootProject) {
      stateHandler.isRootProjectConversation = true;
    }
    const resolvedTurnMode = stateHandler.resolveTurnMode(userMessage2);
    ...
    const isProjectKickoffBeforePrepends = userMessage2.projectDetails !== void 0 && userMessage2.projectDetails.sideChat === void 0 && stateHandler.turns.length === 0;
...
${renderMultitaskModeEnterUserReminder(subagentToolName, multitaskModeEnterReminderOptions)}`;
    ...
    if (sendToInteractionListener) {
      await this.interactionListener.sendUpdate(ctx, RedactedUpdates.userMessageAppended(userMessage2));
    }
    const createMainTurnStart = performance.now();
    const turn = await stateHandler.createAgentTurn(ctx, fromRedactedUserMessage2(userMessage2, PrivacyCapability.UNSAFE_ALWAYS_ALLOWED), requestContext, this.config, this.resourceAccessor, {
...
`);
  ...
  async handle(parentCtx, action, rootPromptExecutor, stateHandler, mcpTools, onStateUpdate, options2 = {}) {
    ...
    try {
      const span = __addDisposableResource29(env_1, createSpan(parentCtx.withName("UserMessageActionHandler.handle")), false);
      const ctx = span.ctx;
  ...
  async handleSingleStep(parentCtx, action, rootPromptExecutor, stateHandler, mcpTools, onStateUpdate, options2 = {}) {
    ...
    try {
      const span = __addDisposableResource29(env_2, createSpan(parentCtx.withName("UserMessageActionHandler.handleSingleStep")), false);
      const ctx = span.ctx;
      if (options2.pausedGoalReactivation !== "suppress") {
        await reactivatePausedGoalForTurn(ctx, stateHandler, onStateUpdate);
      }
  ...
  async handleModelStep(parentCtx, action, rootPromptExecutor, stateHandler, mcpTools, onStateUpdate, options2 = {}) {
    ...
    try {
      const span = __addDisposableResource29(env_3, createSpan(parentCtx.withName("UserMessageActionHandler.handleModelStep")), false);
      const ctx = span.ctx;
      if (options2.pausedGoalReactivation !== "suppress") {
        await reactivatePausedGoalForTurn(ctx, stateHandler, onStateUpdate);
      }
