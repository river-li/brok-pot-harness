...
var __addDisposableResource23 = function(env, value, async) {
...
var __disposeResources23 = /* @__PURE__ */ (function(SuppressedError2) {
  return function(env) {
    function fail(e) {
...
function trailingToolBatchHasFailure(messages2) {
...
function isTrailingTaskToolCallMessage(messages2, taskToolName, toolCallIdentityResolver) {
    ...
    get(target, property, receiver) {
      ...
      if (property === "recordReadPath") {
        return (path31) => {
          stateOps.push({
            type: "recordReadPath",
            path: path31.unwrap(PrivacyCapability.UNSAFE_ALWAYS_ALLOWED)
          });
          return target.recordReadPath(path31);
        };
      }
...
function hasEmptyAssistantText(messages2) {
...
function countImagePartsInMessages(messages2) {
...
var AbstractUserMessageActionHandler = class {
  ...
  getAutomationTriggerContext(messages2) {
    ...
    const result = extractAutomationTriggerContext(messages2);
   ...
   * incremental.
   */
  tapAgentResponseComparisonWarmup(ctx, fullStream, messages2, tools) {
    const capability = this.config.agentResponseComparison;
    if (capability?.warm === void 0 || messages2 === void 0 || tools === void 0 || this.pendingResponseComparison === void 0) {
      return fullStream;
    }
    const maybeWarm = () => this.maybeWarmAgentResponseComparison(ctx, capability, messages2, tools);
  ...
  maybeWarmAgentResponseComparison(ctx, capability, messages2, tools) {
  ...
  createAfterAgentThoughtCallback(invocationId, requestContext) {
    return async (hookCtx, params) => {
      ...
      try {
        const span = __addDisposableResource23(env_1, createSpan(hookCtx.withName("agent.lifecycleHook.afterAgentThought")), false);
  ...
  async runStep(parentCtx, turn, rootPromptExecutor, stateHandler, toolsGenerator, mcpTools, repositoryInfos, requestContext, fileOperationLockManager, onStateUpdate, previousQueuedMessageSource, maxOutputTokenRetryDebug) {
    ...
    try {
      const spanCtxt = __addDisposableResource23(env_2, createSpan(parentCtx.withName("runStep")), false);
      ...
      trackPromptTokenUsage({
        ...
        selectedContext: userMessage2.selectedContext,
      ...
      await this.maybeRetryProjectCoordinatorWithoutSendMessage({
        ...
        isSimulatedUserMessage: userMessage2.isSimulatedMsg === true,
      ...
      const sendMessageCallRequirementSatisfied = hasSendMessageCapability && this.config.featureFlags?.sandSendMessageDeliveryOwed !== false && (turn.hasSendMessageCall() || visibilityReminderAlreadyInjected || userMessage2.isSimulatedMsg === true);
  ...
  async runWithMaxTokensRetry(parentCtx, rootPromptExecutor, fn, emptyResponseRetryTurnBudget) {
    ...
    try {
      const spanCtxt = __addDisposableResource23(env_3, createSpan(parentCtx.withName("runWithMaxTokensRetry")), false);
      const ctx = spanCtxt.ctx;
  ...
  async runWithSummarizationRetry(parentCtx, stateHandler, rootPromptExecutor, requestContext, tools, extraT, descriptionProps, fn) {
    ...
    try {
      const spanCtxt = __addDisposableResource23(env_4, createSpan(parentCtx.withName("runWithSummarizationRetry")), false);
      const ctx = spanCtxt.ctx;
  ...
  async runTurnLoop(parentCtx, rootPromptExecutor, stateHandler, initialTurn, toolsGenerator, mcpTools, repositoryInfo, requestContext, onStateUpdate) {
    ...
    try {
      const span = __addDisposableResource23(env_5, createSpan(parentCtx.withName("runTurnLoop")), false);
      const ctx = span.ctx;
      await this.cancelPendingAgentResponseComparison();
      this.responseComparisonPendingUiAttempted = false;
      this.responseComparisonModelStepStarted = false;
      ...
      const userMessage2 = await initialTurn.userMessage.get(ctx);
      stateHandler.setMode(stateHandler.resolveTurnMode(userMessage2));
...
${sanitizedReminder}
  ...
  markRestoredTokenDetailsStaleAfterCompaction(ctx, stateHandler, rootPromptExecutor) {
    ...
    const tokenDetails = stateHandler.tokenDetails;
    const messages2 = rootPromptExecutor.getMessages();
    const wouldLaunchForRestoredCounts = this.orchestrator.shouldStartBackgroundSummarization(tokenDetails, messages2, ctx);
  ...
  async runSingleStep(parentCtx, rootPromptExecutor, stateHandler, turn, toolsGenerator, mcpTools, repositoryInfo, requestContext, onStateUpdate) {
    ...
    try {
      const span = __addDisposableResource23(env_6, createSpan(parentCtx.withName("runSingleStep")), false);
      const ctx = span.ctx;
  ...
  async runModelStep(parentCtx, rootPromptExecutor, stateHandler, turn, toolsGenerator, mcpTools, repositoryInfo, requestContext, onStateUpdate) {
    ...
    try {
      const span = __addDisposableResource23(env_7, createSpan(parentCtx.withName("runModelStep")), false);
      const ctx = span.ctx;
      const { fileOperationLockManager } = await this.setupStep(ctx, stateHandler, turn);
  ...
  async finalizeStep(parentCtx, splitStepData, toolCallResults, rootPromptExecutor, stateHandler, onStateUpdate) {
    ...
    try {
      const span = __addDisposableResource23(env_8, createSpan(parentCtx.withName("finalizeStep")), false);
      const ctx = span.ctx;
      const lastTurnRef = stateHandler.turns[stateHandler.turns.length - 1];
      if (!lastTurnRef) {
        return {
  ...
  async runModelOnlyStep(parentCtx, turn, rootPromptExecutor, stateHandler, toolsGenerator, mcpTools, repositoryInfos, requestContext, fileOperationLockManager, onStateUpdate, maxOutputTokenRetryDebug) {
    ...
    try {
      const spanCtxt = __addDisposableResource23(env_9, createSpan(parentCtx.withName("runModelOnlyStep")), false);
      const ctx = spanCtxt.ctx;
      const invocationId = getInvocationId(ctx);
      stateHandler.lastStepInvocationId = invocationId;
      spanCtxt.span.setAttribute("invocationId", invocationId);
