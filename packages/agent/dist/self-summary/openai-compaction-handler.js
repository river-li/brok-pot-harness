...
var OpenAICompactionHandler = class {
  ...
  async generateSummary(ctx, partitioned, _options) {
    ...
    logger20.info(ctx, "[openai-compaction] generating summary via compaction prompt", {
...
${rawSummary.text}`;
  ...
  async summarize(ctx, messages2, options2) {
    ...
    try {
      const spanCtxt = __addDisposableResource8(env_1, createSpan(ctx.withName("OpenAICompactionHandler.summarize")), false);
      const innerCtx = spanCtxt.ctx;
      const startTime = performance.now();
      logger20.info(innerCtx, "[openai-compaction] summarize starting", {
      ...
      try {
        ...
        logger20.info(innerCtx, "[openai-compaction] summarize completed", {
...
function collectBudgetedUserMessages(messages2, maxTokens) {
  const userMessages = messages2.filter(isNonSummaryUserMessage).map(toCompactionUserMessage).filter((message) => message !== void 0);
  const selected = [];
