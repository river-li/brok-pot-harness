...
function isMcpDirectoryPath(path31) {
  if (path31 === void 0) {
    return false;
  }
  return normalizePathForMcpsLayout(path31).includes(MCP_PATH_SEGMENT);
}
...
function emitDiskMcpDiscoveryFreshness({ ctx, serverIdentifier, canonicalToolNames, observedToolNames }) {
  ...
  if (outcome === "mismatch") {
    logger38.warn(ctx, "mcp_disk_discovery_freshness_mismatch", {
...
async function runDiskMcpDiscoveryFreshness({ ctx, mcpStateAccessor, projectDir, sanitizedServerFolder, readDiskCatalog, state: providedState }) {
  ...
  if (state === void 0) {
    try {
      ...
      logger38.warn(ctx, "mcp_disk_state_read_failed", {
  ...
  try {
    ...
    logger38.warn(ctx, "mcp_disk_catalog_read_failed", {
      error_type: err instanceof Error ? err.name : typeof err
    });
    return;
  }
...
function scheduleDiskMcpDiscoveryFreshnessOnMcpsPathAccess(ctx, mcpStateAccessor, filePath, readDiskCatalog = readMcpDiskCatalogToolNamesForProjectDir) {
  ...
  const parsed2 = parseMcpsPath(filePath);
  if (parsed2 === void 0) {
    return;
  }
  let key;
  let serverFolderForRun;
  const isMcpsRoot = parsed2.kind === "mcps_root";
  if (parsed2.kind === "mcps_root") {
    key = `${parsed2.projectDir}\0__mcps_root__`;
    serverFolderForRun = void 0;
  } else {
    key = `${parsed2.projectDir}\0${parsed2.sanitizedServerFolder}`;
    serverFolderForRun = parsed2.sanitizedServerFolder;
  }
  ...
  const timer = setTimeout(() => {
    ...
    void runFreshnessCheck().catch((err) => {
      logger38.warn(freshnessCtx, "mcp_disk_discovery_freshness_schedule_failed", {
        error_type: err instanceof Error ? err.name : typeof err
      });
    });
