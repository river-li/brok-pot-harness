...
function withTimeout2(promise2, fallback2, timeoutMs = CLIENT_FETCH_TIMEOUT_MS) {
  let timer;
  return Promise.race([
    promise2.catch(() => fallback2),
    new Promise((resolve29) => {
      timer = setTimeout(() => resolve29(fallback2), timeoutMs);
    })
...
var McpFileSystemWriter = class _McpFileSystemWriter {
  constructor(mcpLease, projectDir, options2) {
    ...
    this.leaseChangeDisposable = this.mcpLease.onDidChange((event) => {
      logger36.info(this.ctx, "Lease change event received", {
        serverIdentifiers: event?.serverIdentifiers,
        reason: event?.reason
      });
      this.onLeaseChanged(this.ctx, event);
    });
    logger36.info(this.ctx, "Constructor: scheduling initial write");
  ...
  async cleanupStaleStagingDirs(ctx) {
    ...
    try {
      ...
      if (err?.code !== "ENOENT") {
        logger36.error(logCtx, "Error clearing stale mcps staging dirs", err);
      }
    }
  }
  ...
  async getToolsForServers(ctx, serverIdentifiers) {
    ...
    try {
      ...
      logger36.warn(logCtx, `getToolsForServers() threw, falling back to filtered getTools(): ${err}`);
      const allTools = await this.mcpLease.getTools(ctx).catch((e) => {
        logger36.warn(logCtx, `getTools() threw during scoped-tool fallback, defaulting to []: ${e}`);
        return [];
      });
  ...
  async getEnabledToolsByServer(ctx) {
    ...
    try {
      ...
      logger36.warn(this.getLogContext(ctx), `getEnabledToolsByServer() threw, falling back to raw direct client tool lists: ${err}`);
      return void 0;
    }
  }
  async fetchClientDataForServer(ctx, clientKey, client, options2) {
    ...
    const state = await withTimeout2(client.getState(ctx), {
      ...
      if (s3.kind === "error" && !("message" in s3)) {
        logger36.warn(ctx, `getState() timed out for "${clientKey}", defaulting to error`);
      }
      return s3;
    });
    ...
    const [clientTools, resources, prompts, instructions] = await Promise.all([
      isReachable && this.useDirectClientTools ? withTimeout2(client.getTools(ctx), []).then((tools) => {
        if (tools.length === 0) {
          logger36.info(logCtx, `Direct client getTools() returned 0 tools for "${clientKey}" (state=${state.kind})`);
        }
        return tools;
      }) : Promise.resolve([]),
      ...
      if (serverData.tools.length === 0) {
        ...
        if (fallbackTools && fallbackTools.length > 0) {
          ...
          if (enabledToolNames !== void 0 && filteredFallbackTools.length === 0) {
            logger36.info(ctx, `Skipping direct client tool fallback for "${key}": settings report 0 enabled tools`);
            continue;
          }
          logger36.info(ctx, `Using direct client tool fallback for "${key}": lease had 0 tools, client has ${fallbackTools.length}, writing ${filteredFallbackTools.length}`);
  ...
  buildServersToWrite(ctx, serverDataMap) {
    ...
    for (const server of serverDataMap.values()) {
      ...
      if (!hasContent && !shouldWriteEmptyState) {
        logger36.info(ctx, `Server "${server.serverIdentifier}" excluded from write: tools=${server.tools.length}, resources=${server.resources.length}, prompts=${server.prompts.length}, hasInstructions=${!!server.instructions}, status=${server.status}`);
        continue;
      }
      if (seenSanitizedIds.has(server.sanitizedId)) {
        logger36.warn(ctx, `Skipping server "${server.serverIdentifier}" \u2014 sanitizedId "${server.sanitizedId}" collides with another server`);
        continue;
      }
  ...
  async refreshServersFromLease(ctx, leaseChangeEvent) {
    ...
    const instructions = await this.mcpLease.getInstructions(ctx).catch((err) => {
      logger36.warn(logCtx, `getInstructions() threw during scoped refresh, defaulting to []: ${err}`);
      return [];
    });
    ...
    if (missingServerIdentifiers.length > 0) {
      const allClients = await this.mcpLease.getClients(ctx).catch((err) => {
        logger36.warn(logCtx, `getClients() threw while confirming targeted server removal, preserving cached data: ${err}`);
        return void 0;
      });
    ...
    for (const serverIdentifier of uniqueServerIdentifiers) {
      ...
      if (refreshedServerData !== void 0) {
        if (transientlyUnavailableServerIdentifiers.has(serverIdentifier) && cachedServerData !== void 0) {
          ...
          logger36.warn(logCtx, `refreshServersFromLease: preserving cached metadata for "${serverIdentifier}" because the client was unavailable during targeted refresh`);
          continue;
        }
      ...
      logger36.warn(logCtx, `refreshServersFromLease: preserving cached data for "${serverIdentifier}" because targeted refresh returned no replacement`);
    }
      ...
      try {
        ...
        const [tools, instructions, clients, enabledToolsByServer] = await Promise.all([
          this.mcpLease.getTools(ctx).catch((err) => {
            logger36.warn(logCtx, `getTools() threw, defaulting to []: ${err}`);
            return [];
          }),
          this.mcpLease.getInstructions(ctx).catch(() => []),
          this.mcpLease.getClients(ctx).catch((err) => {
            logger36.warn(logCtx, `getClients() threw, defaulting to {}: ${err}`);
            return {};
          }),
          this.getEnabledToolsByServer(ctx)
        ]);
        const clientKeys = Object.keys(clients);
        logger36.info(logCtx, `fetchAndWrite: lease returned ${tools.length} tools across ${clientKeys.length} clients`);
    ...
    try {
      ...
      for (const server of servers) {
        ...
        if (oldFp !== newFp) {
          toWrite.push(server);
          if (oldFp !== void 0) {
            logger36.info(logCtx, `Server "${server.serverIdentifier}" fingerprint changed: tools=${server.tools.length}, status=${server.status}`);
          }
        }
      }
      for (const server of servers) {
      ...
      for (const [id] of this.cachedFingerprints) {
        if (!newFingerprints.has(id)) {
          ...
          if (!leaseSanitizedIdMap.has(sanitizedId)) {
            logger36.info(logCtx, `Server "${id}" (sanitized="${sanitizedId}") left the lease, scheduling directory removal`);
            toRemove.add(sanitizedId);
          } else {
            logger36.info(logCtx, `Server "${id}" (sanitized="${sanitizedId}") still in lease but excluded from write, preserving directory`);
      ...
      if (toWrite.length > 0 || toRemove.size > 0) {
        ...
        logger36.info(logCtx, `Incremental write: ${toWrite.length} changed, ${toRemove.size} removed, ${servers.length - toWrite.length} unchanged`, {
      ...
      for (let i = 0; i < toWrite.length; i++) {
        ...
        if (result.status === "rejected") {
          ...
          logger36.error(ctx, "Error writing server directory", result.reason, {
            serverIdentifier: server.serverIdentifier,
            sanitizedId: server.sanitizedId
          }, {
            includeStructuredLogs: true
      ...
      for (let i = 0; i < removalResults.length; i++) {
        ...
        if (removalResult.status === "rejected") {
          ...
          logger36.error(ctx, "Error removing server directory", removalResult.reason, {
            sanitizedId,
            removalIndex: i,
  ...
  async getMcpFileSystemOptions(ctx, options2) {
    ...
    try {
      ...
      if (this.snapshotProvider && writeTimedOut) {
        logger36.warn(logCtx, "Snapshot mode write barrier timed out; falling back to cached descriptors", {
  ...
  async ensureWritesSettled(ctx, timeoutMs) {
    ...
    if (hadPendingDebounce) {
      logger36.info(logCtx, "getMcpFileSystemOptions force-fired pending debounce");
    }
    ...
    if (timeoutMs !== void 0) {
      ...
      if (timedOut) {
        logger36.warn(logCtx, `getMcpFileSystemOptions timed out after ${timeoutMs}ms waiting for write; returning stale cache`);
      }
      return timedOut;
    }
    try {
  ...
  async buildOptionsFromSnapshots(ctx) {
    ...
    try {
      ...
      logger36.info(logCtx, `buildOptionsFromSnapshots completed in ${Date.now() - startMs}ms: ${snapshots.length} snapshots, ${mcpDescriptors.length} descriptors, ${totalToolCount} tools`);
  ...
  checkDescriptorsVsFilesystem(ctx, descriptors, source) {
    ...
    void this._checkDescriptorsVsFilesystemAsync(ctx, descriptors, source).catch((err) => {
      logger36.warn(logCtx, "Divergence check failed", err);
    });
  }
  async _checkDescriptorsVsFilesystemAsync(ctx, descriptors, source) {
    ...
    for (const expected of expectedServerIds) {
      if (!actualServerIds.has(expected)) {
        ...
        logger36.warn(logCtx, "Divergence detected: server missing from filesystem", {
          mcpFileSystemMeta: {
    ...
    for (const serverId of expectedServerIds) {
      ...
      if (actualCount !== expectedCount) {
        ...
        logger36.warn(logCtx, "Divergence detected: tool count mismatch", {
          mcpFileSystemMeta: {
    ...
    logger36.info(logCtx, "MCP filesystem sync check", {
      mcpFileSystemMeta: {
        synced,
    ...
    if (!synced) {
      logger36.warn(logCtx, "MCP filesystem divergence summary", {
        mcpFileSystemMeta: {
          synced,
          expectedServers: expectedServerIds.size,
          actualServers: actualServerIds.size,
