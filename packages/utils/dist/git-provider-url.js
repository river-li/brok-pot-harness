...
function isGitHubDotComHost(hostname3) {
  const normalizedHostname2 = hostname3.toLowerCase();
  return normalizedHostname2 === "github.com" || normalizedHostname2.endsWith(".github.com");
}
function isGitLabHost(hostname3) {
  return hostname3.toLowerCase().split(".").includes("gitlab");
}
function isBitbucketCloudHost(hostname3) {
  const normalizedHostname2 = hostname3.toLowerCase();
  return normalizedHostname2 === "bitbucket.org" || normalizedHostname2 === "www.bitbucket.org";
}
function isAzureDevopsServicesHost(hostname3) {
  const normalizedHostname2 = hostname3.toLowerCase();
