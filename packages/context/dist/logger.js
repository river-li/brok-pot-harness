function shouldUseColors(useColors) {
  ...
  return Boolean((_a19 = process.stdout) === null || _a19 === void 0 ? void 0 : _a19.isTTY);
}
...
function resolveRenderOptions(options2) {
  ...
  return {
    indent: " ".repeat((_a19 = options2.indent) !== null && _a19 !== void 0 ? _a19 : DEFAULT_INDENT),
...
function formatTimestamp(timestamp3) {
  return timestamp3.toISOString().slice(11, 23);
}
...
function applyInlineSerializer(value, options2, path31, depth) {
  if (!options2.inlineSerializer) {
    return value;
  }
  try {
    const serialized = options2.inlineSerializer(value, { path: path31, depth });
    return serialized === void 0 ? value : serialized;
  } catch (_a19) {
    return value;
  }
}
function formatInlineValue(value, options2, colors, depth, seen, path31) {
  var _a19;
  var _b2;
  const serialized = applyInlineSerializer(value, options2, path31, depth);
  ...
  if (isErrorLike(value)) {
    return formatErrorSummary(value, options2, colors, depth, seen, path31);
  }
  if (Array.isArray(value)) {
    return formatInlineArray(value, options2, colors, depth, seen, path31);
  }
  ...
  if (typeof ArrayBuffer !== "undefined" && typeof ArrayBuffer.isView === "function" && ArrayBuffer.isView(value)) {
    ...
    const label = (_b2 = (_a19 = typedArray.constructor) === null || _a19 === void 0 ? void 0 : _a19.name) !== null && _b2 !== void 0 ? _b2 : "TypedArray";
    return colors.dim(`${label}(${typedArray.byteLength})`);
  }
  if (value instanceof Map) {
    return formatInlineMap(value, options2, colors, depth, seen, path31);
  }
  if (value instanceof Set) {
    return formatInlineArray(Array.from(value.values()), options2, colors, depth, seen, path31);
  }
...
function formatInlineArray(value, options2, colors, depth, seen, path31) {
  ...
  seen.add(value);
  if (value.length === 0) {
    const empty2 = colors.dim("[]");
    seen.delete(value);
    return empty2;
  }
  const limit = Math.min(value.length, options2.maxArrayLength);
  const items = value.slice(0, limit).map((entry, index) => formatInlineValue(entry, options2, colors, depth + 1, seen, path31.concat(index)));
...
function formatInlineMap(value, options2, colors, depth, seen, path31) {
  if (depth >= options2.maxDepth) {
  ...
  const items = entries.slice(0, limit).map(([key, entryValue]) => {
    const keyValue = formatInlineValue(key, options2, colors, depth + 1, seen, path31.concat("<key>"));
    const renderedValue = formatInlineValue(entryValue, options2, colors, depth + 1, seen, path31.concat(String(key)));
...
function formatInlineObject(value, options2, colors, depth, seen, path31) {
  var _a19;
  var _b2;
  if (depth >= options2.maxDepth) {
    const label = (_b2 = (_a19 = value === null || value === void 0 ? void 0 : value.constructor) === null || _a19 === void 0 ? void 0 : _a19.name) !== null && _b2 !== void 0 ? _b2 : "Object";
  ...
  const limit = Math.min(entries.length, options2.maxEntries);
  const items = entries.slice(0, limit).map(([key, entryValue]) => {
    const renderedValue = formatInlineValue(entryValue, options2, colors, depth + 1, seen, path31.concat(key));
...
function createPrettyTerminalLoggerBackend(options2 = {}) {
  ...
  const outputConsole = (_a19 = options2.console) !== null && _a19 !== void 0 ? _a19 : console;
  return {
    log: (_ctx, entry) => {
      const parts = [];
      ...
      if (renderOptions.includeContextPath && isContext(entry.context)) {
        const path31 = entry.context.getPath();
        if (path31.length > 0) {
          parts.push(colors.dim(`ctx=${path31.join("/")}`));
        }
      }
