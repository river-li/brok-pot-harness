...
function emailTriggerRequiresAuthPass(trigger2) {
  return trigger2.requireAuthPass !== false;
}
...
function triggerListeners(trigger2) {
  return triggerList(trigger2).filter(
    (member) => member.type === "slack" || member.type === "github" || member.type === "origin"
  );
}
function triggerEventTriggers(trigger2) {
  return triggerList(trigger2).filter(
    (member) => member.type !== "cron"
  );
}
function hasTriggerOfType(trigger2, type2) {
  return triggerList(trigger2).some((member) => member.type === type2);
}
function triggerCronSchedules(trigger2) {
  return triggerList(trigger2).flatMap(
    (member) => member.type === "cron" ? [member.schedule] : []
  );
}
function triggerSchedule(trigger2) {
  return triggerCronSchedules(trigger2)[0] ?? null;
}
...
function isConnectCardPlatform(value) {
  return CONNECT_CARD_PLATFORMS.some((platform2) => platform2 === value);
}
var SandWebhookCredentialTargetError = class extends Error {
  constructor(message) {
    super(message);
    this.name = "SandWebhookCredentialTargetError";
  }
};
var SAND_AUTOMATION_RUN_NOW_REFUSED = "automation-run-now/refused";
var SandAutomationRunNowRefusedError = class extends Error {
  failureCode = SAND_AUTOMATION_RUN_NOW_REFUSED;
  constructor(message) {
    super(message);
    this.name = "SandAutomationRunNowRefusedError";
  }
};
