...
var runCommand2 = (0, import_node_util16.promisify)(import_node_child_process15.execFile);
...
async function readLiveDisplays(socketDir) {
  ...
  const numbers = entries.flatMap((entry) => {
    const num2 = X_SOCKET_NAME.exec(entry)?.[1];
    return num2 !== void 0 ? [num2] : [];
  }).sort((left, right) => Number(left) - Number(right));
  const displays = [];
  for (const num2 of numbers) {
    const stats = (0, import_node_fs100.lstatSync)((0, import_node_path177.join)(socketDir, `X${num2}`), { throwIfNoEntry: false });
    if (stats === void 0 || !stats.isSocket()) continue;
    displays.push({ display: `:${num2}`, uid: stats.uid, gid: stats.gid });
  }
  return displays;
}
function createBoxWallpaperCommands(options2) {
  ...
  return {
    isAvailable: (0, import_node_fs100.existsSync)(toneScriptPath) && (0, import_node_fs100.existsSync)(wallpaperCommandPath),
    async resolvePlan() {
      const result = await runCommand2(nodePath, [toneScriptPath, options2.settingsPath], {
        timeout: COMMAND_TIMEOUT_MS2
      }).catch((error42) => {
        options2.log(`wallpaper: tone helper failed (${describe3(error42)})`);
        return void 0;
      });
      ...
      try {
        for (const { display, uid, gid } of await readLiveDisplays(xSocketDir)) {
          await runCommand2(wallpaperCommandPath, ["paint", display], {
            ...
            options2.log(`wallpaper: paint of ${display} failed (${describe3(error42)})`);
          });
        }
      } catch (error42) {
        options2.log(`wallpaper: reading live displays failed (${describe3(error42)})`);
      }
    }
  };
}
