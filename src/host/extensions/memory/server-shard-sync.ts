...
function readText2(path31) {
  return (0, import_node_fs77.existsSync)(path31) ? (0, import_node_fs77.readFileSync)(path31, "utf8") : "";
}
function listLogFileNames(logDir) {
  return (0, import_node_fs77.existsSync)(logDir) ? (0, import_node_fs77.readdirSync)(logDir).filter((name17) => MEMORY_LOG_FILE_NAME.test(name17)).sort() : [];
}
...
function writeFolder(dir, folder) {
  const logDir = (0, import_node_path125.join)(dir, MEMORY_LOG_DIRNAME);
  (0, import_node_fs77.mkdirSync)(logDir, { recursive: true });
  if (folder.profile.length > 0) {
    writeFileAtomicSync((0, import_node_path125.join)(dir, MEMORY_PROFILE_FILENAME), folder.profile);
  } else {
    (0, import_node_fs77.rmSync)((0, import_node_path125.join)(dir, MEMORY_PROFILE_FILENAME), { force: true });
  }
  ...
  for (const name17 of listLogFileNames(logDir)) {
    if (!keep.has(name17)) (0, import_node_fs77.rmSync)((0, import_node_path125.join)(logDir, name17), { force: true });
  }
}
