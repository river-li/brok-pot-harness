...
function auditJsonlPath(agentId) {
  return (0, import_node_path30.join)(resolveSandAgentDir(agentId), "audit.jsonl");
}
...
function createSandActionAuditor(deps) {
  ...
  const appendLocalLine = deps.appendLocalLine ?? (async (path31, line) => {
    await (0, import_promises24.mkdir)((0, import_node_path30.dirname)(path31), { recursive: true });
    await (0, import_promises24.appendFile)(path31, line, "utf8");
  });
  const loadOutbox = async () => {
    if (outboxLoaded) return;
    outboxLoaded = true;
    try {
      const raw = await (0, import_promises24.readFile)(outboxPath(), "utf8");
  ...
  const persistOutbox = async () => {
    try {
      ...
      if (pending.length === 0) {
        await (0, import_promises24.rm)(path31, { force: true });
        return;
      }
