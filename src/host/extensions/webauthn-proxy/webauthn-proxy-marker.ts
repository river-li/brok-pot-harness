...
function applyWebAuthnProxyMarker(enabled, markerPath = SAND_WEBAUTHN_PROXY_MARKER_PATH, policyCommand = BOX_CHROME_POLICY_COMMAND) {
  if (!(0, import_node_fs100.existsSync)((0, import_node_path178.dirname)(markerPath))) {
    return "not-a-box";
  }
  const present = (0, import_node_fs100.existsSync)(markerPath);
  if (present === enabled) {
    return "unchanged";
  }
  if (enabled) {
    (0, import_node_fs100.writeFileSync)(markerPath, "", { encoding: "utf8", mode: 420 });
  } else {
    (0, import_node_fs100.unlinkSync)(markerPath);
  }
  if ((0, import_node_fs100.existsSync)(policyCommand)) {
