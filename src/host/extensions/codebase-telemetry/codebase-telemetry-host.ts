...
function createSandCodebaseTelemetryHost({
  ...
  const authSource = disposables.add(createAuthSource({ auth: auth2, logger: logger107 }));
  const privacyModeSource = createPrivacyModeSource(authSource.receiver, privacyMode);
  const featureGates = disposables.add(createFeatureGateSource({ experiments, logger: logger107 }));
...
function createAuthSource({
  ...
  const [sender, receiver] = createWatchChannel({
    initialValue: void 0,
    onSubscriberError: (error41) => logger107.error("Authentication subscriber failed", error41)
  });
...
function createFeatureGateSource({
  ...
  const [sender, changes] = createEventChannel({
    onSubscriberError: (error41) => logger107.error("Feature gate subscriber failed", error41)
  });
