...
function getPluginsRootDir(sandRoot) {
  return (0, import_node_path116.join)(sandRoot, "plugins");
}
function getPluginSkillsDir(sandRoot) {
  return (0, import_node_path116.join)(sandRoot, PLUGIN_SKILLS_DIRNAME);
}
function getPluginSkillsCachePath(cacheDir) {
  return (0, import_node_path116.join)(cacheDir, PLUGIN_SKILLS_CACHE_FILENAME);
}
...
function isPluginSkillRecord(value) {
  ...
  return typeof value.id === "string" && isSafePluginSkillId(value.id) && typeof value.pluginId === "string" && value.pluginId.length > 0 && typeof value.pluginName === "string" && typeof value.name === "string" && value.name.length > 0 && typeof value.description === "string" && typeof value.filePath === "string" && (0, import_node_path116.isAbsolute)(value.filePath);
}
...
function withProvenanceDefaults(record2) {
  return {
    ...
    installPath: typeof record2.installPath === "string" && (0, import_node_path116.isAbsolute)(record2.installPath) ? record2.installPath : "",
...
function readPluginSkillsCache(cacheDir) {
  try {
    const parsed2 = JSON.parse(
      (0, import_node_fs68.readFileSync)(getPluginSkillsCachePath(cacheDir), "utf8")
    );
...
`;
  (0, import_node_fs68.mkdirSync)(cacheDir, { recursive: true, mode: AGENT_READABLE_SKILL_DIR_MODE });
  (0, import_node_fs68.chmodSync)(cacheDir, AGENT_READABLE_SKILL_DIR_MODE);
