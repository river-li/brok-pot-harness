...
def check(path: Path) -> bool:
    ...
    print("syntax: {}".format(relative), flush=True)
...
def main() -> int:
    ...
    if failures:
        print(
            "JavaScript syntax failures: " + ", ".join(str(path.relative_to(ROOT)) for path in failures),
            file=sys.stderr,
        )
        return 1
    print("PASS JavaScript syntax: {} maintained and built files".format(len(files)))
    return 0
