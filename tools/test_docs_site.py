...
class DocumentationSiteTest(unittest.TestCase):
    def setUp(self):
        ...
        files = {
            'README.md': '<a href="docs/wiki/Home.md">Docs</a><img src="assets/branding/icon.png">\n',
            'README.zh.md': '<a href="docs/wiki/Home.md">文档</a>\n',
    ...
    def test_workflow_checks_every_pr_and_mermaid_uses_fence_text(self):
        ...
        renderer = (site.ROOT / 'docs/assets/javascripts/mermaid-init.js').read_text()
        self.assertIn('mermaid.initialize({ startOnLoad: false', renderer)
        self.assertIn('mermaid.render(`gbh-mermaid-${index}`, diagram.textContent.trim())', renderer)
        self.assertNotIn('mermaid.run(', renderer)
