...
Use Git, Python 3, Node/npm, and the GitHub CLI (`gh`) authenticated for this repository's read operations. The checkout must have a GitHub.com `origin` whose canonical repository default branch is `main`. Task setup also needs permission to fetch origin/main and create linked worktrees. No private Codex skill, coordinator chat history, or custom plugin is required; the repository skill and this guide contain the workflow instructions.
