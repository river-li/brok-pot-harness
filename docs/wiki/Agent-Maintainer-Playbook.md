...
Write the human review summary in Chinese. Name the reviewed head and base. Separate blocking findings from non-blocking observations and point to files and lines where possible. A verdict applies only to that head/base pair. Any changed head or base returns the PR to review:pending; the author fixes findings and the reviewer checks the new published head.
